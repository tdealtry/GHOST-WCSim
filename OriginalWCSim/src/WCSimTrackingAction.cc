...
WCSimTrackingAction::WCSimTrackingAction() {
	...
	// ProcessList.insert("muMinusCaptureAtRest") ;          // Includes Muon decay from K-shell: for Michel
	// e- from mu0. This dominates/replaces the mu- decay (noticed when switching off this process in
	// PhysicsList)                                                   // TF: IMPORTANT: ONLY USE FROM G4.9.6
	// onwards because buggy/double counting before.
