...
G4ClassificationOfNewTrack WCSimStackingAction::ClassifyNewTrack(const G4Track* aTrack) {
	...
	if(particleType == G4OpticalPhoton::OpticalPhotonDefinition()) {
		   ...
		   (aTrack->GetCreatorProcess() != NULL &&
			...
			if(!DetConstruct->GetIsCombinedPMTCollectionDefined()) {
				...
				if(DetConstruct->GetPMT_QE_Method() == 1) {
					wavelengthQE =
					    DetConstruct->GetPMTQE(WCIDCollectionName, photonWavelength, 1, 240, 660, ratio);
					// wavelengthQE  = DetConstruct->GetPMTQE(volumeName,photonWavelength,1,240,660,ratio);
				}
				else if(DetConstruct->GetPMT_QE_Method() == 2) {
					wavelengthQE =
					    DetConstruct->GetPMTQE(WCIDCollectionName, photonWavelength, 0, 240, 660, ratio);
		...
		}  // aTrack->GetCreatorProcess() == NULL || (aTrack->GetCreatorProcess()))->GetProcessType() !=
		   // fOptical
	}  // optical photon
