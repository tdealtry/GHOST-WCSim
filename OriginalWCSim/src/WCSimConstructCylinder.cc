...
G4LogicalVolume* WCSimDetectorConstruction::ConstructCylinder() {
	...
	G4cout << WCIDRadius << ", " << WCPMTExposeHeight << ", " << mPMT_vessel_cyl_height << ", "
	       << mPMT_vessel_radius << G4endl;
#ifdef DEBUG
	...
	if(hybrid) {
		if(mPMT_vessel_cyl_height + mPMT_vessel_radius < 1. * mm)
			...
			innerAnnulusRadius = WCIDRadius -
			                     std::max(WCPMTExposeHeight, (mPMT_vessel_cyl_height + mPMT_vessel_radius)) -
			                     1. * mm;
	}
	else {
		if(mPMT_vessel_cyl_height + mPMT_vessel_radius < 1. * mm)
	...
	if(isODConstructed) {
		const G4double sphereRadius =
		    (WCPMTODExposeHeight * WCPMTODExposeHeight + WCPMTODRadius * WCPMTODRadius) /
		    (2 * WCPMTODExposeHeight);
		WCODRadius = WCIDRadius + WCBlackSheetThickness + WCODDeadSpace +  // ID Structure
	...
	G4cout << "GEOMCHECK3 WCIDRadius \t" << WCIDRadius << G4endl << "GEOMCHECK3 WCBlackSheetThickness \t"
	       << WCBlackSheetThickness << G4endl << "GEOMCHECK3 WCODDeadSpace \t" << WCODDeadSpace << G4endl
	       ...
	       << "GEOMCHECK3 WCPMTODExposeHeight \t" << WCPMTODExposeHeight << G4endl
	       << "GEOMCHECK3 WCPMTODRadius \t" << WCPMTODRadius << G4endl << "GEOMCHECK3 WCPMTODRadius \t"
	       << WCPMTODRadius << G4endl << "GEOMCHECK3 WCPMTODExposeHeight \t" << WCPMTODExposeHeight << G4endl;
#endif
	...
	WCLength = WCIDHeight + 2 * (WCODHeightWaterDepth + WCBlackSheetThickness + WCODDeadSpace +
	                             WCODTyvekSheetThickness + 1 * mm);
	WCRadius = (outerAnnulusRadius + WCODLateralWaterDepth) / cos(dPhi / 2.);
#ifdef WCSIMCONSTRUCTCYLINDER_VERBOSE
	G4cout << "GEOMCHECK2 WCLength \t" << WCLength << G4endl << "GEOMCHECK2 WCIDHeight \t" << WCIDHeight
	       << G4endl << "GEOMCHECK2 WCODHeightWaterDepth \t" << WCODHeightWaterDepth << G4endl
	       ...
	       << "GEOMCHECK2 WCRadius \t" << WCRadius << G4endl << "GEOMCHECK2 outerAnnulusRadius \t"
	       << outerAnnulusRadius << G4endl << "GEOMCHECK2 WCODLateralWaterDepth \t" << WCODLateralWaterDepth
	       << G4endl;
#endif
	// WCLength    = WCIDHeight + (WCODHeight+WCOuterStructure)*2;	//jl145 - reflects top veto blueprint, cf.
	// Farshid Feyzi WCRadius    = (WCIDDiameter/2. + WCBlackSheetThickness +
	// (WCODRadius+WCODHeight)))/cos(dPhi/2.) ; // BQ: Updated with new HK OD size (2020/12/06) WCLength    =
	// WCIDHeight + 2*2.3*m;	//jl145 - reflects top veto blueprint, cf. Farshid Feyzi WCRadius    =
	// (WCIDDiameter/2. + WCBlackSheetThickness + 1.5*m)/cos(dPhi/2.) ; // TODO: OD
	...
	G4cout << "Computed tolerance = " << G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / mm
	       << " mm" << G4endl;
	...
	if(WCAddGd) {
		water = "Doped Water";
		if(!G4Material::GetMaterial("Doped Water", false))
			AddDopedWater();
	}
	...
	G4LogicalVolume* logicWC = new G4LogicalVolume(solidWC, G4Material::GetMaterial("Air"), "WC", 0, 0, 0);
	...
	// G4cout << " qqqqqqqqqqqqqqqqqqqq " << " WCRadius " << WCRadius << " WCBarrel radius " << WCRadius+1.*m
	// << " half height "  << .5*WCLength << G4endl;

	G4LogicalVolume* logicWCBarrel =
	    new G4LogicalVolume(solidWCBarrel, G4Material::GetMaterial(water), "WCBarrel", 0, 0, 0);
	...
	if(isODConstructed) {
		...
		G4LogicalVolume* logicCaveTyvek =
		    new G4LogicalVolume(solidCaveTyvek, G4Material::GetMaterial("Tyvek"), "CaveTyvek", 0, 0, 0);
		...
		showTyvekCave->SetForceWireframe(
		    true);  // This line is used to give definition to the rings in OGLSX Visualizer
		...
		G4Tubs* solidCaveCapsTyvek =
		    new G4Tubs("CaveCapsTyvek", 0, WCRadius, .5 * (WCODTyvekSheetThickness), 0. * deg, 360. * deg);

		G4LogicalVolume* logicCaveCapsTyvek = new G4LogicalVolume(solidCaveCapsTyvek,
	...
	G4LogicalVolume* logicWCBarrelAnnulus =
	    new G4LogicalVolume(solidWCBarrelAnnulus, G4Material::GetMaterial(water), "WCBarrelAnnulus", 0, 0, 0);
	...
	//-----------------------------------------------------
	G4double RingZ[2] = { -barrelCellHeight / 2., barrelCellHeight / 2. };
	...
	G4LogicalVolume* logicWCBarrelRing =
	    new G4LogicalVolume(solidWCBarrelRing, G4Material::GetMaterial(water), "WCBarrelRing", 0, 0, 0);
	...
	G4LogicalVolume* logicWCBarrelCell =
	    new G4LogicalVolume(solidWCBarrelCell, G4Material::GetMaterial(water), "WCBarrelCell", 0, 0, 0);

	G4VPhysicalVolume* physiWCBarrelCell = new G4PVReplica("WCBarrelCell",
	                                                       logicWCBarrelCell,
	                                                       logicWCBarrelRing,
	                                                       kPhi,
	                                                       WCBarrelRingNPhi,
	...
	G4double annulusBlackSheetRmin[2] = { (WCIDRadius), WCIDRadius };
	...
	G4LogicalVolume* logicWCBarrelCellBlackSheet = new G4LogicalVolume(solidWCBarrelCellBlackSheet,
	...
	G4VPhysicalVolume* physiWCBarrelCellBlackSheet = new G4PVPlacement(0,
	                                                                   G4ThreeVector(0., 0., 0.),
	...
	new G4LogicalSkinSurface("BSBarrelCellSkinSurface", logicWCBarrelCellBlackSheet, BSSkinSurface);
#endif
	...
	if(Vis_Choice == "RayTracer") {
		...
		WCBarrelBlackSheetCellVisAtt->SetForceSolid(
		    true);  // force the object to be visualized with a surface
	...
	if(!(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)) {
		...
		G4cerr << "The extra tower is not correctly implemented. It causes geometry overlaps," << G4endl
		       ...
		       << "You are strongly recommended to use $WCSIMDIR/sample-root-scripts/calcPhotoCoverage.C"
		       << G4endl << " in order to produce a photocoverage that will give you a valid geometry."
		       << G4endl
		       << "Also be warned that, due to how PMTs are laid out in WCSim, you should be careful"
		       << G4endl << " when using a new geometry, especially if it is a HK hybrid geometry with"
		       << G4endl << " a low number of of mPMTs." << G4endl
		       << "You should check that all ID PMTs are laid out in a sensible pattern for your needs,"
		       << G4endl << " and that the number of hits produced is sensible." << G4endl;
		...
		for(int i = 0; i < 2; i++) {
			extraTowerRmin[i] = mainAnnulusRmin[i] != 0 ?
			                        mainAnnulusRmin[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) :
			                        0.;
			extraTowerRmax[i] = mainAnnulusRmax[i] != 0 ?
			                        mainAnnulusRmax[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) :
			                        0.;
		}
		G4Polyhedra* solidWCExtraTower =
		    new G4Polyhedra("WCextraTower",
		...
		G4LogicalVolume* logicWCExtraTower =
		    new G4LogicalVolume(solidWCExtraTower, G4Material::GetMaterial(water), "WCExtraTower", 0, 0, 0);
		...
		G4Polyhedra* solidWCExtraTowerCell = new G4Polyhedra(
		    "WCExtraTowerCell",
		    totalAngle - 2. * pi,  //+dPhi/2., // phi start
		    2. * pi - totalAngle -
		        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // phi end
		    1,                                                                          // NPhi-gon
		    2,
		    RingZ,
		    extraTowerRmin,
		...
		logicWCExtraTowerCell               = new G4LogicalVolume(solidWCExtraTowerCell,
                                                    G4Material::GetMaterial(water),
                                                    "WCExtraTowerCell",
                                                    0,
                                                    0,
                                                    0);
		G4VPhysicalVolume* physiWCTowerCell = new G4PVReplica("extraTowerCell",
		...
		G4Polyhedra* solidWCTowerBlackSheet = new G4Polyhedra(
		...
		G4LogicalVolume* logicWCTowerBlackSheet = new G4LogicalVolume(solidWCTowerBlackSheet,
		                                                              G4Material::GetMaterial("Blacksheet"),
		                                                              "WCExtraTowerBlackSheet",
		                                                              0,
		                                                              0,
		...
		G4VPhysicalVolume* physiWCTowerBlackSheet = new G4PVPlacement(0,
		                                                              G4ThreeVector(0., 0., 0.),
		...
		new G4LogicalSkinSurface("BSTowerCellSkinSurface", logicWCTowerBlackSheet, BSSkinSurface);

		// These lines add color to the blacksheet in the extratower. If using RayTracer, comment the first
		// chunk and use the second. The Blacksheet should be green.
	...
	if(WCTopVeto) {
		...
		solidWCTopVeto = new G4Tubs("WCTopVeto",
		                            0.0 * m,
		...
		logicWCTopVeto =
		    new G4LogicalVolume(solidWCTopVeto, G4Material::GetMaterial(water), "WCTopVeto", 0, 0, 0);

		// G4VPhysicalVolume* physiWCTopVeto =
		new G4PVPlacement(0,
		                  G4ThreeVector(0., 0., WCIDHeight / 2 + 1.0 * m),
		                  logicWCTopVeto,
		                  "WCTopVeto",
		                  logicWCBarrel,
		                  false,
		...
		solidWCTVTyvek =
		    new G4Tubs("WCTVTyvek", 0.0 * m, WCIDRadius, WCTyvekThickness / 2, 0. * deg, 360. * deg);

		G4LogicalVolume* logicWCTVTyvek =
		    new G4LogicalVolume(solidWCTVTyvek, G4Material::GetMaterial("Tyvek"), "WCTVTyvek", 0, 0, 0);

		// Bottom
		// G4VPhysicalVolume* physiWCTVTyvekBot =
		new G4PVPlacement(0,
		                  G4ThreeVector(0., 0., -0.5 * m - WCTyvekThickness / 2),
		                  logicWCTVTyvek,
		                  "WCTVTyvekBot",
		                  logicWCTopVeto,
		                  false,
		...
		solidWCTVTyvekSide = new G4Tubs("WCTVTyvekSide",
		                                WCIDRadius,
		                                WCIDRadius + WCTyvekThickness,
		                                0.5 * m + WCTyvekThickness,
		                                0. * deg,
		...
		G4LogicalVolume* logicWCTVTyvekSide = new G4LogicalVolume(solidWCTVTyvekSide,
		                                                          G4Material::GetMaterial("Tyvek"),
		                                                          "WCTVTyvekSide",
		                                                          0,
		                                                          0,
	...
	if(nID_PMTs <= 1)
		logicWCPMT = ConstructPMT(WCPMTName, WCIDCollectionName, "tank", nID_PMTs);
	else
		logicWCPMT = ConstructMultiPMT(WCPMTName, WCIDCollectionName, "tank", nID_PMTs);
	if(!logicWCPMT) {
	...
	if(hybrid) {
		...
		if(nID_PMTs2 <= 1)
			logicWCPMT2 = ConstructPMT(WCPMTName2, WCIDCollectionName2, "tankPMT2", nID_PMTs2);
		else
			logicWCPMT2 = ConstructMultiPMT(WCPMTName2, WCIDCollectionName2, "tankPMT2", nID_PMTs2);
		if(!logicWCPMT2) {
			G4cerr << "Overlapping PMTs in multiPMT" << G4endl;
			return NULL;
		}
	...
	/*These lines of code will give color and volume to the PMTs if it hasn't been set in
	  WCSimConstructPMT.cc. I recommend setting them in WCSimConstructPMT.cc. If used here, uncomment the
	  SetVisAttributes(WClogic) line, and comment out the SetVisAttributes(G4VisAttributes::Invisible) line.*/
	...
	if(WCTopVeto) {
		...
		for(int i = -TVNCell; i < TVNCell; i++) {
			for(int j = -TVNCell; j < TVNCell; j++) {
				...
				G4ThreeVector cellpos = G4ThreeVector(xoffset, yoffset, -0.5 * m);

				if((sqrt(xoffset * xoffset + yoffset * yoffset) + hybrid ?
				        std::max(WCPMTRadius, WCPMTRadius2) :
				        WCPMTRadius) < WCTVEdgeLimit) {  // B.Q: for hybrid configuration
					...
					new G4PVPlacement(
					    ...
					    (horizontalModulo == verticalModulo && hybrid && WCPMTPercentCoverage2 != 0) ?
					        logicWCPMT2 :
		...
		G4double WCTVEfficiency = icopy * WCPMTRadius *
		                          (hybrid ? std::max(WCPMTRadius, WCPMTRadius2) : WCPMTRadius) /
		                          ((WCIDRadius) * (WCIDRadius));
	...
	if(placeBarrelPMTs) {
...
#ifdef DEBUG
		G4cout << "Debug B.Q : Number of PMT horizontal = " << WCPMTperCellHorizontal
		       << ", number of PMT vertical = " << WCPMTperCellVertical << G4endl;
		G4cout << "Debug B.Q : Horizontal spacing = " << horizontalSpacing
		       << " vertical spacing = " << verticalSpacing << G4endl;
#endif
		...
		for(G4long i = 0; i < WCPMTperCellHorizontal; i++) {
			for(G4long j = 0; j < WCPMTperCellVertical; j++) {
				...
				new G4PVPlacement(WCPMTRotation,  // its rotation
				                  PMTPosition,
				                  (i == j && hybrid && WCPMTPercentCoverage2 != 0) ?
				                      logicWCPMT2 :
				                      logicWCPMT,     // its logical volume
				                  pmtname,            //"WCPMT",             // its name
		...
		if(!(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)) {
			...
			G4cout << "Debug B.Q : WCBarrelRingNPhi = " << WCBarrelRingNPhi
			       << ", WCBarrelNumPMTHorizontal = " << WCBarrelNumPMTHorizontal << G4endl;
#endif
			...
			G4double horizontalSpacingExtra =
			    towerWidth / (WCBarrelNumPMTHorizontal - WCBarrelRingNPhi * WCPMTperCellHorizontal);
			// verticalSpacing is identical to that for normal cell.

			for(G4long i = 0; i < (WCBarrelNumPMTHorizontal - WCBarrelRingNPhi * WCPMTperCellHorizontal);
			    i++) {
				for(G4long j = 0; j < WCPMTperCellVertical; j++) {
					G4ThreeVector PMTPosition =
					    G4ThreeVector(WCIDRadius / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.),
	...
	if(isODConstructed) {
		...
		G4double odTopCapZ[4] = { (-WCODDeadSpace + 1 * mm + WCBlackSheetThickness),
			                      -.5 * (WCODTyvekSheetThickness),
			                      -.5 * (WCODTyvekSheetThickness),
			                      .5 * (WCODTyvekSheetThickness) };
		G4double odBotCapZ[4] = { -(-WCODDeadSpace + 1 * mm + WCBlackSheetThickness),
			                      .5 * (WCODTyvekSheetThickness),
			                      .5 * (WCODTyvekSheetThickness),
			                      -.5 * (WCODTyvekSheetThickness) };
		...
		if(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal) {
			solidWCODTopCapTyvek = new G4Polyhedra("WCODTopCapTyvek",
			                                       0. * deg,          // phi start
			                                       totalAngle,        // phi end
			                                       WCBarrelRingNPhi,  // NPhi-gon
			                                       4,                 // 4 z-planes
			...
			solidWCODBotCapTyvek = new G4Polyhedra("WCODBotCapTyvek",
			                                       0. * deg,          // phi start
			                                       totalAngle,        // phi end
			                                       WCBarrelRingNPhi,  // NPhi-gon
			                                       4,                 // 4 z-planes
			...
			G4Polyhedra* mainPartTop = new G4Polyhedra("WCODTopCapTyvekMainPart",
			                                           0. * deg,          // phi start
			                                           totalAngle,        // phi end
			                                           WCBarrelRingNPhi,  // NPhi-gon
			                                           4,                 // 4 z-planes
			...
			G4Polyhedra* mainPartBot = new G4Polyhedra("WCODBotCapTyvekMainPart",
			                                           0. * deg,          // phi start
			                                           totalAngle,        // phi end
			                                           WCBarrelRingNPhi,  // NPhi-gon
			                                           4,                 // 4 z-planes
			...
			for(int i = 0; i < 4; i++) {
				extraCapRmin[i] = odCapRmin[i] != 0. ?
				                      odCapRmin[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) :
				                      0.;
				extraCapRmax[i] = odCapRmax[i] != 0. ?
				                      odCapRmax[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) :
				                      0.;
			}
			G4Polyhedra* extraSliceTop = new G4Polyhedra(
			    "WCODTopCapTyvekExtraSlice",
			    totalAngle - 2. * pi,  // phi start
			    2. * pi - totalAngle -
			        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // total phi
			...
			G4Polyhedra* extraSliceBot = new G4Polyhedra(
			    "WCODBotCapTyvekExtraSlice",
			    totalAngle - 2. * pi,  // phi start
			    2. * pi - totalAngle -
			        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // total phi
			    // fortunately there are no PMTs an the gap!
			...
			solidWCODTopCapTyvek = new G4UnionSolid("WCODTopCapTyvek", mainPartTop, extraSliceTop);

			solidWCODBotCapTyvek = new G4UnionSolid("WCODBotCapTyvek", mainPartBot, extraSliceBot);
		}
		...
		G4LogicalVolume* logicWCODTopCapTyvek = new G4LogicalVolume(solidWCODTopCapTyvek,
		                                                            G4Material::GetMaterial("Tyvek"),
		                                                            "WCODTopCapTyvek",
		                                                            0,
		                                                            0,
		                                                            0);
		G4LogicalVolume* logicWCODBotCapTyvek = new G4LogicalVolume(solidWCODBotCapTyvek,
		                                                            G4Material::GetMaterial("Tyvek"),
		                                                            "WCODBotCapTyvek",
		                                                            0,
		                                                            0,
		...
		G4VisAttributes* WCCapsODTyvekCellVisAtt = new G4VisAttributes(yellow);
		...
		G4double annulusODTyvekRmax[2] = { (WCODRadius), WCODRadius };
		...
		G4LogicalVolume* logicWCBarrelCellODTyvek = new G4LogicalVolume(solidWCBarrelCellODTyvek,
		                                                                G4Material::GetMaterial("Tyvek"),
		                                                                "WCBarrelCellODTyvek",
		                                                                0,
		                                                                0,
		...
		G4VisAttributes* WCBarrelODTyvekCellVisAtt = new G4VisAttributes(yellow);
		...
		G4double NPMTODByCellFull =
		    NPMTODCovered / WCBarrelRingNPhi;  // NPMT required par cell to achieve ODPercentOverage
		G4double NPMTODByCell =
		    round(NPMTODCovered / WCBarrelRingNPhi);  // NPMT required par cell to achieve ODPercentOverage
		...
		WCODCapPMTSpacing =
		    (pi * WCIDDiameter /
		     (round(WCIDDiameter * sqrt(pi * WCPMTODPercentCoverage) / (10.0 * WCPMTODRadius))));
		...
		if(WCODPMTShift > barrelODCellWidth / 2. - WCPMTODRadius)
			WCODPMTShift = 0. * cm;
		...
		for(G4long i = 0; i < WCPMTODperCellHorizontal; i++) {
			for(G4long j = 0; j < WCPMTODperCellVertical; j++) {
				...
				G4ThreeVector Container =
				    G4ThreeVector(WCODRadius,
				                  -barrelODCellWidth / 2. + (i + 0.5) * horizontalODSpacing +
				                      ((G4int)(std::pow(-1, j)) * (G4int)(WCODPMTShift) / 2),
				                  -(barrelCellHeight * (barrelODCellWidth / barrelCellWidth)) / 2. +
		...
		if(!(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)) {
			...
			for(int i = 0; i < 2; i++) {
				towerODTyvekRmin[i] =
				    annulusODTyvekRmin[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.);
				towerODTyvekRmax[i] =
				    annulusODTyvekRmax[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.);
			}
			G4Polyhedra* solidWCTowerODTyvek = new G4Polyhedra(
			    "WCExtraTowerODTyvek",
			    totalAngle - 2. * pi,  //+dPhi/2., // phi start
			    2. * pi - totalAngle -
			        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // phi end
			...
			G4LogicalVolume* logicWCTowerODTyvek = new G4LogicalVolume(solidWCTowerODTyvek,
			                                                           G4Material::GetMaterial("Tyvek"),
			                                                           "WCExtraTowerODTyvek",
			                                                           0,
			                                                           0,
			...
			WCExtraODPMTRotation->rotateX(2 * pi - (2 * pi - totalAngle) / 2.);  // align the PMT with the
			                                                                     // Cell

			G4double towerWidthOD  = WCODRadius * tan(2 * pi - totalAngle);
			// We don't want the same number of OD PMTs squished horizontally so we scale down the horizontal
			// PMTs by the width of the extra tower
			...
			for(G4long i = 0; i < (WCPMTODperCellHorizontalExtra); i++) {
				for(G4long j = 0; j < WCPMTODperCellVertical; j++) {
					G4cout << "Adding OD PMT in cell " << i << ", " << j << G4endl;
					G4ThreeVector Container = G4ThreeVector(
					    (WCODRadius) / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.),
		...
		for(int i = -CapNCell; i < CapNCell; i++) {
			for(int j = -CapNCell; j < CapNCell; j++) {
				...
				G4ThreeVector topWLSpos =
				    G4ThreeVector(xoffset,
				                  yoffset,
				                  ((WCIDHeight + 2 * WCODDeadSpace) / 2) + WCODTyvekSheetThickness);

				G4ThreeVector bottomWLSpos = G4ThreeVector(xoffset, yoffset, -topWLSpos.getZ());

				if(((sqrt(xoffset * xoffset + yoffset * yoffset) + WCPMTODRadius) < WCODCapEdgeLimit)) {

					//		  G4cout << " qqqqqqqqqqqqqqqqqqqqqqqq cap i " << i << " of " << CapNCell << " j "
					//<< j << " of " << CapNCell << " Container (" << topWLSpos.x() << ", " << topWLSpos.y()
					//				  << ", " << topWLSpos.z() << ") " << G4endl;
		...
		G4cout << " Coverage was calculated to be: "
		       << (icopy * WCPMTODRadius * WCPMTODRadius / (WCIDRadius * WCIDRadius)) << "\n";
		G4cout << "############"
		       << "\n";
	...
	// These lines make the large cap volume invisible to view the caps blacksheets. Need to make invisible
	// for RayTracer
	if(Vis_Choice == "RayTracer") {
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructCaps(G4bool flipz) {
	...
	const G4String bbstr  = G4String("Barrel") + oristr + G4String("Border");  // "Barrel[Top|Bot]Border"
	...
	G4LogicalVolume* logicCapAssembly =
	    new G4LogicalVolume(solidCapAssembly, G4Material::GetMaterial(water), caname, 0, 0, 0);
	...
	if(std::abs(borderAnnulusZ[2] - borderAnnulusZ[0]) > capAssemblyHeight - WCBlackSheetThickness) {
		G4cerr << "IMPOSSIBLE GEOMETRY:  capAssemblyHeight (" << capAssemblyHeight
		       << ") too small to contain border ring (" << std::abs(borderAnnulusZ[2] - borderAnnulusZ[0])
		       << ") and endcap blacksheet (" << WCBlackSheetThickness << ")" << G4endl;
		/// Should throw an exception.
	}

	const G4String bbrname = G4String("WC") + bbstr + G4String("Ring");  // "WCBarrel[Top|Bot]BorderRing"
	...
	G4LogicalVolume* logicWCBarrelBorderRing =
	    new G4LogicalVolume(solidWCBarrelBorderRing, G4Material::GetMaterial(water), bbrname, 0, 0, 0);
	...
	// --------------------------------------------------
	const G4String bbcname = G4String("WC") + bbstr + G4String("Cell");  // "WCBarrel[Top|Bot]BorderCell"
	...
	G4LogicalVolume* logicWCBarrelBorderCell =
	    new G4LogicalVolume(solidWCBarrelBorderCell, G4Material::GetMaterial(water), bbcname, 0, 0, 0);
	// G4cout << *solidWCBarrelBorderCell << G4endl;
	G4VPhysicalVolume* physiWCBarrelBorderCell = new G4PVReplica(bbcname,
	...
	G4Polyhedra* solidWCBarrelBorderCellBlackSheet = new G4Polyhedra(bbcbsname,
	                                                                 -dPhi / 2.,  // phi start
	                                                                 dPhi,        // total phi
	                                                                 1,           // NPhi-gon
	                                                                 2,
	...
	new G4LogicalSkinSurface(bbcwbssname, logicWCBarrelBorderCellBlackSheet, BSSkinSurface);

	// we have to declare the logical Volumes
	// outside of the if block to access it later on
	G4LogicalVolume* logicWCExtraBorderCell   = nullptr;
	G4VPhysicalVolume* physiWCExtraBorderCell = nullptr;
	const G4String etbcname =
	    G4String("WCExtraTower") + oristr + G4String("BorderCell");  // "WCExtraTower[Top|Bot]]BorderCell"
	if(!(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)) {
		//----------------------------------------------
		...
		G4Polyhedra* solidWCExtraBorderCell = new G4Polyhedra(
		    etbcname,
		    totalAngle - 2. * pi,  //+dPhi/2., // phi start
		    2. * pi - totalAngle -
		        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // total phi
		...
		logicWCExtraBorderCell =
		    new G4LogicalVolume(solidWCExtraBorderCell, G4Material::GetMaterial(water), etbcname, 0, 0, 0);
		...
		const G4String etbcbsname =
		    etbcname + G4String("BlackSheet");  // "WCExtraTower[Top|Bot]]BorderCellBlackSheet"
		G4Polyhedra* solidWCExtraBorderBlackSheet = new G4Polyhedra(
		    etbcbsname,
		    totalAngle - 2. * pi,  //+dPhi/2., // phi start
		    2. * pi - totalAngle -
		        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // phi end
		...
		G4LogicalVolume* logicWCExtraBorderBlackSheet =
		    new G4LogicalVolume(solidWCExtraBorderBlackSheet,
		                        ...
		                        0,
		                        0);
		G4VPhysicalVolume* physiWCExtraBorderBlackSheet = new G4PVPlacement(0,
		                                                                    G4ThreeVector(0., 0., 0.),
		...
		new G4LogicalBorderSurface(etbcwsname,
		                           ...
		                           OpWaterBSSurface);
		new G4LogicalSkinSurface(etbcname, logicWCExtraBorderBlackSheet, BSSkinSurface);
	}
	//------------------------------------------------------------
	// add caps
	// -----------------------------------------------------------
...
#ifdef DEBUG
	G4cout << "B.Q Cap: " << totalAngle << ", " << WCBarrelRingNPhi << ", " << outerAnnulusRadius << ", "
	       << innerAnnulusRadius << ", " << WCBlackSheetThickness << ", " << zflip << ", "
	       << WCBarrelPMTOffset << G4endl;
#endif

	if(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal) {
		solidWCCap = new G4Polyhedra(capname,
		                             0. * deg,          // phi start
		                             totalAngle,        // phi end
		                             WCBarrelRingNPhi,  // NPhi-gon
		                             4,                 // 2 z-planes
		...
		// to polyhedra. We have to unite both parts, because there are
		// PMTs that are on the border between both parts.
		G4Polyhedra* mainPart = new G4Polyhedra(capname + G4String("MainPart"),
		                                        0. * deg,          // phi start
		                                        totalAngle,        // phi end
		                                        WCBarrelRingNPhi,  // NPhi-gon
		                                        4,                 // 2 z-planes
		...
		for(int i = 0; i < 4; i++) {
			extraCapRmin[i] =
			    capRmin[i] != 0. ? capRmin[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) : 0.;
			extraCapRmax[i] =
			    capRmax[i] != 0. ? capRmax[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) : 0.;
		}
		G4Polyhedra* extraSlice = new G4Polyhedra(
		    capname + G4String("ExtraSlice"),
		    totalAngle - 2. * pi,  // phi start
		    2. * pi - totalAngle -
		        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // total phi
		    // fortunately there are no PMTs an the gap!
		...
		solidWCCap = new G4UnionSolid(capname, mainPart, extraSlice);
	...
	G4VPhysicalVolume* physiWCCap = new G4PVPlacement(
	    0,  // no rotation
	    G4ThreeVector(0.,
	                  0.,
	                  (-capAssemblyHeight / 2. + 1 * mm + WCBlackSheetThickness) * zflip),  // its position
	...
	G4double capBlackSheetZ[4]    = { -WCBlackSheetThickness * zflip,
		                              0.,
		                              0.,
		                              (WCBarrelPMTOffset - (WCIDRadius - innerAnnulusRadius)) * zflip };
	...
	if(capBlackSheetZ[0] * capBlackSheetZ[3] > 0.) {
		G4cerr << "IMPOSSIBLE GEOMETRY:  z profile array of " << capbsname
		       ...
		       << "\n                 Z[3] = " << capBlackSheetZ[3] << G4endl;
		/// Should throw an exception.
	}

	G4VSolid* solidWCCapBlackSheet = nullptr;
	if(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal) {
		solidWCCapBlackSheet = new G4Polyhedra(capbsname,
		                                       0. * deg,           // phi start
		...
		G4Polyhedra* mainPart = new G4Polyhedra(capbsname + G4String("MainPart"),
		                                        0. * deg,           // phi start
		                                        totalAngle,         // phi end
		                                        WCBarrelRingNPhi,   // NPhi-gon
		                                        4,                  // z-planes
		...
		for(int i = 0; i < 4; i++) {
			extraBSRmin[i] = capBlackSheetRmin[i] != 0. ?
			                     capBlackSheetRmin[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) :
			                     0.;
			extraBSRmax[i] = capBlackSheetRmax[i] != 0. ?
			                     capBlackSheetRmax[i] / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.) :
			                     0.;
		}
		G4Polyhedra* extraSlice = new G4Polyhedra(
		    capbsname + G4String("ExtraSlice"),
		    totalAngle - 2. * pi,  // phi start
		...
		solidWCCapBlackSheet = new G4UnionSolid(capbsname, mainPart, extraSlice);
	}
	G4LogicalVolume* logicWCCapBlackSheet =
	    new G4LogicalVolume(solidWCCapBlackSheet, G4Material::GetMaterial("Blacksheet"), capbsname, 0, 0, 0);
	...
	new G4LogicalBorderSurface(capbsname + G4String("WaterBSSurface"),
	                           physiWCCap,
	                           physiWCCapBlackSheet,
	                           OpWaterBSSurface);

	new G4LogicalSkinSurface(capbsname + G4String("BSSkinSurface"), logicWCCapBlackSheet, BSSkinSurface);
	...
	if(Vis_Choice == "RayTracer") {
		...
		G4VisAttributes* WCCapBlackSheetVisAtt = new G4VisAttributes(G4Colour(1.0, 1.0, 0.0));

		if(!debugMode)
			// logicWCCapBlackSheet->SetVisAttributes(G4VisAttributes::Invisible); //Use this line if you want
			// to make the blacksheet on the caps invisible to view through
			logicWCCapBlackSheet->SetVisAttributes(WCCapBlackSheetVisAtt);
		else
			logicWCCapBlackSheet->SetVisAttributes(WCCapBlackSheetVisAtt);
	}  // RayTracer
	...
	if(placeCapPMTs) {
		...
		G4cout << "total on cap: " << icopy << "\n";
		G4cout << "Coverage was calculated to be: "
		       << (icopy * WCPMTRadius * WCPMTRadius / (WCIDRadius * WCIDRadius)) << "\n";
	}  // end if placeCapPMTs
	...
	if(placeBorderPMTs) {
#ifdef DEBUG
		G4cout << "Debug B.Q : place border pmt, spacing horiz = " << horizontalSpacing
		       << ", vertical = " << verticalSpacing * zflip << G4endl;
#endif
	...
	if(isODConstructed) {
		...
		G4double sphereRadius = (WCPMTODExposeHeight * WCPMTODExposeHeight + WCPMTODRadius * WCPMTODRadius) /
		                        (2 * WCPMTODExposeHeight);
		...
		const G4String bbcodtname = bbcname + G4String("ODTyvek");  // "WCBarrel[Top|Bot]BorderCellODTyvek"
		G4Polyhedra* solidWCBarrelBorderCellODTyvek = new G4Polyhedra(bbcodtname,
		                                                              -dPhi / 2.,  // phi start
		                                                              dPhi,        // total phi
		                                                              1,           // NPhi-gon
		                                                              2,
		...
		G4double odCapZ[2]          = { (-WCBlackSheetThickness - 1. * mm) * zflip,
			                            (WCBarrelPMTOffset - (WCIDRadius - innerAnnulusRadius)) * zflip };
		if(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal) {
			solidWCODCapTyvek = new G4Polyhedra(capname + G4String("Tyvek"),
			                                    0. * deg,            // phi start
			                                    totalAngle,          // phi end
			                                    WCBarrelRingNPhi,    // NPhi-gon
			                                    2,                   // 2 z-planes
			...
			// to polyhedra. We have to unite both parts, because there are
			// PMTs that are on the border between both parts.
			G4Polyhedra* mainPart = new G4Polyhedra(capname + G4String("TyvekMainPart"),
			                                        0. * deg,            // phi start
			                                        totalAngle,          // phi end
			                                        WCBarrelRingNPhi,    // NPhi-gon
			                                        2,                   // 2 z-planes
			...
			for(int i = 0; i < 2; i++) {
				extraCapRmin[i] = annulusODTyvekRmin[i] != 0. ? annulusODTyvekRmin[i] / cos(dPhi / 2.) *
				                                                    cos((2. * pi - totalAngle) / 2.) :
				                                                0.;
				extraCapRmax[i] = annulusODTyvekRmax[i] != 0. ? annulusODTyvekRmax[i] / cos(dPhi / 2.) *
				                                                    cos((2. * pi - totalAngle) / 2.) :
				                                                0.;
			}
			G4Polyhedra* extraSlice = new G4Polyhedra(
			    capname + G4String("TyvekExtraSlice"),
			    totalAngle - 2. * pi,  // phi start
			    2. * pi - totalAngle -
			        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // total phi
			    // fortunately there are no PMTs an the gap!
			...
			solidWCODCapTyvek = new G4UnionSolid(capname + G4String("Tyvek"), mainPart, extraSlice);
		}

		G4LogicalVolume* logicWCODCapTyvek = new G4LogicalVolume(solidWCODCapTyvek,
		                                                         G4Material::GetMaterial("Tyvek"),
		...
		new G4LogicalSkinSurface(capname + G4String("WaterTySurface"), logicWCODCapTyvek, OpWaterTySurface);
		...
		if(!(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)) {
			...
			const G4String etbcodtname =
			    etbcname + G4String("ODTyvek");  // "WCExtraTower[Top|Bot]]BorderCellODTyvek"
			G4Polyhedra* solidWCExtraBorderCellODTyvek = new G4Polyhedra(
			    etbcodtname,
			    totalAngle - 2. * pi,  //+dPhi/2., // phi start
			    2. * pi - totalAngle -
			        G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * m),  // phi end
			...
			for(G4long i = 0; i < (WCPMTODperCellHorizontalExtra); i++) {
				for(G4long j = 0; j < WCPMTODperCellVertical; j++) {
					G4ThreeVector Container =
					    G4ThreeVector((WCODRadius) / cos(dPhi / 2.) * cos((2. * pi - totalAngle) / 2.),
					                  -towerWidthOD / 2. + (i + 0.5) * horizontalODSpacingExtra,
					                  (-(barrelCellHeight * (WCODRadius / WCIDRadius)) / 2. +
					                   (j + 0.5) * verticalODSpacing) *
					                      zflip);
					Container.rotateZ(-(2 * pi - totalAngle) / 2.);  // align with the symmetry
...
// This function finds the factors of a number and aims to minimise the difference between multiplied factors.
// This will result in a more square-like distribution of PMTs per cell. ToDo:  Change the implementation to
// remove the recursion.  It looks like it can run away.
	...
	for(int i = 2; i <= (int)(NPMT / 2); i++) {
		if(NPMT % i == 0) {
			...
			if(d < difference) {
				...
				factors[0] = a;
				factors[1] = b;
			}  // make the difference between multiplied factors as small as possible, for a square rather
			   // than rectangle shape.
		}
	}
	if(!found) {
		FindNiceFactors(NPMT - 1, factors);
	}
}
