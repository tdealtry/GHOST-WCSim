...
WCSimWCDAQMessenger::WCSimWCDAQMessenger(WCSimEventAction* eventaction) :
	...
	DigitizerChoice->SetCandidates("SKI ");
	...
	TriggerChoice->SetCandidates("NDigits "
	                             "NDigits2 "
	                             "NoTrigger ");
	...
	MultiDigitsPerTriggerSet = false;  // this variable is bool & defaults are class specfic; use this to know
	                                   // if the default is overidden
	...
	SaveFailuresTriggerMode->SetGuidance(
	...
	SaveFailuresPreTriggerWindow =
	    new G4UIcmdWithAnInteger("/DAQ/TriggerSaveFailures/PreTriggerWindow", this);
	...
	SetNewValue(SaveFailuresPreTriggerWindow,
	            G4UIcommand::ConvertToString(defaultSaveFailuresPreTriggerWindow));

	int defaultSaveFailuresPostTriggerWindow = 950;
	SaveFailuresPostTriggerWindow =
	    new G4UIcmdWithAnInteger("/DAQ/TriggerSaveFailures/PostTriggerWindow", this);
	...
	SetNewValue(SaveFailuresPostTriggerWindow,
	            G4UIcommand::ConvertToString(defaultSaveFailuresPostTriggerWindow));
	...
	NDigitsTriggerAdjustForNoise->SetGuidance(
	    "Adjust the NDigits trigger threshold automatically dependent on the average noise rate");
	...
	SetNewValue(NDigitsTriggerAdjustForNoise,
	            G4UIcommand::ConvertToString(defaultNDigitsTriggerAdjustForNoise));
...
void WCSimWCDAQMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
	...
	if(command == DigitizerChoice) {
		...
		if(!StoreMultiDigitsPerTrigger)
			G4cout << "Will restrict number of digits per PMT per trigger to <= 1" << initialiseString.c_str()
			       << G4endl;
		else
			G4cout << "Will allow number of digits per PMT per trigger to go > 1" << initialiseString.c_str()
			       << G4endl;
		if(initialised)
	...
	// Generic digitizer options
		...
		G4cout << "Digitizer integration window set to " << newValue << " ns" << initialiseString.c_str()
		       << G4endl;
		...
		G4cout << "Digitizer timing resolution set to " << newValue << " ns" << initialiseString.c_str()
		       << G4endl;
		StoreDigitizerTimingPrecision = DigitizerTimingPrecision->GetNewDoubleValue(newValue);
	}
	else if(command == DigitizerPEPrecision) {
		G4cout << "Digitizer charge resolution set to " << newValue << " p.e." << initialiseString.c_str()
		       << G4endl;
	...
	// Save failures "trigger"
		...
		if(StoreSaveFailuresMode == 0)
			...
			G4cerr << "Unknown value of /DAQ/TriggerSaveFailures/Mode " << StoreSaveFailuresMode
			       << " Exiting..." << G4endl;
			exit(-1);
		}
		...
		G4cout << "Trigger time for events which fail all triggers will be set to " << newValue << " ns"
		...
		G4cout << "SaveFailures pretrigger window set to " << newValue << " ns" << initialiseString.c_str()
		       << G4endl;
		...
		G4cout << "SaveFailures posttrigger window set to " << newValue << " ns" << initialiseString.c_str()
		       << G4endl;
	...
	// NDigits trigger
		...
		if(StoreNDigitsAdjustForNoise)
			G4cout << "Will adjust NDigits trigger threshold using average dark noise rate"
			       << initialiseString.c_str() << G4endl;
	}
		...
		G4cout << "NDigits pretrigger window set to " << newValue << " ns" << initialiseString.c_str()
		       << G4endl;
		...
		G4cout << "NDigits posttrigger window set to " << newValue << " ns" << initialiseString.c_str()
		       << G4endl;
...
void WCSimWCDAQMessenger::SetTriggerOptions() {
	...
	if(MultiDigitsPerTriggerSet) {
		...
		if(!StoreMultiDigitsPerTrigger)
			...
			G4cout << "\tWill allow number of digits per PMT per trigger to go > 1"
			       << initialiseString.c_str() << G4endl;
	}
	...
	G4cout << "\tTrigger time for events which fail all triggers will be set to " << StoreSaveFailuresTime
	       << " ns" << G4endl;
...
void WCSimWCDAQMessenger::SetDigitizerOptions() {
	...
	if(StoreDigitizerIntegrationWindow >= 0) {
		...
		G4cout << "\tDigitizer integration window set to " << StoreDigitizerIntegrationWindow << " ns"
		       << G4endl;
	}
