...
void WCSimWCHit::Draw() {
	...
	if(pVVisManager) {
		...
		// instead of accessing those exactly here, just grab the substring: It should be "a" glassFaceWCPMT.
		// Later optional check for OD?
		   ...
		   volumeName.find("glassFaceWCPMT_refl") != std::string::npos)  // isn't this deprecated??
		{
			...
			if(totalPe > maxPe * .01) {
				...
				double hue = (1. - (double(totalPe - .05 * maxPe) / double(.95 * maxPe))) *
				             230;  // go from hue = 230 to 0
				double saturation = 1.;
				double value      = 1.;
...
void WCSimWCHit::Print() {
	...
	G4cout << " Tube:" << std::setw(4) << tubeID << " Tube type:" << tubeType << " Track:" << std::setw(6)
	       << trackID << " Pe:" << totalPe << " Pos:" << pos / cm << G4endl << "\tTime: ";
...
bool WCSimWCHit::SortFunctor_Hit::operator()(const WCSimWCHit* const& a, const WCSimWCHit* const& b) const {
