...
GdNeutronHPCapture::GdNeutronHPCapture() :
	...
	if(!getenv("G4NEUTRONHPDATA"))
	   throw G4HadronicException(__FILE__, __LINE__, "Please setenv G4NEUTRONHPDATA to point to the neutron
cross-section files."); dirName = getenv("G4NEUTRONHPDATA"); G4String tString = "/Capture"; dirName = dirName
+ tString; numEle = G4Element::GetNumberOfElements(); G4cout <<
"+++++++++++++++++++++++++++++++++++++++++++++++++"<<G4endl;
...
GdNeutronHPCapture::~GdNeutronHPCapture() {
	// delete [] theCapture;
	// G4cout << "Leaving GdNeutronHPCapture::~GdNeutronHPCapture"<<G4endl;
	for(std::vector<G4NeutronHPChannel*>::iterator ite = (*theCapture).begin(); ite != (*theCapture).end();
	    ite++) {
		delete *ite;
	}
	(*theCapture).clear();
...
G4HadFinalState* GdNeutronHPCapture::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus) {
	...
	if(getenv("NeutronHPCapture"))
		G4cout << " ####### GdNeutronHPCapture called" << G4endl;
	...
	for(G4int j = 0; j != iele; j++) {
		...
		if(target_isotope->GetN() == G4NeutronHPManager::GetInstance()->GetReactionWhiteBoard()->GetTargA())
			break;
	}
...
void GdNeutronHPCapture::BuildPhysicsTable(const G4ParticleDefinition&) {
	...
	if(!G4Threading::IsWorkerThread()) {
		...
		if(theCapture == NULL)
			theCapture = new std::vector<G4NeutronHPChannel*>;
