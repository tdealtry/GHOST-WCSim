...
void WCSimDetectorConstruction::SuperK_20inchPMT_20perCent() {
	...
	WCBarrelNumPMTHorizontal =
	    std::lround(WCIDDiameter * sqrt(pi * WCPMTPercentCoverage) / (10.0 * WCPMTRadius));
	WCBarrelNRings = std::lround(
	    (WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (pi * WCIDDiameter))) /
	    WCPMTperCellVertical);
	WCCapPMTSpacing =
	    pi * WCIDDiameter / WCBarrelNumPMTHorizontal;  // distance between centers of top and bottom pmts
	WCCapEdgeLimit        = WCIDDiameter / 2.0 - WCPMTRadius;
	WCBlackSheetThickness = 2.0 * cm;
	WCAddGd               = false;
	hybrid                = false;
...
void HKGeometryNoLongerUsed(G4String& DetectorName) {
	G4cerr << "**********************************" << G4endl << DetectorName << " is an obsolete HK geometry"
	       << G4endl << "You should use one of:" << G4endl << " HyperK_HybridmPMT_WithOD" << G4endl
	       << " HyperK_HybridmPMT" << G4endl << "If you really want to use " << DetectorName
	       ...
	       << "but be careful if comparing to official HK geometries that the number of PMTs are consistent"
...
void WCSimDetectorConstruction::SetHyperKGeometry() {
	...
	WCPMTperCellHorizontal = 4;
	WCPMTperCellVertical   = 3;
	if(WCPMTPercentCoverage < -0.1)
		WCPMTPercentCoverage = 40.0;
	else if(WCPMTPercentCoverage < 0)
		WCPMTPercentCoverage = 0.0;  // protection against negative coverage
	WCBarrelNumPMTHorizontal =
	    std::lround(WCIDDiameter * sqrt(pi * WCPMTPercentCoverage) / (10.0 * WCPMTRadius));
	WCBarrelNRings = std::lround(
	    (WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (pi * WCIDDiameter))) /
...
void WCSimDetectorConstruction::SetHyperKWithODGeometry() {
	...
	WCODCapPMTSpacing = (pi * WCIDDiameter /
...
void WCSimDetectorConstruction::SetHyperK_mPMTGeometry() {
	...
	mPMT_vessel_radius_curv =
	    342. * CLHEP::mm;  // needs to include the vessel thickness, as we construct from outside inwards.
	...
	id_reflector_height =
	    0;  // 9.29*CLHEP::mm;        // for a radius of 7.25mm, for hex: 5.4mm (radius of 6mm)
	...
	nID_PMTs    = 19;
	config_file = wcsimdir_path +
	              "/mPMT-configfiles/mPMTconfig_19_nuPrism_3ring.txt";  // for smaller reflector, use:
	                                                                    // mPMTconfig_19_nuPrism.txt (hex)
	...
	if(WCPMTPercentCoverage < -0.1)
		...
		WCPMTPercentCoverage = 0.0;  // protection against negative coverage
	WCBarrelNumPMTHorizontal =
	    std::lround(WCIDDiameter * sqrt(pi * WCPMTPercentCoverage) / (10.0 * mPMT_vessel_radius));
	G4cout << "## Number of Horizontal PMT along diameter = " << WCBarrelNumPMTHorizontal
	       << ", diameter = " << WCIDDiameter << ", coverage = " << WCPMTPercentCoverage
	       << ", PMT radius = " << WCPMTRadius
	       << ", test = " << WCIDDiameter * sqrt(pi * WCPMTPercentCoverage) / (10.0 * WCPMTRadius) << G4endl;
	WCBarrelNRings = std::lround(
	    (WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (pi * WCIDDiameter))) /
	    WCPMTperCellVertical);
	WCCapPMTSpacing =
...
void WCSimDetectorConstruction::SetHyperK_HybridmPMTGeometry() {
	...
	WCIDHeight   = 65.751 * m;  // 54.8*m; // = 60m - 2*(60cm ID wall + 2m OD)
	WCBarrelPMTOffset =
	    std::max(WCPMTRadius, mPMT_vessel_tot_height) + 1. * CLHEP::mm;  // offset from vertical
	WCIDVerticalPosition = 0.;
	...
	if(WCPMTPercentCoverage < -0.1)
		...
		WCPMTPercentCoverage = 0.0;  // protection against negative coverage
	if(WCPMTPercentCoverage2 < -0.1)
		WCPMTPercentCoverage2 = 10.1075288187831;
	else if(WCPMTPercentCoverage2 < 0)
		WCPMTPercentCoverage2 = 0.0;  // protection against negative coverage
	WCPMTperCellHorizontal =
	    std::lround((WCPMTPercentCoverage + WCPMTPercentCoverage2) /
	                (WCPMTPercentCoverage2 == 0 ? (WCPMTPercentCoverage + WCPMTPercentCoverage2) :
	                                              WCPMTPercentCoverage2));
	WCPMTperCellVertical = WCPMTperCellHorizontal;
	// WCPMTperCellHorizontal= ((int)
	// WCPMTPercentCoverage2/(WCPMTPercentCoverage+WCPMTPercentCoverage2));//Should be in agreement with
	// coverage of PMT 1 & 2: here, 4/5 PMTs are B&L + 1/5 mPMT WCPMTperCellVertical  = ((int)
	// WCPMTPercentCoverage2/(WCPMTPercentCoverage+WCPMTPercentCoverage2));//Should be in agreement with
	// coverage of PMT 1 & 2: here, 4/5 PMTs are B&L + 1/5 mPMT

	WCBarrelNumPMTHorizontal =
	    std::lround(WCIDDiameter * sqrt(pi * (WCPMTPercentCoverage + WCPMTPercentCoverage2)) /
	                (10.0 * std::max(WCPMTRadius, mPMT_vessel_radius)));
	WCBarrelNRings = std::lround(
	    (WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (pi * WCIDDiameter))) /
	    WCPMTperCellVertical);
	WCCapPMTSpacing =
	    pi * WCIDDiameter / WCBarrelNumPMTHorizontal;  // distance between centers of top and bottom pmts
	// WCCapEdgeLimit        = WCIDDiameter/2.0 -
	// 5*std::max(WCPMTExposeHeight,mPMT_vessel_tot_height);//3.0*cm;//WCIDDiameter/2.0 -
	// std::max(WCPMTRadius,mPMT_vessel_tot_height);//WCPMTRadius;
...
void WCSimDetectorConstruction::SetHyperK_HybridmPMT10PCGeometry() {
	...
	// WCBarrelNumPMTHorizontal =
	// std::lround(WCIDDiameter*sqrt(pi*(WCPMTPercentCoverage+WCPMTPercentCoverage2))/(10.0*std::max(WCPMTRadius,mPMT_vessel_tot_height)));
	WCBarrelNumPMTHorizontal =
	    std::lround(WCIDDiameter * sqrt(pi * (WCPMTPercentCoverage + WCPMTPercentCoverage2)) /
	                (10.0 * std::max(WCPMTRadius, mPMT_vessel_radius)));
	WCBarrelNRings = std::lround(
	    (WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (pi * WCIDDiameter))) /
	    WCPMTperCellVertical);
	// G4cout << "## Number of Horizontal PMT along diameter = " << WCBarrelNumPMTHorizontal << ", diameter =
	// "<< WCIDDiameter << ", coverage = "<< WCPMTPercentCoverage << ", PMT radius = " << WCPMTRadius << ",
	// test = " << WCIDDiameter*sqrt(pi*WCPMTPercentCoverage)/(10.0*WCPMTRadius) << G4endl;
	WCCapPMTSpacing =
	    pi * WCIDDiameter / WCBarrelNumPMTHorizontal;  // distance between centers of top and bottom pmts
	// WCCapEdgeLimit        = WCIDDiameter/2.0 -
...
void WCSimDetectorConstruction::SetNuPrismGeometry(G4String PMTType,
	...
	WCBarrelNumPMTHorizontal =
	    std::lround(WCIDDiameter * sqrt(pi * WCPMTPercentCoverage / 100.0) / WCPMTRadius);
...
void WCSimDetectorConstruction::SetNuPrismBeamTest_mPMTGeometry() {
	...
	G4cerr << "**********************************" << G4endl << WCDetectorName
	       << " is not supported in this version of WCSim" << G4endl
	       << "Please use code from the WCTE/WCSim repository to simulate WCTE" << G4endl << "Exiting..."
	       << G4endl;
	exit(-1);
	WCIDCollectionName  = WCDetectorName + "-glassFaceWCPMT";
	mPMT_ID_PMT         = "PMT3inchR12199_02";  // can be changed in macro through mPMT settings.
...
void WCSimDetectorConstruction::SetIWCDGeometry() {
	...
	WCBarrelNRings           = 10;
	WCCapPMTSpacing =
	    pi * WCIDDiameter / WCBarrelNumPMTHorizontal * 1.2 / 1.1;  // ad-hoc number to get 52 mPMTs on cap
	WCCapEdgeLimit        = 3.3 * m;
	WCBlackSheetThickness = 2.0 * cm;  // deprecate soon.
	WCAddGd               = false;
	hybrid                = false;
...
void WCSimDetectorConstruction::Cylinder_60x74_3inchmPMT_14perCent() {
	...
	WCPMTRadius =
	    0.254 *
	    m;  // outer_module_radius;//PMT-T>GetRadius(); //Actually, this name is very misleading, because it
	        // should be moduleRadius (eg. PMT + acrylic), this is used in two places in WCConstructCylinder

	// mPMT params go first because detector depends on it:
	mPMT_vessel_cyl_height =
	    0. * CLHEP::mm;  // the 50mm should exist only for OD and extends behind the iD wall
	...
	mPMT_outer_material_d =
	    0.03 * 2 * mPMT_vessel_radius * CLHEP::mm;  // using NEMO article safety margin of 0.03 for t/D
	// Radius of cone at z=reflectorHeight
	id_reflector_height   = 7.5 * CLHEP::mm;  // 7.5mm from KM3Net JINST
	id_reflector_z_offset = 1.1 * CLHEP::mm;  // from KM3Net CAD drawings
	id_reflector_angle =
	    CLHEP::pi / 4 * CLHEP::rad;  // Based on KM3Net JINST: 45 deg wrt normal, so 7.5mm xtra
	G4double mPMT_vessel_tot_height = mPMT_vessel_radius + mPMT_vessel_cyl_height;
	...
	WCPMTperCellVertical     = 3;
	WCPMTPercentCoverage     = 13.51;
	WCBarrelNumPMTHorizontal = std::lround(
	    WCIDDiameter * sqrt(pi * WCPMTPercentCoverage) /
	    (10.0 * 0.254 *
	     m));  // Used same radius as 20in to have same granularity in mPMTs (28% difference otherwise
	WCBarrelNRings = std::lround(
	    (WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (pi * WCIDDiameter))) /
	    WCPMTperCellVertical);
	WCCapPMTSpacing =
...
void WCSimDetectorConstruction::Cylinder_60x74_3inchmPMT_40perCent() {
	...
	WCPMTPercentCoverage   = 40.0;
	WCBarrelNumPMTHorizontal =
	    std::lround(WCIDDiameter * sqrt(pi * WCPMTPercentCoverage) / (10.0 * 0.254 * m));  // WCPMTRadius));
	WCBarrelNRings = std::lround(
	    (WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (pi * WCIDDiameter))) /
	    WCPMTperCellVertical);
	WCCapPMTSpacing =
...
void WCSimDetectorConstruction::DumpDetectorConfigInfo() {
	...
	G4cout << "GEOMCHECK WCDetectorName \t" << WCDetectorName << G4endl << "GEOMCHECK WCIDCollectionName \t"
	       << WCIDCollectionName << G4endl << "GEOMCHECK WCPMTName \t" << WCPMTName << G4endl
	       << "GEOMCHECK WCPMTExposeHeight \t" << WCPMTExposeHeight << G4endl << "GEOMCHECK WCPMTRadius \t"
	       << WCPMTRadius << G4endl << "GEOMCHECK WCIDDiameter \t" << WCIDDiameter << G4endl
	       << "GEOMCHECK WCIDHeight \t" << WCIDHeight << G4endl << "GEOMCHECK WCBarrelPMTOffset \t"
	       << WCBarrelPMTOffset << G4endl << "GEOMCHECK WCPMTperCellHorizontal \t" << WCPMTperCellHorizontal
	       << G4endl << "GEOMCHECK WCPMTperCellVertical \t" << WCPMTperCellVertical << G4endl
	       ...
	       << "GEOMCHECK WCBarrelNRings \t" << WCBarrelNRings << G4endl << "GEOMCHECK WCCapPMTSpacing \t"
	       << WCCapPMTSpacing << G4endl << "GEOMCHECK WCCapEdgeLimit \t" << WCCapEdgeLimit << G4endl
	       << "GEOMCHECK WCBlackSheetThickness \t" << WCBlackSheetThickness << G4endl
	       << "GEOMCHECK WCAddGd \t" << WCAddGd << G4endl << "GEOMCHECK CaveTyvekSheetThickness \t"
	       << CaveTyvekSheetThickness << G4endl << "GEOMCHECK isODConstructed \t" << isODConstructed << G4endl
	       ...
	       << "GEOMCHECK WCODDiameter \t" << WCODDiameter << G4endl << "GEOMCHECK WCODCollectionName \t"
	       << WCODCollectionName << G4endl << "GEOMCHECK WCPMTODName \t" << WCPMTODName << G4endl
	       ...
	       << "GEOMCHECK WCODPMTShift \t" << WCODPMTShift << G4endl << "GEOMCHECK WCODCapPMTSpacing \t"
	       << WCODCapPMTSpacing << G4endl << "GEOMCHECK WCODCapEdgeLimit \t" << WCODCapEdgeLimit << G4endl;
#endif
}
