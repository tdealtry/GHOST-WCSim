...
WCSimWCTriggerBase::WCSimWCTriggerBase(G4String name,
    ...
    myDetector(inDetector),
    detectorElement(inDetectorElement),
    triggerClassName("") {
	G4String colName;
	if(detectorElement == "tank")
		colName = "WCDigitizedCollection";
	else if(detectorElement == "tankPMT2")
		colName = "WCDigitizedCollection2";
	else if(detectorElement == "OD")
		colName = "WCDigitizedCollection_OD";
	collectionName.push_back(colName);

#ifdef HYPER_VERBOSITY
	if(detectorElement == "OD")
		G4cout << "WCSimWCTriggerBase::WCSimWCTriggerBase ☆ recording collection name " << colName << " for "
...
void WCSimWCTriggerBase::GetVariables() {
	...
	G4cout << (multiDigitsPerTrigger ? "Using mutiple digits per PMT per trigger" :
	                                   "Using a maximum of 1 digit per PMT per trigger")
	       << G4endl << "Using NDigits threshold " << ndigitsThreshold
	...
	if(saveFailuresMode > 0)
		G4cout << "Using SaveFailures trigger time" << saveFailuresTime << " ns" << G4endl
		       << "Using SaveFailures event pretrigger window " << saveFailuresPreTriggerWindow << " ns"
		       << G4endl << "Using SaveFailures event posttrigger window " << saveFailuresPostTriggerWindow
		       << " ns" << G4endl;
}
...
void WCSimWCTriggerBase::AdjustNDigitsThresholdForNoise() {
	...
	if(detectorElement == "tank")
		...
		npmts = this->myDetector->GetTotalNumPmts2();
	else if(detectorElement == "OD")
		npmts = this->myDetector->GetTotalNumODPmts();
	...
	G4cout << "Average number of PMTs in detector active in a " << ndigitsWindow
	       << "ns window with a dark noise rate of " << PMTDarkRate << "kHz is " << average_occupancy << " ("
	       << npmts << " total PMTs)" << G4endl << "Updating the NDigits threshold, from " << ndigitsThreshold
...
void WCSimWCTriggerBase::Digitize() {
	...
	if(detectorElement == "tank")
		...
		collectiondetectorname = "/WCSim/glassFaceWCPMT2";
	else if(detectorElement == "OD")
		collectiondetectorname = "/WCSim/glassFaceWCPMT_OD";
	...
	if(detectorElement == "tank")
		...
		untriggeredcollectionname = "WCDigitizedStoreCollection2";
	else if(detectorElement == "OD")
...
#ifdef HYPER_VERBOSITY
	if(detectorElement == "OD") {
		G4cout << "WCSimWCTriggerBase::Digitize ☆ making triggered digits collection " << collectionName[0]
		       << " for " << detectorElement << " and calling DoTheWork on " << untriggeredcollectionname
		       << " to fill it." << G4endl;
	}
#endif
...
#ifdef HYPER_VERBOSITY
	if(detectorElement == "OD") {
		G4cout << "WCSimWCTriggerBase::Digitize ☆ storing the triggered digits collection "
		       << collectionName[0] << " which has " << DigitsCollection->entries() << " entries" << G4endl;
	}
#endif
...
void WCSimWCTriggerBase::AlgNDigits(WCSimWCDigitsCollection* WCDCPMT, bool remove_hits, bool test) {
	...
	// loop over PMTs, and Digits in each PMT.  If ndigits > Threshhold in a time window, then we have a
	// trigger
	...
	G4cout << "WCSimWCTriggerBase::AlgNDigits. Found first/last hits. Looping from " << window_start_time
	       << " to " << window_end_time << " in steps of " << window_step_size << G4endl;
#endif

	std::vector<G4double> digit_times;

#ifdef WCSIMWCTRIGGER_VERBOSE
	G4cout << "WCSimWCTriggerBase::AlgNDigits. Number of entries in input digit collection: "
	       << WCDCPMT->entries() << G4endl;
	int temp_total_pe = 0;
	for(G4int i = 0; i < WCDCPMT->entries(); i++) {
	...
	while(window_start_time <= window_end_time) {
		int n_digits = 0;
		double triggertime;  // save each digit time, because the trigger time is the time of the first hit
		                     // above threshold
		...
		for(G4int i = 0; i < WCDCPMT->entries(); i++) {
			...
			// Loop over each Digit in this PMT
			//  When stacking triggers, some digits are removed from the map, so the keys are no longer
			//  monotonic. Use an iterator to loop over the digits present
			for(std::map<int, double>::const_iterator digit_time_it = (*WCDCPMT)[i]->GetTimeMapBegin();
				...
				G4double digit_time = (*WCDCPMT)[i]->GetTime(ip);
				if(digit_time > window_start_time && digit_time < next_hit_time)
					next_hit_time = digit_time;
...
#ifdef WCSIMWCTRIGGER_VERBOSE
		if(n_digits)
			G4cout << n_digits << " digits found in 200nsec trigger window [" << window_start_time << ", "
			       << window_start_time + ndigitsWindow << "]. Threshold is: " << this_ndigitsThreshold
			       << G4endl;
#endif
...
void WCSimWCTriggerBase::FillDigitsCollection(WCSimWCDigitsCollection* WCDCPMT,
	...
	//  optionally removes digits from the input digits collection (when running different Alg* methods
	//  concurently) so they are not used in subsequent trigger decisions or saved twice
	...
	for(unsigned int itrigger = 0; itrigger < TriggerTimes.size(); itrigger++) {
		...
		// these are the boundary of the trigger gate: we want to add all digits within these bounds to the
		// output collection
		...
		if(itrigger) {
			double upperbound_previous =
			    TriggerTimes[itrigger - 1] + GetPostTriggerWindow(TriggerTypes[itrigger - 1]);
		...
		G4cout << "Saving trigger " << itrigger << " of type " << WCSimEnumerations::EnumAsString(triggertype)
		       ...
		       << " with trigger time " << triggertime << " and additional trigger info";
		...
		for(G4int i = 0; i < WCDCPMT->entries(); i++) {
			...
			//  The format of the loop iteration this time is a little unconventional, because erasing
			//  elements (via RemoveDigitizedGate) during iteration would normally invalidate the iterators.
			//  This format is safe.
			for(std::map<int, double>::const_iterator digit_time_it = (*WCDCPMT)[i]->GetTimeMapBegin(),
				...
				if(digit_time >= lowerbound && digit_time <= upperbound) {
...
#ifdef WCSIMWCTRIGGER_VERBOSE
					G4cout << "Saving digit on PMT " << tube << " time " << digihittime << " pe " << peSmeared
					       << " digicomp";
					...
					if(DigiHitMap[tube] == 0) {
						...
						(*DigitsCollection)[DigiHitMap[tube] - 1]->AddDigiCompositionInfo(
						    itrigger,
					...
					// we've found a digit on this PMT. If we're restricting to just 1 digit per trigger
					// window (e.g. SKI)
	...
	G4cout << "WCSimWCTriggerBase::FillDigitsCollection. Number of entries in output digit collection: "
...
WCSimWCDigiTrigger::WCSimWCDigiTrigger(const WCSimWCDigiTrigger& right) : G4VDigi() {
...
void WCSimWCDigiTrigger::Print() {
	G4cout << "TubeID: " << tubeID << ", Number of Gates: " << NumberOfGates() << G4endl;
	...
	for(; it_pe != pe.end(), it_time != time.end(); ++it_pe, ++it_time) {
		...
		G4cout << "Gate = " << it_pe->first << " PE: " << it_pe->second << " Time: " << it_time->second
		       << G4endl;
	}
}
