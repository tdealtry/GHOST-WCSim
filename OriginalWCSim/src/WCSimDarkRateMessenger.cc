...
void WCSimDarkRateMessenger::Initialize() {
	...
	if(SetDetectorElement == 0) {
		SetDetectorElement = new G4UIcmdWithAString("/DarkRate/SetDetectorElement", this);
	}
	...
	for(std::map<std::string, WCSimWCAddDarkNoise*>::iterator it = darknoiseptrs.begin();
	    it != darknoiseptrs.end();
	    ++it) {
		theguidance   += it->first;
		theguidance   += "\n";
		thecandidates += it->first;
		thecandidates += " ";
	}
...
void WCSimDarkRateMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
	if(command == SetFrequency) {
		...
		if(darknoiseptrs.count(newValue) > 0) {
			...
			G4cout << "Setting detectorElement value " << newValue << initaliseString.c_str() << G4endl;
		}
	}
}

void WCSimDarkRateMessenger::AddDarkRateInstance(WCSimWCAddDarkNoise* darkratepoint,
                                                 G4String detectorElementin) {
...
void WCSimDarkRateMessenger::RemoveDarkRateInstance(G4String detectorElementin) {
	if(darknoiseptrs.count(detectorElementin) > 0) {
		...
		std::map<std::string, WCSimWCAddDarkNoise*>::iterator thepointer =
		    darknoiseptrs.find(detectorElementin);
		darknoiseptrs.erase(thepointer);
		if(darknoiseptrs.size() == 0) {
			delete this;
		}
	}
	else {
		G4cout << "Attempt to remove nonexistant element " << detectorElementin << " from DarkRateMessenger!"
		       << G4endl;
	}
}
