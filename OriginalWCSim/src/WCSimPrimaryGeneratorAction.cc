...
inline double atof(const string& str) {
		...
		if(useNuanceTextFormat) {
			...
			if(token.size() == 0 || token[0] == "stop") {
				...
				while(token[0] == "nuance" && iVertex < MAX_N_VERTICES) {
					...
					token = readInLine(inputFile, lineSize, inBuf);
					vtxs[iVertex] =
					    G4ThreeVector(atof(token[1]) * cm, atof(token[2]) * cm, atof(token[3]) * cm);
					...
					// B.Q: there can be some cases (2p2h i.e. neut mode = 2) where there are 2 targets. The
					// while loop is added for this purpose.
...
#pragma GCC diagnostic pop
			...
			G4double random_z =
			    ((myDetector->GetWaterTubePosition()) - .5 * (myDetector->GetWaterTubeLength()) + 1. * m +
		...
		if(!fInputRootrackerFile->IsOpen()) {
			G4cout << "Set a Rootracker vector file using the command /mygen/vecfile name" << G4endl;
			return;
		}
		...
		// The current neut vector files do not correspond directly to the detector dimensions, so only keep
		// those events within the detector
		...
		particleGun->SetParticlePosition(vtx);
		particleGun->SetParticleMomentumDirection(dir);
		// Will want to include some beam time structure at some point, but not needed at the moment since we
		// only simulate 1 interaction per events particleGun->SetParticleTime(time);
		...
		for(int i = 0; i < fTmpRootrackerVtx->StdHepN; i++) {
			...
			particleGun->SetParticlePosition(vtx);
			particleGun->SetParticleMomentumDirection(dir);
			// Will want to include some beam time structure at some point, but not needed at the moment since
			// we only simulate 1 interaction per events particleGun->SetParticleTime(time);
			particleGun->GeneratePrimaryVertex(anEvent);  // Place vertex in stack
		}
	}
...
#pragma GCC diagnostic pop
		...
		G4double mass     = anEvent->GetPrimaryVertex()
		                    ->GetPrimary()
		                    ->GetMass();  // will be 0 for photon anyway, but for other gps particles not
		G4int pdg = anEvent->GetPrimaryVertex()->GetPrimary()->GetPDGcode();
		...
		// mode            = LASER; //actually could also be particle gun here. Gps and laser will be separate
		// soon!!
	...
	else if(useInjectorEvt)  // K.M.Tsui: addition of injector events
	{
		...
		G4cout << " GPS primary vertex (" << vtx.x() << ", " << vtx.y() << ", " << vtx.z() << "), dir ("
		       << dir.x() << ", " << dir.y() << ", " << dir.z() << ") m " << m << " E " << E << " pdg " << pdg
		       << G4endl;
	...
	else if(useRadonEvt) {  // G. Pronost: Add Radon (adaptation of Radioactive event)

		// Currently only one generator is possible
		// In order to have several, we need to find a solution for the fitting graphes (which are static
		// currently) Idea: array of fitting graphes? (each new generators having a specific ID)
		if(!myRn222Generator) {
		...
		*/
		...
		for(G4int u = 0; u < number_of_sources; u++) {
			...
			// G4cout << " vertex " << u << " of " << number_of_sources << " (" << vtx.x() << ", " << vtx.y()
			// << ", " << vtx.z() << ") with pdg: " << pdg << G4endl;
		...
		// DEBUG PRINTS
		G4cout << G4endl;
		G4cout << "Generated at position : " << vtx.getX() / m << "m " << vtx.getY() / m << "m "
		       << vtx.getZ() / m << "m " << G4endl;
			...
			for(int u = 0; u < n_vertices; u++) {
				...
				if(IsotopeLocation.compareTo("water") == 0) {
					...
					MyGPS->GetCurrentSource()->GetPosDist()->SetHalfZ(
					    myDetector->GetGeo_Dm(2) * CLHEP::cm / 2. - 2. * PMT->GetRadius());
					...
					G4ThreeVector position =
					    random_pmt_center + random_R * (orientation * random_cos_theta +
					                                    axis_1 * random_sin_theta * cos(random_phi) +
					                                    axis_2 * random_sin_theta * sin(random_phi));

					// G4cout << " random id " << random_pmt_id << " of " << npmts << " costheta " <<
					// random_cos_theta << " sintheta " << random_sin_theta << " phi " << random_phi << "
					// WCIDCollectionName " << WCIDCollectionName << " PMT_radius " << PMT_radius << " expose
					// " << expose << " sphereRadius " << sphereRadius << " Rmin " << Rmin << " Rmax " << Rmax
					// << " random_R " << random_R << " orientation (" << orientation.x() << ", " <<
					// orientation.y() << ", " << orientation.z() << ") center (" << random_pmt_center.x() <<
					// ", " << random_pmt_center.y() << ", " << random_pmt_center.z() << ") position (" <<
...
void WCSimPrimaryGeneratorAction::OpenRootrackerFile(G4String fileName) {
	...
	if(!fRooTrackerTree) {
		G4cout
		    << "File: " << fileName
		    << " does not contain a Rootracker nRooTracker tree - please check you intend to process Rootracker events"
		    << G4endl;
		exit(1);
	}
