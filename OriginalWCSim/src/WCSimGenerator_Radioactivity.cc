...
void WCSimGenerator_Radioactivity::Initialize() {
	...
	// Radon diffusion coefficient in water from
	// https://pdfs.semanticscholar.org/a6d1/7962c7ae2af1e9663349be4e85eb304541ca.pdf
	...
	fRn_Border = fRn_PerPMT * nPMT /
	             (2. * TMath::Pi() * fCurrentDetector_R_max * fCurrentDetector_R_max +
	              2. * TMath::Pi() * fCurrentDetector_R_max * fCurrentDetector_Z_max * 2.);
...
void WCSimGenerator_Radioactivity::Configuration(G4int iScenario, G4double dLifeTime) {
	...
	if(fScenario == 0) {
		...
		G4cout << " Scenario " << fScenario << ": Rn concentration is defined with the following parameters "
		       << G4endl;
		G4cout << " PMT number: " << myDetector->Get_Pmts()->size() << G4endl;
		G4cout << " Detector radius: " << fCurrentDetector_R_max
		       << " ; Detector height: " << fCurrentDetector_Z_max * 2. << G4endl;
		G4cout << " Surface: " << (2. * TMath::Pi() * fR2_max + 2. * TMath::Pi() * fR_max * (fZ_max - fZ_min))
		       << G4endl;
		// G4cout << fCurrentDetector_R_max << " " << fCurrentDetector_Z_max * 2 << G4endl;
		G4cout << " Mean activity in the full ID:   " << fIntegral
		       << " mBq  ( Concentration: " << fConcentrationID << " mBq / m^3 ) " << G4endl;
		G4cout << " Mean activity in the fiducial volume:   " << fIntegralFV
		       << " mBq  ( Concentration: " << fConcentrationFV << " mBq / m^3 ) " << G4endl;
	}
	G4cout << " ========================================================================== " << G4endl;
}
...
#include "RnModel_Fit_Params.hh"
	...
	for(int iZ = RNMODEL_BIN_Z_MIN; iZ < RNMODEL_BIN_Z_MAX; iZ++) {
		...
		if(dMaxZ < dMinZ) {
			G4cout << "[ERROR] RnModel: For " << iZ << " dMaxZ < dMinZ this should not happen ( " << dMaxZ
			       << " , " << dMinZ << " ) " << G4endl;
		}
	}
	G4cout << "[NOTICE] RnModel: fConc_Middle is set to " << fConc_Middle << G4endl;
	for(int iZ = RNMODEL_BIN_Z_MIN; iZ < RNMODEL_BIN_Z_MAX; iZ++) {
		G4cout << "[NOTICE] RnModel: Layer R2 " << iZ << " is between: " << vLayer_MinZ_R2[iZ] << " "
		       << vLayer_MaxZ_R2[iZ] << G4endl;
	}
	for(int iR = 0; iR < RNMODEL_BIN_R_MAX; iR++) {
		G4cout << "[NOTICE] RnModel: Layer Z " << iR << " is between: " << vLayer_MinR2_Z[iR] << " "
		       << vLayer_MaxR2_Z[iR] << G4endl;
	}
...
double WCSimGenerator_Radioactivity::RadonFormulaR(double* val, double* par) {
	...
	if(lR2 > dShift_Post) {
		...
		dConcentration =
		    dConstant + (dUpperConc - dDiffusion_C) * DiffusionR(lR2, dShift_Post, dSlope) + dDiffusion;
	}
...
double WCSimGenerator_Radioactivity::RadonFormulaZ(double* val, double* par) {
	...
	if(fRn_Border <= 0.)
		dConcentration_PMT = 0;

	double dConcentration_Below =
	    dConc10 + (dConc10b - dConc10) * DiffusionZ(lZ, dLimit_PeakLeft, dFactorL) + dConcentration_PMT;
	double dConcentration_Peak = dConc10b + dConcentration_PMT;
	double dConcentration_Above =
	    dConc0 + (dConc10b - dConc0) * DiffusionZ(lZ, dLimit_PeakRight, dFactorR) + dConcentration_PMT;

	// G4cout << "[DEBUG] RadonFormulaZ z= " << lZ << " " << dConcentration_Peak << " " <<
	// dConcentration_Below << " " << dConcentration_Above << G4endl;
...
double WCSimGenerator_Radioactivity::RadonFormula(double* val, double* par) {
	...
	//----------------------------------//
	// Extremum cases:
	if(dConcentration_Z < 0 && dConcentration_R2 >= 0)
		return dConcentration_R2;
	if(dConcentration_R2 < 0 && dConcentration_Z >= 0)
		return dConcentration_Z;
