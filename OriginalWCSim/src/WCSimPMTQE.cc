...
G4double WCSimDetectorConstruction::GetPMTQE(G4String CollectionName,
	...
	if(flag == 1) {
		/*ODCOMMENT -- We use photons < 280nm so we need ->*/  // if (PhotonWavelength <= low_wl ||
		                                                       // PhotonWavelength >= high_wl ){
		if(PhotonWavelength <= low_wl || PhotonWavelength >= high_wl || PhotonWavelength <= 280 ||
		   PhotonWavelength >= 660) {
			return 0;
		}
	}
	else if(flag == 0) {
	...
	if(flag == 1) {
		...
		for(int i = 0; i < n; i++) {
			if(PhotonWavelength <= *(wavelength + (i + 1))) {
				wavelengthQE = *(QE + i) + (*(QE + (i + 1)) - *(QE + i)) /
				                               (*(wavelength + (i + 1)) - *(wavelength + i)) *
	...
	if(flag == 1) {
		if(PMT && WLS) {
			return (G4double)(std::max(PMT->GetgQE()->Eval(PhotonWavelength, 0, "S"),
			                           WLS->GetgAbs()->Eval(PhotonWavelength, 0, "S"))) *
			       ratio;
		}
		else {
