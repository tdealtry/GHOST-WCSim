...
namespace {
	...
	Y linterpolate(const X& x, const X* x_begin, const X* x_end, const Y* y_begin) {
...
WCSimDetectorConstruction::WCSimDetectorConstruction(G4int DetConfig,
    ...
    WCSimTuningParams(WCSimTuningPars), totalNum_mPMTs(0), totalNum_mPMTs2(0) {
...
G4VPhysicalVolume* WCSimDetectorConstruction::Construct() {
	...
	if(isNuPrism)
		position.setY(WCIDVerticalPosition);
	...
	G4LogicalVolume* logicExpHall =
	    new G4LogicalVolume(solidExpHall, G4Material::GetMaterial("Vacuum"), "expHall", 0, 0, 0);
	...
	G4VPhysicalVolume* physiExpHall =
	    new G4PVPlacement(0, G4ThreeVector(), logicExpHall, "expHall", 0, false, 0, true);
	...
	G4VPhysicalVolume* physiWCBox =
	    new G4PVPlacement(transform, logicWCBox, "WCBox", logicExpHall, false, checkOverlaps);
	...
	TraverseReplicas(physiWCBox, 0, G4Transform3D(), &WCSimDetectorConstruction::DescribeAndRegisterPMT);

	TraverseReplicas(physiWCBox, 0, G4Transform3D(), &WCSimDetectorConstruction::GetWCGeom);
...
void WCSimDetectorConstruction::UpdateODGeo() {
	...
	WCODCapEdgeLimit =
	    std::min(WCIDDiameter / 2.0 - WCPMTODRadius, WCIDDiameter / 2.0 - WCODWLSPlatesLength / 2);
...
void WCSimDetectorConstruction::CreateCombinedPMTQE(const std::vector<G4String>& CollectionName) {
	...
	for(unsigned iPMT = 0; iPMT < nPMTs; iPMT++) {
		...
		// Debugging output
		G4cout << G4endl;
		G4cout << "### Recover PMT collection name " << CollectionName[iPMT] << G4endl;
		for(int iwl = 0; iwl < nwl; iwl++) {
			G4cout << "wavelength[" << *(awl_b + iwl) << "nm] : " << *(aqe_b + iwl) << G4endl;
		}
	}
	...
	for(unsigned iPMT = 0; iPMT < nPMTs; iPMT++) {
		for(auto pwl = wl_be[iPMT].first; pwl != wl_be[iPMT].second; pwl++) {
			...
			for(unsigned jPMT = 0; jPMT < nPMTs; jPMT++) {
				qe_interps.push_back(
				    linterpolate(*pwl, wl_be[jPMT].first, wl_be[jPMT].second, qe_be[jPMT].first));
			}
...
WCSimWLSProperties* WCSimDetectorConstruction::CreateWLSObject(G4String WLSType) {
	...
	if(WLSType == "EljenEJ286") {
		...
		G4cout << WLSType << " is not a recognized WLS Type. Exiting WCSim." << G4endl;
		exit(1);
	}
}
