...
WCSimWCDigi::WCSimWCDigi(const WCSimWCDigi& right) : G4VDigi() {
...
int WCSimWCDigi::operator==(const WCSimWCDigi& right) const {
	return ((tubeID == right.tubeID) && (pe == right.pe) && (time == right.time) &&
	        (time_presmear == right.time_presmear) && (TriggerTimes == right.TriggerTimes) &&
	        (tubeID == right.tubeID) && (tubeType == right.tubeType));
}
...
void WCSimWCDigi::Print() {
	G4cout << "TubeID: " << tubeID << "Tube type: " << tubeType << "Number of Gates " << NumberOfGates();
	for(unsigned int i = 0; i < pe.size(); i++) {
		G4cout << "Gate = " << i << " PE: " << pe[i] << " Time:" << time[i] << G4endl;
	}
}
...
void WCSimWCDigi::RemoveDigitizedGate(G4int gate) {
	// this removes an element from the maps, vectors, and sets, and counters that were filled by
	// WCSimWCDigitizerBase::AddNewDigit() Gates and TriggerTimes are NOT set
...
bool WCSimWCDigi::SortFunctor_Hit::operator()(const WCSimWCDigi* const& a,
