...
G4LogicalVolume* WCSimDetectorConstruction::ConstructEggShapedHyperK() {
	...
	G4LogicalVolume* blackSheetZLV = new G4LogicalVolume(
	    new G4Box("blackSheetZ", waterTank_Height / 2., waterTank_Height / 2., blackSheetThickness / 2.),
	...
	G4VSolid* waterTank_union =
	    new G4UnionSolid("Union",
	                     waterTank_top,
	                     waterTank_bot,
	                     G4Transform3D(rotm, G4ThreeVector(0, -waterTank_Height / 2., 0)));

	waterTankLV = new G4LogicalVolume(waterTank_union, FindMaterial("G4_WATER"), "Tank");
	waterTankLV->SetVisAttributes(G4VisAttributes::Invisible);

	new G4PVPlacement(0,
	                  G4ThreeVector(0, waterTank_Height / 4., 0),
	                  waterTankLV,
	                  "Tank",
	                  eggShapedHyperKLV,
	                  false,
	...
	pmtCellLV = ConstructRadialPMT(true,
	                               innerPMT_TopR,
	                               innerPMT_Height,
	                               waterTank_UpperA,
	                               innerPMT_Expose,
	...
	pmtCellLV = ConstructRadialPMT(false,
	                               innerPMT_BotR,
	                               innerPMT_Height,
	                               waterTank_LowerB,
	                               innerPMT_Expose,
	...
	new G4PVPlacement(g4rot,
	                  G4ThreeVector(0., 0., -innerPMT_Expose / 2.),
	                  logicWCPMT,
	                  "PMT",
	                  pmtCellLV,
	                  false,
	...
	pmtCellLV = ConstructCeilingPMT(true, innerPMT_TopW, innerPMT_Height, innerPMT_Apitch, innerPMT_Expose);
	...
	pmtCellLV = ConstructCeilingPMT(false, innerPMT_BotW, innerPMT_Height, innerPMT_Apitch, innerPMT_Expose);
	...
	pmtCellLV = ConstructCeilingPMT(true, outerPMT_TopW, outerPMT_Height, outerPMT_Apitch, outerPMT_Expose);
	...
	pmtCellLV = ConstructCeilingPMT(false, outerPMT_BotW, outerPMT_Height, outerPMT_Apitch, outerPMT_Expose);
...
G4VSolid* WCSimDetectorConstruction::ConstructHalf(G4double waterTank_Radius, G4double waterTank_OffSet) {
	...
	G4Trap* waterTank_trap = new G4Trap("TankTrap",
	                                    waterTank_Length / 2.,
	                                    0.,
	                                    0.,
	                                    waterTank_Height / 4.,
	...
	G4VSolid* waterTank_slice = new G4SubtractionSolid(
	    "TankSlice",
	    new G4Tubs("WaterTubs", 0., waterTank_Radius, waterTank_Length / 2., -phi / 2., phi),
	    new G4Box("WaterBox",
	...
	G4VSolid* waterTank_union1 =
	    new G4UnionSolid("Union1",
	                     waterTank_trap,
	                     waterTank_slice,
	                     G4Transform3D(rotm, G4ThreeVector(-waterTank_OffSet, -waterTank_Height / 4., 0)));
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructRadialPMT(G4bool top,
	...
	G4LogicalVolume* pmtAnnulusLV = new G4LogicalVolume(
	    new G4Tubs("PMTAnnulus", radius - 2. * expose, radius, length / 2., -phi / 2., phi),
	...
	if(inner) {
		blackSheetALV = new G4LogicalVolume(
		    new G4Tubs("blackSheetA", radius, radius + blackSheetThickness, length / 2., -phi / 2., phi),
		    FindMaterial("G4_POLYETHYLENE"),
		    "blackSheetA");
	...
	G4LogicalVolume* pmtRingLV =
	    new G4LogicalVolume(new G4Tubs("PMTRing", radius - 2. * expose, radius, pitchZ / 2., -phi / 2., phi),
	                        FindMaterial("G4_WATER"),
	                        "PMTRings");
	pmtRingLV->SetVisAttributes(G4VisAttributes::Invisible);

	G4LogicalVolume* pmtCellLV = new G4LogicalVolume(
	    new G4Tubs("PMTCell", radius - 2. * expose, radius, pitchZ / 2., -dphi / 2., dphi),
	    FindMaterial("G4_WATER"),
	    "PMTCell");
	...
	G4VPhysicalVolume* pmtRingPV = new G4PVReplica("PMTRing", pmtRingLV, pmtAnnulusLV, kZAxis, nz, pitchZ);

	new G4PVReplica("PMTCell", pmtCellLV, pmtRingPV, kPhi, nphi, dphi, -phi / 2.);
	...
	new G4PVPlacement(rotm,
	                  G4ThreeVector(-t * offset, -waterTank_Height / 4., 0),
	                  pmtAnnulusLV,
	                  "PMTAnnulus",
	                  waterTankLV,
	                  false,
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructEndWallPMT() {
	G4LogicalVolume* pmtCellLV = new G4LogicalVolume(
	    new G4Box("PMTCell", innerPMT_Apitch / 2., innerPMT_Apitch / 2., innerPMT_Apitch / 2.),
	    FindMaterial("G4_WATER"),
	    "PMTCell");
	pmtCellLV->SetVisAttributes(G4VisAttributes::Invisible);
...

	{
		...
		while(y < innerPMT_Height - 2 * innerPMT_Expose) {
			...
			if(dv.mag() > innerPMT_TopR - 2 * innerPMT_Expose)
				break;
			j++;
			y  += innerPMT_Apitch;
		...
		while(y < innerPMT_Height - 2 * innerPMT_Expose) {
			...
			if(dv.mag() > innerPMT_BotR - 2 * innerPMT_Expose)
				break;
			j++;
			y  += innerPMT_Apitch;
			yv = G4ThreeVector(0., y, 0.);
		...
		G4LogicalVolume* pmtSlabLV = new G4LogicalVolume(
		    new G4Box("PMTSlab", innerPMT_Apitch / 2., (yt + yb) / 2., innerPMT_Apitch / 2.),
		    FindMaterial("G4_WATER"),
		    "PMTSlab");
		pmtSlabLV->SetVisAttributes(G4VisAttributes::Invisible);

		new G4PVReplica("PMTCell", pmtCellLV, pmtSlabLV, kYAxis, ny, innerPMT_Apitch);
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructCeilingPMT(G4bool top,
	...
	G4LogicalVolume* pmtSlabLV =
	    new G4LogicalVolume(new G4Box("PMTSlab", xlength / 2., size / 2., zlength / 2.),
	                        FindMaterial("G4_WATER"),
	                        "PMTSlab");
	pmtSlabLV->SetVisAttributes(G4VisAttributes::Invisible);
	...
	if(inner) {
		blackSheetYLV = new G4LogicalVolume(
		    new G4Box("blackSheetY", xlength / 2., blackSheetThickness / 2., zlength / 2.),
		    FindMaterial("G4_POLYETHYLENE"),
		    "blackSheetY");
	...
	G4LogicalVolume* pmtSliceLV =
	    new G4LogicalVolume(new G4Box("PMTSlice", xlength / 2., size / 2., pitch / 2.),
	                        FindMaterial("G4_WATER"),
	                        "PMTSlab");
	pmtSliceLV->SetVisAttributes(G4VisAttributes::Invisible);

	G4LogicalVolume* pmtCellLV = new G4LogicalVolume(new G4Box("PMTCell", pitch / 2., size / 2., pitch / 2.),
	                                                 FindMaterial("G4_WATER"),
	                                                 "PMTCell");
	pmtCellLV->SetVisAttributes(G4VisAttributes::Invisible);

	G4VPhysicalVolume* pmtSlicePV = new G4PVReplica("PMTSlice", pmtSliceLV, pmtSlabLV, kZAxis, nz, pitch);

	new G4PVReplica("PMTCell", pmtCellLV, pmtSlicePV, kXAxis, nx, pitch);
...
G4Material* WCSimDetectorConstruction::FindMaterial(G4String name) {
	if(name == "G4_AIR")
		return FindMaterial("Air");
	if(name == "G4_WATER")
		return FindMaterial("Water");
	if(name == "G4_POLYETHYLENE")
		return FindMaterial("Blacksheet");
