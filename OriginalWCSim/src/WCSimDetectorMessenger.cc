...
WCSimDetectorMessenger::WCSimDetectorMessenger(WCSimDetectorConstruction* WCSimDet) :
	...
	SetPMTCoverage->SetGuidance(
	    "Set the PMT percentage coverage to be used. Should be necessarily called before WCGeom, as the detector parameters are defined there based on coverage.");
	...
	SetPMTCoverage2->SetGuidance(
	    "Set the PMT percentage coverage to be used for second PMT type. Should be necessarily called before WCGeom, as the detector parameters are defined there based on coverage.");
	...
	mPMT_material_outer_thickness =
	    new G4UIcmdWithADoubleAndUnit("/WCSim/mPMT/material_outer_thickness", this);
	...
	mPMT_numModulesFixed->SetGuidance(
	    "Keep the number of mPMT modules in the detector fixed even though mPMT dimensions change.");
	...
	mPMT_pmtOpeningAngle->SetGuidance(
	    "Set size of PMT sub-assembly (opening angle). Must be able to contain reflector if wanted.");
	...
	SetPMTType->SetCandidates(
	    "PMT3inch PMT3inchGT PMT3inchR12199_02 PMT3inchR14374 PMT3inch_ETEL9302B PMT5inch PMT8inch PMT10inchHQE PMT10inch PMT12inchHQE HPD20inchHQE PMT20inch");
	SetPMTType->SetDefaultValue("PMT10inch");

	// Set the vertical position of the nuPRISM-lite detector
	SetDetectorVerticalPosition =
	    new G4UIcmdWithADoubleAndUnit("/WCSim/nuPRISM/SetDetectorVerticalPosition", this);
	SetDetectorVerticalPosition->SetGuidance(
	    "Set the vertical position of the nuPRISM inner detector (unit mm cm m).");
...
void WCSimDetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
	if(command == PMTConfig) {
		...
		if(newValue == "SuperK") {
			...
			WCSimDetector->Cylinder_60x74_3inchmPMT_14perCent();  // MUST be Called after the mPMT settings,
			                                                      // otherwise unknown
		}
		else if(newValue == "Cylinder_60x74_3inchmPMT_40perCent") {
			WCSimDetector->Cylinder_60x74_3inchmPMT_40perCent();  // MUST be Called after the mPMT settings,
			                                                      // otherwise unknown
		}
		else if(newValue == "Cylinder_60x74_3inch_14perCent") {
			WCSimDetector->Cylinder_60x74_3inch_14perCent();  // MUST be Called after the mPMT settings,
			                                                  // otherwise unknown
		}
		else if(newValue == "Cylinder_60x74_3inch_40perCent") {
			WCSimDetector->Cylinder_60x74_3inch_40perCent();  // MUST be Called after the mPMT settings,
			                                                  // otherwise unknown
		}
	...
	if(command == WCConstruct) {
		// If the OD geometry has been changed, then reconstruct the whole tank with the proper recalculated
		// dimensions
		if(WCSimDetector->GetODEdited() == true) {
			WCSimDetector->UpdateODGeo();
		}
	...
	if(command == mPMT_CylRadiusCurv) {
		G4cout << "Set Vessel Radius of Curvature of MultiPMT to " << newValue << " "
		       << G4endl;  // doesn't work
	...
	if(command == mPMT_material_outer_thickness) {
		WCSimDetector->SetmPMT_MaterialOuterThickness(
		    mPMT_material_outer_thickness->GetNewDoubleValue(newValue));
	}
	...
	if(command == mPMT_DistPMTVessel) {
		G4cout << "Set Distance of PMT(s) to pressure vessel to " << newValue << " "
		       << G4endl;  // doesn't work
		// G4cout << "Set Cylinder Radius of MultiPMT to " << newValue  << " " << G4endl;
	...
	if(WCSimDetector->GetIsNuPrism()) {
		if(command == SetPMTType)
			WCSimDetector->SetPMTType(newValue);
		// else if (command == SetPMTCoverage) WCSimDetector->SetPMTCoverage(atof(newValue));
		else if(command == SetDetectorHeight)
			WCSimDetector->SetDetectorHeight(SetDetectorHeight->GetNewDoubleValue(newValue));
		else if(command == SetDetectorVerticalPosition)
			WCSimDetector->SetDetectorVerticalPosition(
			    SetDetectorVerticalPosition->GetNewDoubleValue(newValue));
		else if(command == SetDetectorDiameter)
			WCSimDetector->SetDetectorDiameter(SetDetectorDiameter->GetNewDoubleValue(newValue));
