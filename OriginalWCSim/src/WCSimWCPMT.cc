...
WCSimWCPMT::WCSimWCPMT(G4String name, WCSimDetectorConstruction* inDetector, G4String inDetectorElement) :
	...
	if(detectorElement == "tank")
		...
		collectionName.push_back("WCRawPMTSignalCollection2");
	else if(detectorElement == "OD")
		collectionName.push_back("WCRawPMTSignalCollection_OD");
	else
...
void WCSimWCPMT::Digitize() {
	...
	G4cout << "WCSimWCPMT::Digitize Making digits collection (WCSimWCDigitsCollection*)"
	       << DigitsCollectionName << " for " << detectorElement << " and calling MakePeCorrection on "
	       << WCCollectionName << " to fill it." << G4endl;
#endif
...
void WCSimWCPMT::MakePeCorrection(WCSimWCHitsCollection* WCHC) {
	...
	// Correct timing to be within 1 sec (needed for radioactive decay as forcing the decay lead to some
	// strange results)
	...
	for(G4int i = 0; i < WCHC->entries(); i++) {
		...
		for(G4int ip = 0; ip < (*WCHC)[i]->GetTotalPe(); ip++) {
			...
			// should not use /grdm/decayBiasProfile biasprofile.dat as it messes up all the timing of the
			// decays, and force to use only one nucleus
			if(i == 0 && ip == 0 && RelativeHitTime &&
			   !fFirst_Time_Flag /*&& (*WCHC)[i]->GetTime(ip) > 1e5*/) {  // Set Max at 10 musec
				// G4cout << " Apply time correction to event hits of " << (*WCHC)[i]->GetTime(ip) << " ns" <<
				// G4endl;
				fFirst_Time_Flag = true;
				fFirst_Time      = time_true;
			}
			time_PMT = time_true - fFirst_Time;  // currently no PMT time smearing applied
			// G4cout<<"First time = "<<fFirst_Time<<", time true = "<<time_true<<", PMT time =
			// "<<time_PMT<<G4endl;

			peSmeared = rn1pe();
#ifdef DEBUG
			G4cout << "tube : " << i << " (ID=" << tube << ")"
			       << " hit in tube : " << ip << " (time=" << time_true << "ns)"
			       << " pe value : " << peSmeared << G4endl;  // TD debug
#endif
			...
			if(DigiHitMapPMT[tube] == 0) {
				...
				(*DigitsCollection)[DigiHitMapPMT[tube] - 1]->SetLogicalVolume(
				    (*WCHC)[0]->GetLogicalVolume());
...
#ifdef DEBUG
		G4cout << "tube : " << i << " (ID=" << tube << ")"
		       << " total digitized pe in : " << QinTOT << G4endl;  // TD debug
#endif
	}  // Loop over PMTs
#ifdef DEBUG
	G4cout << G4endl << G4endl << "The maximum amount of pe stored by a PMT is : " << maxTotalPe << G4endl
	       << G4endl;
#endif
}
