...
WCSimRunActionMessenger::WCSimRunActionMessenger(WCSimRunAction* WCSimRA) : WCSimRun(WCSimRA) {
	...
	WriteDefaultRootFile->SetGuidance(
	    "Do you want to write out the standard ROOT file format. The new FLAT one you get for free.");
...
void WCSimRunActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
	...
	if(command == RootFile) {
		...
		G4cout << "You chose to write out the standard ROOT file: "
		       << WriteDefaultRootFile->GetNewBoolValue(newValue) << G4endl;
	}

	if(command == RooTracker) {
		WCSimRun->SetSaveRooTracker(RooTracker->GetNewBoolValue(newValue));
		if(newValue)
			G4cout << "Saving NEUT RooTracker information to output file" << G4endl;
	}
