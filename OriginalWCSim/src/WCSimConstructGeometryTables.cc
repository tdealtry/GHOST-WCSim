...
void WCSimDetectorConstruction::PrintGeometryTree(G4VPhysicalVolume* aPV,
	...
	for(int levels = 0; levels < aDepth; levels++)
		G4cout << " ";
	G4cout << aPV->GetName() << " Level:" << aDepth << " Pos:" << aTransform.getTranslation()
	       << " Rot:" << aTransform.getRotation().getTheta() / deg << ","
	       << aTransform.getRotation().getPhi() / deg << "," << aTransform.getRotation().getPsi() / deg
	       << G4endl;
}
...
void WCSimDetectorConstruction::GetWCGeom(G4VPhysicalVolume* aPV,
	...
	if((aPV->GetName() == "WCBox")) {  // last condition is the HyperK Envelope name.
		...
		WCXRotation = G4ThreeVector(aTransform.getRotation().xx(),
		                            aTransform.getRotation().xy(),
		                            aTransform.getRotation().xz());
		WCYRotation = G4ThreeVector(aTransform.getRotation().yx(),
		                            aTransform.getRotation().yy(),
		                            aTransform.getRotation().yz());
		WCZRotation = G4ThreeVector(aTransform.getRotation().zx(),
		                            aTransform.getRotation().zy(),
		                            aTransform.getRotation().zz());
	}
	...
	if((aPV->GetName() == "WCCapBlackSheet") ||
	   (aPV->GetName().find("glassFaceWCPMT") != std::string::npos)) {
		...
		// G4cout << "determine height: " << zmin << "  " << zmax << " " << aPV->GetName()<<" " << z  <<
		// G4endl;
	}
}
...
void WCSimDetectorConstruction::DescribeAndRegisterPMT(G4VPhysicalVolume* aPV,
	...
	if(aPV->GetName() == WCIDCollectionName || aPV->GetName() == WCIDCollectionName2 ||
	   aPV->GetName() == WCODCollectionName) {
		...
		if(aPV->GetName() == WCIDCollectionName) {
			if(tubeLocationMap.find(tubeTag) != tubeLocationMap.end()) {
				...
				G4cerr << "Assigned to both tube #" << tubeLocationMap[tubeTag] << " and #" << totalNumPMTs
				       << G4endl;
			...
			if(tubeLocationMap2.find(tubeTag) != tubeLocationMap2.end()) {
				G4cerr << "Repeated tube tag: " << tubeTag << G4endl;
				G4cerr << "Assigned to both tube #" << tubeLocationMap2[tubeTag] << " and #" << totalNumPMTs2
				       << G4endl;
				G4cerr << "Cannot continue -- hits will not be recorded correctly." << G4endl;
				G4cerr
				    << "Please make sure that logical volumes with multiple placements are each given a unique copy number"
			...
			if(ODtubeLocationMap.find(tubeTag) != ODtubeLocationMap.end()) {
				G4cerr << "Repeated tube tag: " << tubeTag << G4endl;
				G4cerr << "Assigned to both tube #" << ODtubeLocationMap[tubeTag] << " and #"
				       << totalNumODPMTs << G4endl;
				G4cerr << "Cannot continue -- hits will not be recorded correctly." << G4endl;
				G4cerr
				    << "Please make sure that logical volumes with multiple placements are each given a unique copy number"
				    << G4endl;
...
void WCSimDetectorConstruction::DumpGeometryTableToFile() {
	...
	if(isEggShapedHyperK) {
		...
		geoFile << "Detector radius & height " << setw(8) << innerradius << setw(8) << WCCylInfo[2];
	}
	geoFile << G4endl;

	geoFile << "Type 1 ID PMT number & size " << setw(10) << totalNumPMTs << setw(8) << WCPMTSize << setw(4)
	        << G4endl;

	geoFile << "Type 2 ID PMT number & size " << setw(10) << totalNumPMTs2 << setw(8) << WCPMTSize2 << setw(4)
	        << G4endl;

	geoFile << "OD PMT number & size        " << setw(10) << totalNumODPMTs << setw(8) << WCPMTODRadius
	        << setw(4) << G4endl;

	geoFile << "Centre offset " << setw(8) << WCOffset(0) << setw(8) << WCOffset(1) << setw(8) << WCOffset(2)
	        << G4endl;
	...
	for(int tubeID = totalNumPMTs; tubeID >= 1; tubeID--) {
		...
		if(mPMTIDMap[tubeID].second == 0 || mPMTIDMap[tubeID].second == nID_PMTs) {
			...
			//  or if the highest numbered tube of mPMT is at centre of mPMT module (e.g. on cap points in ±z
			//  direction)
		...
		geoFile.precision(9);
		geoFile << setw(7) << tubeID << " " << setw(5) << mPMTIDMap[tubeID].first << " " << setw(3)
		        << mPMTIDMap[tubeID].second << " " << setw(15) << newTransform.getTranslation().getX() / cm
		        << " " << setw(15) << newTransform.getTranslation().getY() / cm << " " << setw(15)
		        << newTransform.getTranslation().getZ() / cm << " " << setw(12) << pmtOrientation.x() << " "
		        << setw(12) << pmtOrientation.y() << " " << setw(12) << pmtOrientation.z() << " " << setw(3)
		        << cylLocation << G4endl;
	...
	for(int tubeID = totalNumPMTs2; tubeID >= 1; tubeID--) {
		...
		geoFile.precision(9);
		geoFile << setw(7) << tubeID << " " << setw(5) << mPMTIDMap2[tubeID].first << " " << setw(3)
		        << mPMTIDMap2[tubeID].second << " " << setw(15) << newTransform.getTranslation().getX() / cm
		        << " " << setw(15) << newTransform.getTranslation().getY() / cm << " " << setw(15)
		        << newTransform.getTranslation().getZ() / cm << " " << setw(12) << pmtOrientation.x() << " "
		        << setw(12) << pmtOrientation.y() << " " << setw(12) << pmtOrientation.z() << " " << setw(3)
		        << cylLocation << G4endl;
	...
	for(int tubeID = totalNumODPMTs; tubeID >= 1; tubeID--) {
		...
		geoFile.precision(9);
		geoFile << setw(7) << tubeID << " " << setw(5) << mPMTODMap[tubeID].first << " " << setw(3)
		        << mPMTODMap[tubeID].second << " " << setw(15)
		        << newTransform.getTranslation().getX() / CLHEP::cm << " " << setw(15)
		        << newTransform.getTranslation().getY() / CLHEP::cm << " " << setw(15)
		        << newTransform.getTranslation().getZ() / CLHEP::cm << " " << setw(12) << pmtOrientation.x()
		        << " " << setw(12) << pmtOrientation.y() << " " << setw(12) << pmtOrientation.z() << " "
		        << setw(3) << cylLocation << G4endl;
