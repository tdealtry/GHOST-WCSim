...
void NRooTrackerVtx::Reset() {
	...
	if(GeomPath)
		GeomPath->SetString("Not-set");
	if(GeneratorName)
		GeneratorName->SetString("Not-set");
	if(EvtCode)
		EvtCode->SetString("Not-set");
	if(OrigFileName)
		OrigFileName->SetString("Not-set");
	if(OrigTreeName)
		OrigTreeName->SetString("Not-set");
...
void NRooTrackerVtx::Print(const Option_t*) const {
	...
	for(int i = 0; i < StdHepN; i++) {
		cout << setfill(' ') << setw(4) << i << " -> pdgc: "
		     << " / ist: " << setfill(' ') << setw(3) << StdHepStatus[i] << " / mom: " << setfill(' ')
		     << setw(3) << StdHepFm[i] << " / daughters: (" << setfill(' ') << setw(3) << StdHepFd[i] << ", "
		     << setfill(' ') << setw(3) << StdHepLd[i] << ") / E = " << setfill(' ') << setw(10)
		     << StdHepP4[i][kNStdHepIdxE] << ", px = " << setfill(' ') << setw(10)
		     << StdHepP4[i][kNStdHepIdxPx] << ", py = " << setfill(' ') << setw(10)
		     << StdHepP4[i][kNStdHepIdxPy] << ", pz = " << setfill(' ') << setw(10)
		     << StdHepP4[i][kNStdHepIdxPz] << endl;
	}

	cout << endl << "Jnu-beam pass through info:" << endl;
	cout << "  -->  NuParentPdg      = " << NuParentPdg << endl;
	cout << "  -->  NuParentDecMode  = " << NuParentDecMode << endl;
	cout << "  -->  NuParentDecP4[4] = " << NuParentDecP4[0] << ", " << NuParentDecP4[1] << ", "
	     << NuParentDecP4[2] << ", " << NuParentDecP4[3] << endl;
	cout << "  -->  NuParentDecX4[4] = " << NuParentDecX4[0] << ", " << NuParentDecX4[1] << ", "
	     << NuParentDecX4[2] << ", " << NuParentDecX4[3] << endl;
	cout << "  -->  NuParentProP4[4] = " << NuParentProP4[0] << ", " << NuParentProP4[1] << ", "
	     << NuParentProP4[2] << ", " << NuParentProP4[3] << endl;
	cout << "  -->  NuParentProX4[4] = " << NuParentProX4[0] << ", " << NuParentProX4[1] << ", "
	     << NuParentProX4[2] << ", " << NuParentProX4[3] << endl;
	...
	cout << "ip NEabspvert[ip] NEabstpvert[ip] NEipvert[ip] NEiverti[ip] NEivertf[ip] NEdirvert[ip][3]"
	     << endl;
	for(int ip = 0; ip < NEnvcvert; ip++) {
		cout << ip << " " << NEabspvert[ip] << " " << NEabstpvert[ip] << " " << NEipvert[ip] << " "
		     << NEiverti[ip] << " " << NEivertf[ip] << " ";
		for(int ip2 = 0; ip2 < 3; ip2++) {
