...
WCSimTuningMessenger::WCSimTuningMessenger(WCSimTuningParameters* WCTuningPars) :
    WCSimTuningParams(WCTuningPars) {
	...
	Qeff->SetGuidance(
	    "Set the correction of cathode QE parameter (nominal = 1). Used when changing the cathode reflectivity to keep the total charge constant.");
	...
	CommandWCODWLSCladdingReflectivity =
	    new G4UIcmdWithADouble("/WCSim/tuning/WCODWLSCladdingReflectivity", this);
...
void WCSimTuningMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
	...
	if(command == Rayff) {
		...
		G4cout << "Seting the correction of cathode QE parameter (nominal = 1) "
		       << Qeff->GetNewDoubleValue(newValue) << G4endl;
	}
	...
	// TD 2019.7.16
		...
		G4cout << "Setting saturation threshold for PMTs %f (-1 means no saturation) "
		       << PMTSatur->GetNewDoubleValue(newValue) << G4endl;
	}
	...
	// G4cout << " Setting the uncertainty on amount of charge created in the PMT " <<
	// Qoiff->GetNewDoubleValue(newValue) << G4endl;
	...
	// G4cout << " Setting the non-linearity info for time resolutions in PMTs to " <<
	// NLTinfo->GetNewDoubleValue(newValue) << G4endl;
	...
	// }
		...
		// Set the Top Veto PMT Spacing
		WCSimTuningParams->SetWCODWLSCladdingReflectivity(
		    CommandWCODWLSCladdingReflectivity->GetNewDoubleValue(newValue));
		G4cout << "Setting OD WLS plate cladding reflectivity "
		       << CommandWCODWLSCladdingReflectivity->GetNewDoubleValue(newValue) << G4endl;
	}

	else if(command == CommandWCODTyvekReflectivity) {
		// Set the Top Veto PMT Spacing
		WCSimTuningParams->SetWCODTyvekReflectivity(
		    CommandWCODTyvekReflectivity->GetNewDoubleValue(newValue));
		G4cout << "Setting OD tyvek reflectivity "
		       << CommandWCODTyvekReflectivity->GetNewDoubleValue(newValue) << G4endl;
	}
}
