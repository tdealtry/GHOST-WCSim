...
G4bool WCSimWCSD::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
	...
	if(detectorElement == "tank")
		...
		WCCollectionName = fdet->GetODCollectionName();
	...
	//  Not an issue yet, because then interior needs to be a sensitive detector, when postStepPoint is the
	//  glass.
	...
	if(detectorElement == "tank")
		...
		replicaNumber = WCSimDetectorConstruction::GetTubeID2(tubeTag.str());
	else if(detectorElement == "OD")
		replicaNumber = WCSimDetectorConstruction::GetODTubeID(tubeTag.str());
	else
		G4cout << "detectorElement not defined..." << G4endl;
	...
	if(G4UniformRand() <= photonQE) {
		...
		theta_angle      = acos(fabs(local_z) / sqrt(pow(local_x, 2) + pow(local_y, 2) + pow(local_z, 2))) /
		              3.1415926 * 180.;
