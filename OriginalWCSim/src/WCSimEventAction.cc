...
#ifdef DEBUG
	G4cout << "Debug B.Q: hybrid PMT = " << myDetector->GetHybridPMT()
	       << ", hybrid geom, so construct digitizer for 2 PMT types" << G4endl;
#endif
...
void WCSimEventAction::CreateDAQInstances() {
	...
	G4cout << "Creating digitizer and trigger class instances in WCSimEventAction::CreateDAQInstances()"
	       << G4endl;
	...
	if(DigitizerChoice == "SKI") {
		WCSimWCDigitizerSKI* WCDM =
		    new WCSimWCDigitizerSKI("WCReadoutDigits", detectorConstructor, DAQMessenger, "tank");
	...
	if(TriggerChoice == "NDigits") {
		WCSimWCTriggerNDigits* WCTM =
		    new WCSimWCTriggerNDigits("WCReadout", detectorConstructor, DAQMessenger, "tank");
		DMman->AddNewModule(WCTM);
	}
	else if(TriggerChoice == "NDigits2") {
		WCSimWCTriggerNDigits2* WCTM =
		    new WCSimWCTriggerNDigits2("WCReadout", detectorConstructor, DAQMessenger, "tank");
		DMman->AddNewModule(WCTM);
	}
	else if(TriggerChoice == "NoTrigger") {
		WCSimWCTriggerNoTrigger* WCTM =
		    new WCSimWCTriggerNoTrigger("WCReadout", detectorConstructor, DAQMessenger, "tank");
		DMman->AddNewModule(WCTM);
	}
	else {
	...
	if(DigitizerChoice == "SKI") {
		...
		if(detectorConstructor->GetHybridPMT()) {
			WCDM2 =
			    new WCSimWCDigitizerSKI("WCReadoutDigits2", detectorConstructor, DAQMessenger, "tankPMT2");
	...
	if(detectorConstructor->GetIsODConstructed()) {
		if(DigitizerChoice == "SKI") {
			WCSimWCDigitizerSKI* WCDM_OD =
			    new WCSimWCDigitizerSKI("WCReadoutDigits_OD", detectorConstructor, DAQMessenger, "OD");
			DMman->AddNewModule(WCDM_OD);
		}

		// create your choice of trigger module
		if(TriggerChoice == "NDigits") {
			WCSimWCTriggerNDigits* WCTM_OD =
			    new WCSimWCTriggerNDigits("WCReadout_OD", detectorConstructor, DAQMessenger, "OD");
			DMman->AddNewModule(WCTM_OD);
		}
		else if(TriggerChoice == "NDigits2") {
			WCSimWCTriggerNDigits2* WCTM_OD =
			    new WCSimWCTriggerNDigits2("WCReadout_OD", detectorConstructor, DAQMessenger, "OD");
			DMman->AddNewModule(WCTM_OD);
		}
		else if(TriggerChoice == "NoTrigger") {
			WCSimWCTriggerNoTrigger* WCTM_OD =
			    new WCSimWCTriggerNoTrigger("WCReadout_OD", detectorConstructor, DAQMessenger, "OD");
			DMman->AddNewModule(WCTM_OD);
		}
		else {
			G4cerr << "Unknown TriggerChoice " << TriggerChoice << G4endl;
...
void WCSimEventAction::EndOfEventAction(const G4Event* evt) {
	...
	if(generatorAction->IsUsingPoissonPMT()) {
		...
		for(std::vector<WCSimPmtInfo*>::iterator pmtIt = detectorConstructor->Get_Pmts()->begin();
			...
			for(size_t existingHit = 0; existingHit < WCHC->GetSize(); existingHit++) {
				...
				if((*WCHC)[existingHit]->GetTubeID() == (*pmtIt)->Get_tubeid()) {
					...
					G4cout
					    << "/mygen/pmtPoisson Error: Hits found when placing dummy PEs. Use lower energy primary particles or set '/mygen/pmtPoisson' to false for a real run."
					    << G4endl;

					if((*WCHC)[existingHit]->GetTotalPe() > nPoisson) {
						G4cout
						    << "/mygen/pmtPoisson Error: More PEs in original hit than in the Poisson throw. Poisson distribution will be distorted. Use lower energy primary particles or set '/mygen/pmtPoisson' to false for a real run."
						    << G4endl;
					}
					else {
						// Take into account already exising PEs... this is not ideal as they might not be in
						// time.
			...
			const G4ThreeVector& pos =
			    detectorConstructor->GetTubeTransform((*pmtIt)->Get_tubeid()).getTranslation();
			(*WCHC)[hitIndex]->SetTubeID((*pmtIt)->Get_tubeid());
			(*WCHC)[hitIndex]->SetTrackID(0);
			(*WCHC)[hitIndex]->SetEdep(0.);
			(*WCHC)[hitIndex]->SetPos(pos);
			(*WCHC)[hitIndex]->SetRot(
			    detectorConstructor->GetTubeTransform((*pmtIt)->Get_tubeid()).getRotation());
	...
	G4cout << "WCSimEventAction::EndOfEventAction() (WCSimWCHitsCollection*)" << WCIDCollectionName << " has "
	       << WCHC->entries() << " entries" << G4endl;
	G4cout << G4endl;
#endif
	...
	WCSimWCPMT* WCDMPMT = (WCSimWCPMT*)DMman->FindDigitizerModule("WCReadoutPMT");
	...
	WCSimWCAddDarkNoise* WCDNM = (WCSimWCAddDarkNoise*)DMman->FindDigitizerModule("WCDarkNoise");
	...
	WCSimWCDigitizerBase* WCDM = (WCSimWCDigitizerBase*)DMman->FindDigitizerModule("WCReadoutDigits");
	...
	WCSimWCTriggerBase* WCTM = (WCSimWCTriggerBase*)DMman->FindDigitizerModule("WCReadout");

	// tell it the dark noise rate (for calculating the average dark occupancy -> can adjust the NDigits
	// threshold)
...
#ifdef TIME_DAQ_STEPS
	ms->Stop();
	G4cout << " Digtization :  Real = " << ms->RealTime() << " ; CPU = " << ms->CpuTime() << "\n";
#endif
	...
	WCSimWCTriggeredDigitsCollection* WCDC =
	    (WCSimWCTriggeredDigitsCollection*)DMman->GetDigiCollection(WCDCID);
	/*
	// To use Do like This:
	// --------------------
	if(WCDC)
	...
	*/
...
#ifdef DEBUG
	G4cout << "Debug B.Q: Entering the PMT2 type readout, hybrid = " << detectorConstructor->GetHybridPMT()
	       << ", PMT list = " << detectorConstructor->GetIDCollectionName() << G4endl;
#endif
	WCSimWCTriggeredDigitsCollection* WCDC2 = nullptr;

	if(detectorConstructor->GetHybridPMT()) {
		G4cout << "Debug B.Q: Entering the PMT2 type readout, hybrid = "
		       << detectorConstructor->GetHybridPMT() << G4endl;
		...
		// Get a pointer to the WC PMT module
		WCDMPMT2 = (WCSimWCPMT*)DMman->FindDigitizerModule("WCReadoutPMT2");
		...
		// Get a pointer to the WC Dark Noise Module
		WCDNM2 = (WCSimWCAddDarkNoise*)DMman->FindDigitizerModule("WCDarkNoise2");
		...
		// Get a pointer to the WC Digitizer Module
		WCDM2 = (WCSimWCDigitizerBase*)DMman->FindDigitizerModule("WCReadoutDigits2");
		...
		// Get a pointer to the WC Trigger Module
		WCTM2 = (WCSimWCTriggerBase*)DMman->FindDigitizerModule("WCReadout2");
	...
	if(detectorConstructor->GetIsODConstructed()) {
		...
		if(HCE) {
			...
			G4int collectionID = SDman->GetCollectionID(name);
			if(collectionID > -1)
				WCHC_OD = (WCSimWCHitsCollection*)HCE->GetHC(collectionID);
			G4cout << G4endl;
	...
	for(unsigned int u = 0; u < nvtxs; u++) {
		...
		for(G4int i = 0; i < n_trajectories; i++) {
			...
			if(abs(trj->GetPDGEncoding()) == PDG_e && trj->GetParentID() == (int)u + 1 &&
			   trj->GetTrackID() < trkid_e) {
				trkid_e = trj->GetTrackID();
				idx_e   = i;
			}
			if(abs(trj->GetPDGEncoding()) == PDG_v_e && trj->GetParentID() == (int)u + 1 &&
			   trj->GetTrackID() < trkid_v_e) {
				trkid_v_e = trj->GetTrackID();
				idx_v_e   = i;
			}
			if(abs(trj->GetPDGEncoding()) == PDG_gam && trj->GetParentID() == (int)u + 1 &&
			   trj->GetTrackID() < trkid_gam) {
		...
		if(idx_e != INT_MAX) {
			WCSimTrajectory* trj = (WCSimTrajectory*)((*(evt->GetTrajectoryContainer()))[idx_e]);
			jhfNtuple.ipnu[npar] = trj->GetPDGEncoding();  // id
			jhfNtuple.flag[npar] = 0;                      // target
			jhfNtuple.m[npar] =
			    particleTable->FindParticle(trj->GetPDGEncoding())->GetPDGMass();  // mass (always a neutrino)
			jhfNtuple.p[npar]        = trj->GetInitialMomentum().mag();            // momentum magnitude
			jhfNtuple.E[npar]        = sqrt(jhfNtuple.m[npar] * jhfNtuple.m[npar] +
                                     jhfNtuple.p[npar] * jhfNtuple.p[npar]);  // energy (total!)
		...
		if(idx_v_e != INT_MAX) {
			WCSimTrajectory* trj = (WCSimTrajectory*)((*(evt->GetTrajectoryContainer()))[idx_v_e]);
			jhfNtuple.ipnu[npar] = trj->GetPDGEncoding();  // id
			jhfNtuple.flag[npar] = 0;                      // target
			jhfNtuple.m[npar] =
			    particleTable->FindParticle(trj->GetPDGEncoding())->GetPDGMass();  // mass (always a neutrino)
		...
		if(idx_gam != INT_MAX) {
			WCSimTrajectory* trj = (WCSimTrajectory*)((*(evt->GetTrajectoryContainer()))[idx_gam]);
			jhfNtuple.ipnu[npar] = trj->GetPDGEncoding();  // id
			jhfNtuple.flag[npar] = 0;                      // target
			jhfNtuple.m[npar] =
			    particleTable->FindParticle(trj->GetPDGEncoding())->GetPDGMass();  // mass (always a neutrino)
	...
	if(GetRunAction()->GetRootFileOption()) {
		...
		if(detectorConstructor->GetIsODConstructed()) {
			FillRootEvent(event_id, jhfNtuple, trajectoryContainer, WCDC_hits_OD, WCDC_OD, "OD");
		}
	...
	if(!SavedOptions) {
		...
		if(detectorConstructor->GetHybridPMT())
			WCDNM2->SaveOptionsToOutput(wcsimopt, "tankPMT2");
...
G4int WCSimEventAction::WCSimEventFindStartingVolume(G4ThreeVector vtx) {
	...
	if(vtxVolumeName == "outerTube" || vtxVolumeName == "innerTube" || vtxVolumeName == "rearEndCap" ||
	   vtxVolumeName == "frontEndCap")
		...
		if((vtxVolumeName.contains("WCBarrel")) ||
		   (vtxVolumeName.contains("Tank")))  // aah I added "Tank" as MB equivalent of Barrel
			vtxvol = 10;
		else if(vtxVolumeName == "WCBox")
			vtxvol = -2;
		else if(vtxVolumeName.contains("PMT") || vtxVolumeName.contains("Cap") ||
...
G4int WCSimEventAction::WCSimEventFindStoppingVolume(G4String stopVolumeName) {
	...
	if(stopVolumeName.contains("WC") && !stopVolumeName.contains("FV")) {
		...
		if((stopVolumeName.contains("WCBarrel")) ||
		   (stopVolumeName.contains("Tank")))  // aah I added "Tank" as MB equivalent of Barrel
			stopvol = 10;
		else if(stopVolumeName == "WCBox")
			stopvol = -2;
		else if(stopVolumeName.contains("PMT") || stopVolumeName.contains("Cap") ||
		...
		if(stopVolumeName == "WCFVBarrel" || stopVolumeName == "WCFVAnnulus" || stopVolumeName == "WCFVRing")
			stopvol = 10;
...
void WCSimEventAction::FillRootEvent(G4int event_id,
	...
	for(int index = 0; index < ngates; index++) {
		if(index >= 1) {
			...
			wcsimrootevent->SetHeader(event_id, 0, 0, index + 1);  // date & # of subevent
		...
		wcsimrootevent->SetTriggerInfo(WCTM->GetTriggerType(index),
		                               triggerInfo);  // Added by B.Q to keep track of he trigger, 2019/01/17
		triggerInfo.clear();
	}
	...
	for(int i = 0; i < n_trajectories; i++) {
		...
		if(trj->GetPDGEncoding() == 13)
			muonList.insert(trj->GetTrackID());
		if(trj->GetPDGEncoding() == -13)
			antimuonList.insert(trj->GetTrackID());
		if(trj->GetPDGEncoding() == 211)
			pionList.insert(trj->GetTrackID());
		if(trj->GetPDGEncoding() == -211)
			antipionList.insert(trj->GetTrackID());
		if(trj->GetParentID() == 0)
			primaryList.insert(trj->GetTrackID());
		...
		if(trj->GetSaveFlag()) {
			...
			if(trj->GetCreatorProcessName() == "nCapture" ? detectorConstructor->SaveCaptureInfo() :
			                                                !((ipnu == 22) && (parentType == 999))) {
				int choose_event = 0;

				if(ngates) {

					if(ttime > WCTM->GetTriggerTime(0) + 950. && WCTM->GetTriggerTime(1) + 950. > ttime)
						choose_event = 1;
					if(ttime > WCTM->GetTriggerTime(1) + 950. && WCTM->GetTriggerTime(2) + 950. > ttime)
						choose_event = 2;
					if(choose_event >= ngates)
						choose_event = ngates - 1;  // do not overflow the number of events
				}
			...
			if(detectorConstructor->SaveCaptureInfo() && trj->GetCreatorProcessName() == "nCapture") {
				G4cout << "Capture particle: " << trj->GetParticleName() << " Parent: " << trj->GetParentID()
				       << " T:" << ttime << " vtx:(" << start[0] << "," << start[1] << "," << start[2]
				       << ") dir:(" << dir[0] << "," << dir[1] << "," << dir[2] << ") E:" << energy << G4endl;
	...
	if(detectorElement == "tank")
		fpmts = detectorConstructor->Get_Pmts();
	else if(detectorElement == "tankPMT2")
		fpmts = detectorConstructor->Get_Pmts2();
	else if(detectorElement == "OD")
		fpmts = detectorConstructor->Get_ODPmts();
	...
	if(WCDC_hits) {
		...
		for(int idigi = 0; idigi < WCDC_hits->entries(); idigi++) {
			...
			for(G4int id = 0; id < (*WCDC_hits)[idigi]->GetTotalPe(); id++) {
				...
				hit_photon_startpos  = TVector3((*WCDC_hits)[idigi]->GetPhotonStartPos(id)[0],
                                               (*WCDC_hits)[idigi]->GetPhotonStartPos(id)[1],
                                               (*WCDC_hits)[idigi]->GetPhotonStartPos(id)[2]);
				hit_photon_endpos    = TVector3((*WCDC_hits)[idigi]->GetPhotonEndPos(id)[0],
                                             (*WCDC_hits)[idigi]->GetPhotonEndPos(id)[1],
                                             (*WCDC_hits)[idigi]->GetPhotonEndPos(id)[2]);
				hit_photon_startdir  = TVector3((*WCDC_hits)[idigi]->GetPhotonStartDir(id)[0],
                                               (*WCDC_hits)[idigi]->GetPhotonStartDir(id)[1],
                                               (*WCDC_hits)[idigi]->GetPhotonStartDir(id)[2]);
				hit_photon_enddir    = TVector3((*WCDC_hits)[idigi]->GetPhotonEndDir(id)[0],
			...
			if(digi_tubeid < NPMTS_VERBOSE || digi_tubeid == VERBOSE_PMT) {
				G4cout << "Adding " << truetime.size() << " Cherenkov hits in tube " << digi_tubeid
				       << " with truetime:smeartime:primaryparentID";
				for(size_t id = 0; id < truetime.size(); id++) {
					G4cout << " " << truetime[id] << "\t" << smeartime[id] << "\t" << primaryParentID[id]
					       << G4endl;
				}  // id
				G4cout << G4endl;
			}
	...
	if(WCDC) {
		...
		for(int index = 0; index < ngates; index++) {
				...
				if((*WCDC)[k]->HasHitsInGate(index)) {
					...
					for(unsigned int iv = 0; iv < vec_pe.size(); iv++) {
						...
						if(tubeID < NPMTS_VERBOSE || tubeID == VERBOSE_PMT) {
							G4cout << "Adding digit " << iv << " for PMT " << tubeID << " pe " << vec_pe[iv]
							       << " time " << vec_time[iv] << " digicomp";
						...
						wcsimrootevent->AddCherenkovDigiHit(vec_pe[iv],
						                                    vec_time[iv],
						                                    tubeID,
						                                    pmt->Get_mPMTid(),
						                                    pmt->Get_mPMT_pmtid(),
			...
			wcsimrootevent->SetNumDigitizedTubes(
			    countdigihits);  // TF: Aren't these total number of hits, NOT Tubes?
			...
			G4cout << "digihits collection size =  " << wcsimrootevent->GetCherenkovDigiHits()->GetEntries()
			       << "\n";
			G4cout << "tracks collection size =  " << wcsimrootevent->GetTracks()->GetEntries()
	...
	for(int i = 0; i < wcsimrootsuperevent->GetNumberOfEvents(); i++) {
		...
		if(evtID % 100 == 0)
			G4cout << ">>>Root event " << std::setw(5) << evtID << "\n";
		// if (WCDC){
	...
	/*
	// Check we are supposed to be saving the NEUT vertex and that the generator was given a NEUT vector file
	to process
...
void WCSimEventAction::FillRootEventHybrid(G4int event_id,
	...
	for(int i = 0; i < wcsimrootsuperevent->GetNumberOfEvents(); i++) {
		...
		int evtID      = wcsimrootevent->GetHeader()->GetEvtNum();
		if(evtID % 100 == 0)
			G4cout << ">>>Root event " << std::setw(5) << evtID << G4endl;
		// if (WCDC){
		// G4cout <<"WC digi:"<<std::setw(4)<<wcsimrootevent->GetNcherenkovdigihits()<<"  ";
		// G4cout <<"WC digi sumQ:"<<std::setw(4)<<wcsimrootevent->GetSumQ()<<"  ";
	}
...
void WCSimEventAction::FillFlatTree(G4int event_id,
	...
	thisNtuple->vtx_z =
	    injhfNtuple.vtxs[0][2];  // now copying from jhfNtuple, in the future replacing jhfNtuple
	...
	for(int i = 0; i < n_trajectories; i++) {
		...
		// Add later?: https://twiki.cern.ch/twiki/bin/view/Geant4/LoweAtomicDeexcitation (neutron capture and
		// O16)
		if(trj->GetSaveFlag()) {
			// initial point of the trajectory
			...
			thisNtuple->energy[thisNtuple->nTracks] =
			    sqrt(mom.mag2() +
			         thisNtuple->mass[thisNtuple->nTracks] * thisNtuple->mass[thisNtuple->nTracks]) /
			    CLHEP::MeV;
	...
	if(detectorElement == "tank")
		nMpmtID_pmts = detectorConstructor->GetmPMT_nID();
	else if(detectorElement == "tankPMT2")
		nMpmtID_pmts = detectorConstructor->GetmPMT_nID2();
	else if(detectorElement == "OD")
		nMpmtID_pmts = 0;
	else {
		G4cout << "Unknown detector element: " << detectorElement << G4endl;
	...
	if(WCDC_hits) {
		...
		for(int idigi = 0; idigi < WCDC_hits->entries(); idigi++) {
			...
			G4cout << "TubeID " << digi_tubeid << " (TrackID " << (*WCDC_hits)[idigi]->GetTrackID()
			       << ") has " << (*WCDC_hits)[idigi]->GetTotalPe() << " CherenkovHits (Dark noise included)"
			       << G4endl;
	#endif
			...
			for(G4int id = 0; id < (*WCDC_hits)[idigi]->GetTotalPe(); id++) {
	...
	#ifdef _SAVE_RAW_HITS_VERBOSE
				G4cout << "Hit " << id << " with time " << digi_time << " and parentID " << hit_parentid
				       << G4endl;
	#endif
				...
				*/
				...
				thisNtuple->mPMT_pmtid[totNumHits] =
				    (digi_tubeid % nMpmtID_pmts == 0 ? nMpmtID_pmts :
				                                       digi_tubeid % nMpmtID_pmts);  // No. 1 to nID
				...
				thisNtuple->tube_dirx[totNumHits] =
				    pmt->Get_orienx();  // Can now also grab the ThreeVector Orientation from WCDigi
			...
			for(G4int id = totNumHits; id > (totNumHits - (*WCDC_hits)[idigi]->GetTotalPe()); id--) {
				thisNtuple->totalPe_noNoise[id - 1] =
				    (*WCDC_hits)[idigi]->GetTotalPe() -
				    numNoiseHits;  // id-1 because I already did totNumHits++ above for next PMT
			}
		}  // idigi
	}
	...
	*fpmts = detectorConstructor->Get_Pmts2();
	...
	if (WCDC_hits)
	{
	  ...
	  thisNtuple->tubeid[totNumHits] = digi_tubeid;
	  thisNtuple->mPMTid[totNumHits] = digi_tubeid/nMpmtID_pmts;
	  thisNtuple->mPMT_pmtid[totNumHits] = (digi_tubeid%nMpmtID_pmts == 0 ? nMpmtID_pmts :
  digi_tubeid%nMpmtID_pmts ); // No. 1 to nID
	  ...
	  thisNtuple->tube_y[totNumHits] = pos[1];
	  thisNtuple->tube_z[totNumHits] = pos[2];
	  thisNtuple->tube_dirx[totNumHits] = pmt->Get_orienx();  //Can now also grab the ThreeVector Orientation
  from WCDigi thisNtuple->tube_diry[totNumHits] = pmt->Get_orieny(); thisNtuple->tube_dirz[totNumHits] =
  pmt->Get_orienz(); totNumHits++; if(hit_parentid > 0){ totNumHits_noDN++;
	  ...
	  numTubesHit_noDN++;
	    //Subtract noiseHits from realHits:
	    for(G4int id = totNumHits; id > (totNumHits-(*WCDC_hits)[idigi]->GetTotalPe()); id--){
	  thisNtuple->totalPe_noNoise[id-1] = (*WCDC_hits)[idigi]->GetTotalPe() - numNoiseHits; //id-1 because I
  already did totNumHits++ above for next PMT
	    }
	  }//idigi
	}
	thisNtuple->numTubesHit = WCDC_hits->entries();
	...
	*/
	...
	for(int index = 0; index < ngates; index++) {
		...
		if(WCDC) {
			...
			for(int k = 0; k < WCDC->entries(); k++) {
				if((*WCDC)[k]->HasHitsInGate(index)) {
					...
					for(unsigned int iv = 0; iv < vec_pe.size();
					    iv++) {  // TF: so far haven't seen iv > 0 yet. Delayed hits?
#ifdef SAVE_DIGITS_VERBOSE
						G4cout << "Adding digit " << iv << " for PMT " << tubeID << " pe " << vec_pe[iv]
						       << " time " << vec_time[iv] << " digicomp";
						for(unsigned int ivv = 0; ivv < vec_digicomp[iv].size(); ivv++)
		...
		GetRunAction()->GetTriggerTree()->Fill();
		GetRunAction()->GetEventInfoTree()->Fill();
		// Check we are supposed to be saving the NEUT vertex and that the generator was given a NEUT vector
		// file to process If there is no NEUT vector file an empty NEUT vertex will be written to the output
		// file
		if(GetRunAction()->GetSaveRooTracker() && generatorAction->IsUsingRootrackerEvtGenerator()) {
