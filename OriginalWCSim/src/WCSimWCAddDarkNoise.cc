...
WCSimWCAddDarkNoise::~WCSimWCAddDarkNoise() {
	...
	DarkRateMessenger->RemoveDarkRateInstance(
	    detectorElement);  // DarkRateMessenger singleton calls it's destructor when map is empty
	DarkRateMessenger = 0;
}
...
void WCSimWCAddDarkNoise::SetPMTDarkDefaults() {
	...
	if(detectorElement == "tank")
		...
		G4cout << "### detectorElement undefined ..." << G4endl;
	WCSimPMTObject* PMT = myDetector->GetPMTPointer(WCCollectionName);

	double const conversion_to_kHz =
	    1000000;  // ToDo: remove this and treat DarkRate in CLHEP units throughout the class.
...
void WCSimWCAddDarkNoise::AddDarkNoise() {
	...
	if(detectorElement == "tank")
		...
		thecollectionName = "WCRawPMTSignalCollection2";
	else if(detectorElement == "OD")
		thecollectionName = "WCRawPMTSignalCollection_OD";
	else
		G4cout << "### detectorElement undefined ..." << G4endl;
	...
	if(detectorElement == "tankPMT2") {
		G4cout << "WCSimWCAddDarkNoise::AddDarkNoise retrieved hit collection (WCSimWCDigitsCollection*)"
		       << thecollectionName << " which has " << WCHCPMT->entries() << " entries" << G4endl;
	}
	else if(detectorElement == "OD") {
		G4cout << "WCSimWCAddDarkNoise::AddDarkNoise ☆ retrieved hit collection (WCSimWCDigitsCollection*)"
		       << thecollectionName << " which has " << WCHCPMT->entries() << " entries" << G4endl;
	...
	if(thetriggertype == "TankDigits") {
		...
		// if triggertype is TankDigits, then we should add noise in the windows around the *tank* digits, not
		// this collection's digits, because those are the windows that will be relevant when reading out
		// triggers.
		...
		if(detectorElement == "tankPMT2") {
			G4cout
			    << "WCSimWCAddDarkNoise::AddDarkNoise retrieved hit collection (WCSimWCDigitsCollection*)WCRawPMTSignalCollection for finding dark noise windows, which has "
			    << WCHCPMT_tank->entries() << " entries" << G4endl;
		}
		else if(detectorElement == "OD") {
			G4cout
			    << "WCSimWCAddDarkNoise::AddDarkNoise ☆ retrieved hit collection (WCSimWCDigitsCollection*)WCRawPMTSignalCollection for finding dark noise windows, which has "
	   ...
	   (this->PMTDarkRate > 1E-307)) {
		...
		if(DarkMode == 1) {
			if(add_noise_based_on_tank) {
				WCSimWCAddDarkNoise* WCDNM_tank =
				    (WCSimWCAddDarkNoise*)DigiMan->FindDigitizerModule("WCDarkNoise");
				double DarkWindow_tank = WCDNM_tank->GetDarkWindow();
#ifdef HYPER_VERBOSITY
				G4cout
				    << "WCSimWCAddDarkNoise::AddDarkNoise calling FindDarkNoiseRanges() with tank raw collection and darkwindow size"
				    << G4endl;
#endif
...
#ifdef HYPER_VERBOSITY
		G4cout << "WCSimWCAddDarkNoise::AddDarkNoise adding dark noise hits in " << result.size()
		       << " window(s) around found digits." << G4endl;
#endif
		int windowfordarknoise = 0;

		for(std::vector<std::pair<double, double> >::iterator it2 = result.begin(); it2 != result.end();
		    it2++) {
#ifdef HYPER_VERBOSITY
			G4cout << "WCSimWCAddDarkNoise::AddDarkNoise adding dark noise in window number "
			       << windowfordarknoise << G4endl;
#endif
			windowfordarknoise++;
...
void WCSimWCAddDarkNoise::AddDarkNoiseBeforeDigi(WCSimWCDigitsCollection* WCHCPMT, double num1, double num2) {
	...
	if(detectorElement == "tank")
		...
		thenum_pmts = myDetector->GetTotalNumPmts2();
	else if(detectorElement == "OD")
		thenum_pmts = myDetector->GetTotalNumODPmts();
	else
		G4cout << "### detectorElement undefined ..." << G4endl;
	...
	if(detectorElement == "tank")
		...
		thewcpmtname = "WCReadoutPMT2";
	else if(detectorElement == "OD")
		thewcpmtname = "WCReadoutPMT_OD";
	else
		G4cout << "### detectorElement undefined ..." << G4endl;
	...
	G4cout << "Dark Rate = " << this->PMTDarkRate
	       << ", WCSimWCAddDarkNoise::AddDarkNoiseBeforeDigi Going to add " << nnoispmt
	       << " dark noise hits in time window [" << num1 << "," << num2 << "] duration " << num2 - num1
	       << G4endl;
#endif
	for(int i = 0; i < nnoispmt; i++) {
		...
		// each PMT.
		int noise_pmt =
		    static_cast<int>(G4UniformRand() * number_pmts) + 1;  // so that pmt numbers runs from 1 to Npmt
		...
		if(list[noise_pmt] == 0) {
			...
			if(detectorElement == "tank")
				ahit->SetLogicalVolume(G4LogicalVolumeStore::GetInstance()->GetVolume(
				    myDetector->GetDetectorName() + "-glassFaceWCPMT"));
			else if(detectorElement == "tankPMT2")
				ahit->SetLogicalVolume(G4LogicalVolumeStore::GetInstance()->GetVolume(
				    myDetector->GetDetectorName() + "-glassFaceWCPMT2"));
			else if(detectorElement == "OD")
				ahit->SetLogicalVolume(G4LogicalVolumeStore::GetInstance()->GetVolume(
				    myDetector->GetDetectorName() + "-glassFaceWCPMT_OD"));
			else
				G4cout << "### detectorElement undefined ..." << G4endl;

			// G4cout<<"1
			// "<<(G4LogicalVolumeStore::GetInstance()->GetVolume("glassFaceWCPMT"))->GetName()<<"\n";
			...
			// G4RotationMatrix pmt_rotation(hit_rot[0], hit_rot[1], hit_rot[2]); // WRONG, takes Euler angles
			// as arg. These are not Euler angles. ahit->SetRot(pmt_rotation);
			...
			if(noise_pmt < NPMTS_VERBOSE)
				G4cout
				    << "WCSimWCAddDarkNoise::AddDarkNoiseBeforeDigi Added NEW DIGI with dark noise hit at time "
				    << current_time << " to PMT " << noise_pmt << G4endl;
#endif
		}
		else {
			...
			(*WCHCPMT)[list[noise_pmt] - 1]->SetPreSmearTime(
			    PMTindex[noise_pmt],
			    current_time);  // presmear==postsmear for dark noise
			...
			(*WCHCPMT)[list[noise_pmt] - 1]->SetPhotonStartPos(PMTindex[noise_pmt],
			                                                   (*WCHCPMT)[list[noise_pmt] - 1]->GetPos());
			(*WCHCPMT)[list[noise_pmt] - 1]->SetPhotonEndPos(PMTindex[noise_pmt],
			                                                 (*WCHCPMT)[list[noise_pmt] - 1]->GetPos());
			(*WCHCPMT)[list[noise_pmt] - 1]->SetPhotonStartDir(
			    PMTindex[noise_pmt],
			    -(*WCHCPMT)[list[noise_pmt] - 1]->GetOrientation());
			(*WCHCPMT)[list[noise_pmt] - 1]->SetPhotonEndDir(
			    PMTindex[noise_pmt],
			    -(*WCHCPMT)[list[noise_pmt] - 1]->GetOrientation());
			PMTindex[noise_pmt]++;
#ifdef WCSIMWCADDDARKNOISE_VERBOSE
			if(noise_pmt < NPMTS_VERBOSE)
				G4cout
				    << "WCSimWCAddDarkNoise::AddDarkNoiseBeforeDigi Added to exisiting digi a dark noise hit at time "
				    << current_time << " to PMT " << noise_pmt << G4endl;
#endif
		}
...
void WCSimWCAddDarkNoise::FindDarkNoiseRanges(WCSimWCDigitsCollection* WCHCPMT, double width) {
	...
	if(detectorElement == "mrd")
		G4cout << "WCSimWCAddDarkNoise::FindDarkNoiseRanges ☆ " << ranges.size()
