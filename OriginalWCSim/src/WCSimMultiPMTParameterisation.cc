...
void WCSimMultiPMTParameterisation::PreCalculateTransform() {
	...
	for(unsigned int copy = 0; copy < fNoPmt; copy++) {
		...
		// = 1/(vNiCLocal[vCircleLocal[copy]]) * (pi * vNiCLocal[vCircleLocal[copy]] -
		// vNiCLocal[vCircleLocal[copy]] * pi + 2pi ) = 2pi / (vNiCLocal[vCircleLocal[copy]]) --> more logical
		// and intuitive
		// G4double angle = ((vNiCLocal[vCircleLocal[copy]]-2)*CLHEP::pi/vNiCLocal[vCircleLocal[copy]]); //
		// Internal angle of each polygon
		...
		// G4cout << "PMT " << copy << " (theta,phi): (" << theta*(180./CLHEP::pi)  << "," <<
		// phi*(180./CLHEP::pi) << ")" << G4endl; G4cout << "PMT " << copy << " (x,y,z): (" << origin.getX()
		// << "," <<  origin.getY() << "," << origin.getZ() << ")" << G4endl;
