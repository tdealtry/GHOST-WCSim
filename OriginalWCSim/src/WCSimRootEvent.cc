...
void WCSimRootTrigger::Initialize()  // actually allocate memory for things in here
{
	...
	// TClonesArray of WCSimRootCherenkovHits
	fCherenkovHits     = new TClonesArray("WCSimRootCherenkovHit", 10000);
	fCherenkovHitTimes = new TClonesArray("WCSimRootCherenkovHitTime", 10000);
	...
	// TClonesArray of WCSimRootCherenkovDigiHits
	fCherenkovDigiHits = new TClonesArray("WCSimRootCherenkovDigiHit", 10000);
...
void WCSimRootTrigger::SetHeader(Int_t i, Int_t run, int64_t date, Int_t subevent) {
...
void WCSimRootTrigger::SetTriggerInfo(TriggerType_t trigger_type, std::vector<Double_t> trigger_info) {
...
void WCSimRootTrigger::SetCaptureParticle(Int_t parent,
	...
	if(ipnu == 22)
		capture->AddGamma(id, energy, dir);
	else
		capture->SetInfo(vtx, time, ipnu);
}

//_____________________________________________________________________________

void WCSimRootPi0::Set(Double_t pi0Vtx[3], Int_t gammaID[2], Double_t gammaE[2], Double_t gammaVtx[2][3]) {
	for(int i = 0; i < 2; i++) {
...
void WCSimRootCapture::SetInfo(Double_t captureVtx[3], Double_t captureT, Int_t captureNucleus) {
	for(int i = 0; i < 3; i++)
		fCaptureVtx[i] = captureVtx[i];
	fCaptureT       = captureT;
...
void WCSimRootCapture::AddGamma(Int_t gammaID, Double_t gammaE, Double_t gammaDir[3]) {
...
WCSimRootCaptureGamma::WCSimRootCaptureGamma(Int_t id, Double_t energy, Double_t* dir) {
	fID     = id;
	fEnergy = energy;
	for(int i = 0; i < 3; i++)
		fDir[i] = dir[i];
...
WCSimRootTrack* WCSimRootTrigger::AddTrack(Int_t ipnu,
	...
	WCSimRootTrack* track = new(tracks[fNtrack_slots++]) WCSimRootTrack(ipnu,
	                                                                    flag,
	                                                                    m,
	                                                                    p,
	                                                                    E,
...
WCSimRootTrack* WCSimRootTrigger::AddTrack(WCSimRootTrack* track) {
	...
	TClonesArray& tracks      = *fTracks;
	WCSimRootTrack* track_out = new(tracks[fNtrack_slots++]) WCSimRootTrack(track->GetIpnu(),
	                                                                        track->GetFlag(),
	                                                                        track->GetM(),
	                                                                        track->GetP(),
	                                                                        track->GetE(),
...
WCSimRootCherenkovHit* WCSimRootTrigger::AddCherenkovHit(Int_t tubeID,
	...
	for(unsigned int i = 0; i < truetime.size(); i++) {
		...
		new(cherenkovhittimes[fNcherenkovhittimes++]) WCSimRootCherenkovHitTime(truetime[i],
		                                                                        primParID[i],
		                                                                        photonStartTime[i],
		                                                                        startPos,
		                                                                        endPos,
...
#ifdef DEBUG
	std::cout << "Debug B.Q: A new Cherenkov hit was added for tube " << tubeID
	          << ", corresponding to PMT = " << mPMTID << ", and to small PMT in mPMT = " << mPMT_PMTID
	          << std::endl;
#endif
	...
	WCSimRootCherenkovHit* cherenkovhit =
	    new(cherenkovhits[fNcherenkovhits++]) WCSimRootCherenkovHit(tubeID, mPMTID, mPMT_PMTID, WC_Index);
...
WCSimRootCherenkovHit::WCSimRootCherenkovHit(Int_t tubeID, Int_t mPMTID, Int_t mPMT_PMTID, Int_t totalPe[2]) {
	// Create a WCSimRootCherenkovHitIndex object and fill it with stuff
...
WCSimRootCherenkovDigiHit* WCSimRootTrigger::AddCherenkovDigiHit(Double_t q,
	...
	WCSimRootCherenkovDigiHit* cherenkovdigihit = new(cherenkovdigihits[fNcherenkovdigihits_slots++])
	    WCSimRootCherenkovDigiHit(q, t, tubeid, mpmtid, mpmt_pmtid, photon_ids);
	fNcherenkovdigihits++;
...
WCSimRootCherenkovDigiHit* WCSimRootTrigger::AddCherenkovDigiHit(WCSimRootCherenkovDigiHit* digit) {
	...
	WCSimRootCherenkovDigiHit* cherenkovdigihit =
	    new(cherenkovdigihits[fNcherenkovdigihits_slots++]) WCSimRootCherenkovDigiHit(digit->GetQ(),
	                                                                                  digit->GetT(),
	                                                                                  digit->GetTubeId(),
	                                                                                  digit->GetmPMTId(),
	                                                                                  digit->GetmPMT_PMTId(),
...
bool WCSimRootTrack::CompareAllVariables(const WCSimRootTrack* c) const {
	...
	failed = (!ComparisonPassed(fStartvol, c->GetStartvol(), typeid(*this).name(), __func__, "Startvol")) ||
	         failed;
	failed =
	    (!ComparisonPassed(fStopvol, c->GetStopvol(), typeid(*this).name(), __func__, "Stopvol")) || failed;
	for(int i = 0; i < 3; i++) {
		failed = (!ComparisonPassed(fDir[i],
		                            c->GetDir(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "Dir", i))) ||
		         failed;
	}  // i
	for(int i = 0; i < 3; i++) {
		failed = (!ComparisonPassed(fPdir[i],
		                            c->GetPdir(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "Pdir", i))) ||
		         failed;
	}  // i
	for(int i = 0; i < 3; i++) {
		failed = (!ComparisonPassed(fStop[i],
		                            c->GetStop(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "Stop", i))) ||
		         failed;
	}  // i
	for(int i = 0; i < 3; i++) {
		failed = (!ComparisonPassed(fStart[i],
		                            c->GetStart(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "Start", i))) ||
		         failed;
	}  // i
	failed =
	    (!ComparisonPassed(fParenttype, c->GetParenttype(), typeid(*this).name(), __func__, "Parenttype")) ||
	    failed;
	...
	failed = (!ComparisonPassed(fParentId, c->GetParentId(), typeid(*this).name(), __func__, "ParentId")) ||
	         failed;
...
bool WCSimRootCherenkovHit::CompareAllVariables(const WCSimRootCherenkovHit* c) const {
	...
	failed =
	    (!ComparisonPassed(fmPMT_PMTID, c->GetmPMT_PMTID(), typeid(*this).name(), __func__, "mPMT_PMTID")) ||
	    failed;
	for(int i = 0; i < 2; i++) {
		failed = (!ComparisonPassed(fTotalPe[i],
		                            c->GetTotalPe(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "TotalPe", i))) ||
		         failed;
	}  // i
...
bool WCSimRootCherenkovHitTime::CompareAllVariables(const WCSimRootCherenkovHitTime* c) const {
	...
	failed = (!ComparisonPassed(fTruetime, c->GetTruetime(), typeid(*this).name(), __func__, "Truetime")) ||
	         failed;
	failed = (!ComparisonPassed(fPrimaryParentID,
	                            c->GetParentID(),
	                            typeid(*this).name(),
	                            __func__,
	                            "PrimaryParentID")) ||
	         failed;
	failed = (!ComparisonPassed(fPhotonStartTime,
	                            c->GetPhotonStartTime(),
	                            typeid(*this).name(),
	                            __func__,
	                            "PhotonStartTime")) ||
	         failed;
	for(int i = 0; i < 3; i++) {
		failed = (!ComparisonPassed(fPhotonStartPos[i],
		                            c->GetPhotonStartPos(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "PhotonStartPos", i))) ||
		         failed;
		failed = (!ComparisonPassed(fPhotonEndPos[i],
		                            c->GetPhotonEndPos(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "PhotonEndPos", i))) ||
		         failed;
		failed = (!ComparisonPassed(fPhotonStartDir[i],
		                            c->GetPhotonStartDir(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "PhotonStartDir", i))) ||
		         failed;
		failed = (!ComparisonPassed(fPhotonEndDir[i],
		                            c->GetPhotonEndDir(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "PhotonEndDir", i))) ||
		         failed;
	}  // i
...
bool WCSimRootCherenkovDigiHit::CompareAllVariables(const WCSimRootCherenkovDigiHit* c) const {
	...
	failed =
	    ...
	    failed;
	failed =
	    (!ComparisonPassedVec(fPhotonIds, c->GetPhotonIds(), typeid(*this).name(), __func__, "PhotonIds")) ||
	    failed;
...
bool WCSimRootCapture::CompareAllVariables(const WCSimRootCapture* c) const {
	...
	failed = (!ComparisonPassed(fCaptureParent,
	                            c->GetCaptureParent(),
	                            typeid(*this).name(),
	                            __func__,
	                            "CaptureParent")) ||
	         failed;
	for(int i = 0; i < 3; i++) {
		failed = (!ComparisonPassed(fCaptureVtx[i],
		                            c->GetCaptureVtx(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("CaptureVtx[%d]", i))) ||
		         failed;
	}  // i
	failed = (!ComparisonPassed(fNGamma, c->GetNGamma(), typeid(*this).name(), __func__, "NGamma")) || failed;
	failed = (!ComparisonPassed(fTotalGammaE,
	                            c->GetTotalGammaE(),
	                            typeid(*this).name(),
	                            __func__,
	                            "TotalGammaE")) ||
	         failed;
	failed = (!ComparisonPassed(fCaptureT, c->GetCaptureT(), typeid(*this).name(), __func__, "CaptureT")) ||
	         failed;
	failed = (!ComparisonPassed(fCaptureNucleus,
	                            c->GetCaptureNucleus(),
	                            typeid(*this).name(),
	                            __func__,
	                            "CaptureNucleus")) ||
	...
	if(this->GetGammas()->GetEntries() != c->GetGammas()->GetEntries()) {
		cerr << "WCSimRootTrigger::CompareAllVariables() Different number of gammas: "
		     << this->GetGammas()->GetEntries() << ", " << c->GetGammas()->GetEntries() << endl;
		failed = true;
	}
...
#endif
		bool thisfailed = !((WCSimRootCaptureGamma*)this->GetGammas()->At(i))
		                       ->CompareAllVariables((WCSimRootCaptureGamma*)c->GetGammas()->At(i));
...
bool WCSimRootEventHeader::CompareAllVariables(const WCSimRootEventHeader* c) const {
	...
	failed = (!ComparisonPassed(fSubEvtNumber,
	                            c->GetSubEvtNumber(),
	                            typeid(*this).name(),
	                            __func__,
	                            "SubEvtNumber")) ||
...
bool WCSimRootPi0::CompareAllVariables(const WCSimRootPi0* c) const {
	...
	for(int i = 0; i < 3; i++) {
		failed = (!ComparisonPassed(fPi0Vtx[i],
		                            c->GetPi0Vtx(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "Pi0Vtx", i))) ||
		         failed;
	}  // i
	for(int i = 0; i < 2; i++) {
		failed = (!ComparisonPassed(fGammaID[i],
		                            c->GetGammaID(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "GammaID", i))) ||
		         failed;
	}  // i
	for(int i = 0; i < 2; i++) {
		failed = (!ComparisonPassed(fGammaE[i],
		                            c->GetGammaE(i),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("%s[%d]", "GammaE", i))) ||
		         failed;
	}  // i
	for(int i = 0; i < 2; i++) {
		for(int j = 0; j < 3; j++) {
			failed = (!ComparisonPassed(fGammaVtx[i][j],
			                            c->GetGammaVtx(i, j),
			                            typeid(*this).name(),
			                            __func__,
			                            TString::Format("%s[%d][%d]", "GammaVtx", i, j))) ||
			         failed;
		}  // j
	}      // i
...
bool WCSimRootTrigger::CompareAllVariables(const WCSimRootTrigger* c, bool deep_comparison) const {
	...
	if(this->GetTracks()->GetEntries() != c->GetTracks()->GetEntries()) {
		cerr << "WCSimRootTrigger::CompareAllVariables() Different number of tracks: "
		     << this->GetTracks()->GetEntries() << ", " << c->GetTracks()->GetEntries() << endl;
		failed = true;
	}
	if(this->GetCherenkovHits()->GetEntries() != c->GetCherenkovHits()->GetEntries()) {
		cerr << "WCSimRootTrigger::CompareAllVariables() Different number of Cherenkov hits: "
		     << this->GetCherenkovHits()->GetEntries() << ", " << c->GetCherenkovHits()->GetEntries() << endl;
		failed = true;
	}
	if(this->GetCherenkovHitTimes()->GetEntries() != c->GetCherenkovHitTimes()->GetEntries()) {
		cerr << "WCSimRootTrigger::CompareAllVariables() Different number of Cherenkov hit times: "
		     << this->GetCherenkovHitTimes()->GetEntries() << ", " << c->GetCherenkovHitTimes()->GetEntries()
		     << endl;
		failed = true;
	}
	if(this->GetCherenkovDigiHits()->GetEntries() != c->GetCherenkovDigiHits()->GetEntries()) {
		cerr << "WCSimRootTrigger::CompareAllVariables() Different number of Cherenkov digi hits: "
		     << this->GetCherenkovDigiHits()->GetEntries() << ", " << c->GetCherenkovDigiHits()->GetEntries()
		     << endl;
		failed = true;
	}
	if(this->GetCaptures()->GetEntries() != c->GetCaptures()->GetEntries()) {
		cerr << "WCSimRootTrigger::CompareAllVariables() Different number of captures: "
		     << this->GetCaptures()->GetEntries() << ", " << c->GetCaptures()->GetEntries() << endl;
		failed = true;
	}
	...
	while(true) {
		...
		if(!tmp_track_1 || !tmp_track_2)
			break;
#ifdef VERBOSE_COMPARISON
		cout << "Comparing track " << ithis " in this with track" << ithat " in that" << endl;
#endif
		failed = !(tmp_track_1->CompareAllVariables(tmp_track_2)) || failed;
		ncomp_track++;
	}  // ithis ithat
	if(ncomp_track != fNtrack && ncomp_track != c->GetNtrack()) {
		cerr << "Only compared " << ncomp_track << " tracks. There should be "
		     << TMath::Min(fNtrack, c->GetNtrack()) << " comparisons" << endl;
	}

	// check hits & hit times
	for(int i = 0;
	    i < TMath::Min(this->GetCherenkovHits()->GetEntries(), c->GetCherenkovHits()->GetEntries());
	    i++) {
#ifdef VERBOSE_COMPARISON
		cout << "Hit " << i << endl;
#endif
		bool thisfailed = !((WCSimRootCherenkovHit*)this->GetCherenkovHits()->At(i))
		                       ->CompareAllVariables((WCSimRootCherenkovHit*)c->GetCherenkovHits()->At(i));
		failed = thisfailed || failed;
...
#endif
				failed = !((WCSimRootCherenkovHitTime*)this->GetCherenkovHitTimes()->At(i))
				              ->CompareAllVariables(
				                  (WCSimRootCherenkovHitTime*)c->GetCherenkovHitTimes()->At(i)) ||
				         failed;
	...
	while(true) {
		...
		if(!tmp_digit_1 || !tmp_digit_2)
			break;
#ifdef VERBOSE_COMPARISON
		cout << "Comparing digit " << ithis " in this with digit" << ithat " in that" << endl;
#endif
	...
	if(ncomp_digi != fNcherenkovdigihits && ncomp_digi != c->GetNcherenkovdigihits()) {
		cerr << "Only compared " << ncomp_digi << " digits. There should be "
		     << TMath::Min(fNcherenkovdigihits, c->GetNcherenkovdigihits()) << " comparisons" << endl;
	}
	if(!deep_comparison)
		...
		// We're therefore going to do the check under the assumption that the order of digits can be
	...
	for(int i = 0; i < TMath::Min(this->GetCaptures()->GetEntries(), c->GetCaptures()->GetEntries()); i++) {
		...
		cout << "Capture " << i << endl;
#endif
		bool thisfailed = !((WCSimRootCapture*)this->GetCaptures()->At(i))
		                       ->CompareAllVariables((WCSimRootCapture*)c->GetCaptures()->At(i));
		failed = thisfailed || failed;
	}  // i (WCSimRootCapture)

	failed = (!ComparisonPassed(fNvtxs, c->GetNvtxs(), typeid(*this).name(), __func__, "Nvtxs")) || failed;
	for(int ivtx = 0; ivtx < fNvtxs; ivtx++) {
		failed = (!ComparisonPassed(fMode[ivtx],
		                            c->GetMode(),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("Mode[%d]", ivtx))) ||
		         failed;
		failed = (!ComparisonPassed(fVtxsvol[ivtx],
		                            c->GetVtxsvol(ivtx),
		                            typeid(*this).name(),
		                            __func__,
		                            TString::Format("Vtxvols[%d]", ivtx))) ||
		         failed;
		for(int i = 0; i < 4; i++) {
			failed = (!ComparisonPassed(fVtxs[ivtx][i],
			                            c->GetVtxs(ivtx, i),
			                            typeid(*this).name(),
			                            __func__,
			                            TString::Format("%s[%d][%d]", "Vtxs", ivtx, i))) ||
			         failed;
		}  // i
	}      // ivtx
	failed = (!ComparisonPassed(fVecRecNumber,
	                            c->GetVecRecNumber(),
	                            typeid(*this).name(),
	                            __func__,
	                            "VecRecNumber")) ||
	...
	failed = (!ComparisonPassed(fNumTubesHit,
	                            c->GetNumTubesHit(),
	                            typeid(*this).name(),
	                            __func__,
	                            "NumTubesHit")) ||
	         failed;
	failed = (!ComparisonPassed(fNumDigitizedTubes,
	                            c->GetNumDigiTubesHit(),
	                            typeid(*this).name(),
	                            __func__,
	                            "NumDigitizedTubes")) ||
	         failed;
	...
	ComparisonPassed(fNtrack_slots,
	                 c->GetNtrack_slots(),
	                 typeid(*this).name(),
	                 __func__,
	                 "Ntrack_slots (shouldn't necessarily be equal)");
	failed = (!ComparisonPassed(fNcherenkovhits,
	                            c->GetNcherenkovhits(),
	                            typeid(*this).name(),
	                            __func__,
	                            "Ncherenkovhits")) ||
	         failed;
	failed = (!ComparisonPassed(fNcherenkovhittimes,
	                            c->GetNcherenkovhittimes(),
	                            typeid(*this).name(),
	                            __func__,
	                            "Ncherenkovhittimes")) ||
	         failed;
	failed = (!ComparisonPassed(fNcherenkovdigihits,
	                            c->GetNcherenkovdigihits(),
	                            typeid(*this).name(),
	                            __func__,
	                            "Ncherenkovdigihits")) ||
	         failed;
	// don't expect this to pass in general, so don't affect failed
	ComparisonPassed(fNcherenkovdigihits_slots,
	                 c->GetNcherenkovdigihits_slots(),
	                 typeid(*this).name(),
	                 __func__,
	                 "Ncherenkovdigihits_slots (shouldn't necessarily be equal)");
	failed = (!ComparisonPassed(fSumQ, c->GetSumQ(), typeid(*this).name(), __func__, "SumQ")) || failed;
	failed = (!ComparisonPassed(fTriggerType,
	                            c->GetTriggerType(),
	                            typeid(*this).name(),
	                            __func__,
	                            "TriggerType")) ||
	         failed;
	failed = (!ComparisonPassedVec(fTriggerInfo,
	                               c->GetTriggerInfo(),
	                               typeid(*this).name(),
	                               __func__,
	                               "TriggerInfo")) ||
...
bool WCSimRootEvent::CompareAllVariables(const WCSimRootEvent* c, bool deep_comparison) const {
	...
	if(this->GetNumberOfEvents() != c->GetNumberOfEvents()) {
		cerr << "WCSimRootEvent::CompareAllVariables() Different number of events: "
		     << this->GetNumberOfEvents() << ", " << c->GetNumberOfEvents() << endl;
		failed = true;
	}
