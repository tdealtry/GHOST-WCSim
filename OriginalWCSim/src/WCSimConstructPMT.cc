...
G4LogicalVolume* WCSimDetectorConstruction::ConstructPMT(
    ...
    G4int nIDPMTs)  // Modified by B.Q, 2018/12 to incorporate number of ID PMTs in the function input
{
	...
	G4double reflectorRadius =
	    radius + id_reflector_height * tan(id_reflector_angle);  // PMT radius+ r = h * tan (theta)
	G4double reflectorThickness =
	    0.344 *
	    CLHEP::mm;  // the actual reflector thickness is 0.5 mm but due to solid works design, I am taking the
	                // horizontal component to calculate the reflector radius. Refer the CAD drawing.
	...
	if(nIDPMTs == 1) {
		...
		solidWCPMT = new G4Polycone("WCPMT",
		                            0.0 * deg,
		                            360.0 * deg,
		                            2,
		                            PMTHolderZ,
		...
		if(addPMTBase){
		  ...
		  baseHeight = expose;
		  baseRadius = std::max(radius,reflectorRadius) + reflectorThickness; //radius + reflectorThickness;
		too tight
		}
		*/
		...
		G4cout << mPMT_vessel_radius_curv << ", " << mPMT_outer_material_d << ", " << expose << ", "
		       << dist_pmt_vessel << ", radius curv = " << mPMT_vessel_radius_curv
		       << ", pmt module height = " << pmtModuleHeight << G4endl;
		...
		solidWCPMT = new G4Sphere("WCPMT",
		                          mPMT_vessel_radius_curv - mPMT_outer_material_d -
		                              pmtModuleHeight,  // rMin = 342 - 10 - 59.62 = 272.38 mm, 59.62mm is
		                                                // position of support structure
	...
	*/
	...
	if(nIDPMTs == 1)
		pmt_material_pmtAssembly = "Water";  // For single PMT, not gel, only pure PMT.
	else
		pmt_material_pmtAssembly =
		    mPMT_material_pmtAssembly;  // Else, use SiGel (defined inf WCSimDetectorConfigs).

	G4LogicalVolume* logicWCPMT = new G4LogicalVolume(
	    solidWCPMT,
	...
	G4Box* solidCutOffTubs =
	    new G4Box("cutOffTubs", sphereRadius + 1. * cm, sphereRadius + 1. * cm, PMTOffset);
	...
	G4SubtractionSolid* solidInteriorWCPMT =
	    new G4SubtractionSolid("InteriorWCPMT", tmpSolidInteriorWCPMT, solidCutOffTubs);
	...
	G4SubtractionSolid* solidGlassFaceWCPMT =
	    new G4SubtractionSolid(CollectionName, tmpGlassFaceWCPMT, solidCutOffTubs);

	G4LogicalVolume* logicGlassFaceWCPMT =
	    new G4LogicalVolume(solidGlassFaceWCPMT, G4Material::GetMaterial("Glass"), CollectionName, 0, 0, 0);
	...
	if(Vis_Choice == "RayTracer") {
		...
		// used in OGLSX visualizer
		// G4VisAttributes* WCPMTVisAtt = new G4VisAttributes(G4Colour(0.2,0.2,0.2));  //better for seeing
		// hits WCPMTVisAtt->SetForceWireframe(true);
		G4VisAttributes* WCPMTVisAtt =
		    new G4VisAttributes(G4Colour(0.0, 1.0, 0.0));  // better for seeing geometry
	...
	if(id_reflector_height > 0.1 * CLHEP::mm && (reflectorRadius - radius) > -5 * CLHEP::mm) {
		 ...
		 */
		G4Cons* reflectorCone = new G4Cons("WCPMT_reflect",
	...
	G4cout << id_reflector_height << ", " << id_reflector_height - radius << ", " << radius << ", "
	       << reflectorThickness << ", " << reflectorRadius << ", " << id_reflector_z_offset << ", "
	       << position_z_offset << G4endl;
	...
	if(nIDPMTs > 1) {
		...
		if(id_reflector_height > 0.1 * CLHEP::mm && (reflectorRadius - radius) > -5 * CLHEP::mm) {
			...
			// Holder of the PMT
			// G4double ReflectorHolderZ[3] = {0, id_reflector_z_offset,
			// id_reflector_z_offset+id_reflector_height}; G4double ReflectorHolderR[3] =
			// {tan(mPMT_pmt_openingAngle)*(mPMT_vessel_radius_curv - mPMT_outer_material_d -expose -
			// dist_pmt_vessel),
			//       tan(mPMT_pmt_openingAngle)*(mPMT_vessel_radius_curv - mPMT_outer_material_d-expose -
			//       dist_pmt_vessel+id_reflector_z_offset),
			//       tan(mPMT_pmt_openingAngle)*(mPMT_vessel_radius_curv - mPMT_outer_material_d-expose -
			//       dist_pmt_vessel+id_reflector_z_offset + id_reflector_height)};
			...
			//  Each set of (Z, R, r) defines an edge around the polycone by it's height position (z) inner
			...
			G4double ReflectorHolderR[4] = { 39.733 * mm,
				                             43.07 * mm,
				                             45.86 * mm,
				                             46.615 * mm };  // To avoid overflowing the mother cone
			G4double ReflectorHolderr[4] = { 26.75 * mm, 40.52 * mm, 40.52 * mm, 45.935 * mm };

			G4Polycone* solidWCPMTsupport2 = new G4Polycone("WCPMTsupport2",
			                                                0.0 * deg,
			                                                360.0 * deg,
			                                                4,
			                                                ReflectorHolderZ,
			...
			G4LogicalVolume* logicWCPMTsupport2 = new G4LogicalVolume(solidWCPMTsupport2,
			...
			double reflectorHolderZ = 272.36 * mm;  // position of z=0 of the reflector holder with respect to
			                                        // the origin of the mother volume
			new G4PVPlacement(0,
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructPMT(G4String PMTName,
	...
	if(Vis_Choice == "RayTracer") {
		...
		if(detectorElement == "OD")
			WCPMTVisAtt = new G4VisAttributes(G4Colour(1., 0., 0.));
		else
	...
	G4Sphere* tmp_PMT_holder =
	    new G4Sphere("tmp_PMT_holder", 0., sphereRadius, 0.0 * deg, 360.0 * deg, 0.0 * deg, 90.0 * deg);

	G4SubtractionSolid* PMT_holder = new G4SubtractionSolid("WCPMT", tmp_PMT_holder, solidCutOffTubs);
	...
	G4Polycone* solidWCPMT = new G4Polycone("WCPMT",
	                                        0.0 * deg,
	                                        360.0 * deg,
	                                        2,
	                                        PMTHolderZ,
	...
	if(WLS) {
		logicWCPMT = new G4LogicalVolume(PMT_holder, G4Material::GetMaterial("Water"), "WCPMT", 0, 0, 0);
	}
	else {
	...
	G4LogicalVolume* logicInteriorWCPMT =
	    new G4LogicalVolume(solidInteriorWCPMT, G4Material::GetMaterial("Air"), "InteriorWCPMT", 0, 0, 0);
	...
	// For either visualization type, logicGlassFaceWCPMT will either be visible or invisible depending on
	// which line is commented at the end of the respective if statements
	...
	if(Vis_Choice == "OGLSX") {
		...
		G4VisAttributes* WCPMTVisAtt;
		if(detectorElement == "OD")
			WCPMTVisAtt = new G4VisAttributes(G4Colour(1.0, 0.0, 0.0));
		else
			WCPMTVisAtt = new G4VisAttributes(G4Colour(0.2, 0.2, 0.2));
		WCPMTVisAtt->SetForceWireframe(true);
		// logicGlassFaceWCPMT->SetVisAttributes(G4VisAttributes::Invisible);
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructPMTAndWLSPlate(G4String PMTName,
	...
	G4cout << " create WLSPlate with inner radius " << radius / m << " m, half side "
	       << WCODWLSPlatesLength / 2 / m << " m, half thickness " << WCODWLSPlatesThickness / 2 / m
	       << " m, cladding thickness " << CladdingWidth / m << " m, PMT expose " << expose / m
	       << " m, sphereRadius " << sphereRadius / m << " m, WLS_plate_offset " << WLS_plate_offset / m
	       << " m" << G4endl;
	...
	G4cout << " qqqqqqqqqqqqqqqqqqqqqqqqqq plate_diagonal " << plate_diagonal << " expose " << expose
	       << G4endl;

	G4Polycone* container = new G4Polycone("rectangleWLS",
	                                       0.0 * deg,
	                                       360.0 * deg,
	                                       2,
	                                       PMTHolderZ,
	...
	G4LogicalVolume* logicContainer =
	    new G4LogicalVolume(container, G4Material::GetMaterial(Water), "WCODContainer", 0, 0, 0);

	G4VisAttributes* visContainer = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0));
	...
	// WLS
	G4Box* WLSPlateAndCladding = new G4Box("WLSPlateAndCladding",
	                                       (WCODWLSPlatesLength + 2 * CladdingWidth) / 2,
	                                       (WCODWLSPlatesLength + 2 * CladdingWidth) / 2,
	                                       WCODWLSPlatesThickness / 2);

	G4Box* WLSPlate =
	    new G4Box("WLSPlate", WCODWLSPlatesLength / 2, WCODWLSPlatesLength / 2, WCODWLSPlatesThickness / 2);

	// Need a volume to cut away excess behind blacksheet
	G4double PMTOffset = sphereRadius - expose;
	G4Box* solidCutOffTubs =
	    new G4Box("cutOffTubs", sphereRadius + 1. * cm, sphereRadius + 1. * cm, PMTOffset);
	G4Sphere* tmp_glass_outer_surface = new G4Sphere("tmp_glass_outer_surface",
	                                                 0.0 * m,
	                                                 sphereRadius,
	                                                 0.0 * deg,
	                                                 360.0 * deg,
	...
	G4SubtractionSolid* glass_outer_surface =
	    new G4SubtractionSolid("glass_outer_surface", tmp_glass_outer_surface, solidCutOffTubs);

	G4RotationMatrix* NullRotation = new G4RotationMatrix();
	G4Transform3D WLSplateTransform(
	    *NullRotation,
	    G4ThreeVector(0,
	                  0,
	                  -WCODWLSPlatesThickness / 2. - PMTOffset -
	                      WLS_plate_offset));  // center of glass outer surface in WLSPlate coordinates
	G4SubtractionSolid* extrudedWLS =
	    new G4SubtractionSolid("extrudedWLS", WLSPlate, glass_outer_surface, WLSplateTransform);
	...
	G4SubtractionSolid* WLSCladding = new G4SubtractionSolid("WLSCladding", WLSPlateAndCladding, WLSPlate);

	logicWCODWLSPlate =
	    new G4LogicalVolume(extrudedWLS, G4Material::GetMaterial(WLS_Material), "WCODWLSPlate", 0, 0, 0);

	G4VisAttributes* visWLS = new G4VisAttributes(G4Colour(0.0, 1.0, 1.0));
	...
	logicWCODWLSPlateCladding = new G4LogicalVolume(WLSCladding,
	...
	G4VisAttributes* visWLSCladding = new G4VisAttributes(G4Colour(1.0, 1.0, 1.0));
