...
WCSimWCDigitizerBase::WCSimWCDigitizerBase(G4String name,
    ...
    DigitizerClassName(""),
    DigitizerType(digitype) {
	G4String colName;
	if(detectorElement == "tank")
		colName = "WCDigitizedStoreCollection";
	else if(detectorElement == "tankPMT2")
		colName = "WCDigitizedStoreCollection2";
	else if(detectorElement == "OD")
		colName = "WCDigitizedStoreCollection_OD";
	collectionName.push_back(colName);
	ReInitialize();

#ifdef HYPER_VERBOSITY
	if(detectorElement == "OD")
		G4cout << "WCSimWCDigitizerBase::WCSimWCDigitizerBase ☆ recording collection name " << colName
...
void WCSimWCDigitizerBase::Digitize() {
	...
	if(detectorElement == "tank")
		...
		rawcollectionName = "WCRawPMTSignalCollection2";
	else if(detectorElement == "OD")
		rawcollectionName = "WCRawPMTSignalCollection_OD";
...
#ifdef HYPER_VERBOSITY
	if(detectorElement == "OD") {
		G4cout << "WCSimWCDigitizerBase::Digitize ☆ making digits collection (WCSimWCDigitsCollection*)"
		       << collectionName[0] << " for " << detectorElement << " and calling DigitizeHits on "
		       << rawcollectionName << " to fill it" << G4endl;
	}
#endif
	...
	if(WCHCPMT) {
		...
		G4cout << "WCSimWCDigitizerBase::Digitize didn't find hit collection for " << detectorElement
		       << G4endl;
	}
...
bool WCSimWCDigitizerBase::AddNewDigit(int tube,
	...
	if(tube < NPMTS_VERBOSE || tube == VERBOSE_PMT) {
		G4cout << "Adding hit " << gate << " in tube " << tube << " with time " << digihittime_d << " charge "
		       << peSmeared_d << " (truncated from t: " << digihittime << " q: " << peSmeared << ")"
...
#ifdef WCSIMWCDIGITIZER_VERBOSE
		if(tube < NPMTS_VERBOSE || tube == VERBOSE_PMT)
			G4cout << "DIGIT REJECTED with charge " << peSmeared_d << " time " << digihittime_d << G4endl;
#endif
		return false;
	}
}
...
void WCSimWCDigitizerSKI::DigitizeHits(WCSimWCDigitsCollection* WCHCPMT) {
	...
	if(detectorElement == "OD")
		G4cout << "OD # ";
#ifdef WCSIMWCDIGITIZER_VERBOSE
	G4cout << "WCSimWCDigitizerSKI::DigitizeHits START WCHCPMT->entries() = " << WCHCPMT->entries() << " hits"
	       << G4endl;
#endif
...
#ifdef WCSIMWCDIGITIZER_VERBOSE
		if(tube < NPMTS_VERBOSE || tube == VERBOSE_PMT) {
			G4cout << "tube " << tube << " totalpe = " << (*WCHCPMT)[i]->GetTotalPe() << " times";
		...
		for(G4int ip = 0; ip < (*WCHCPMT)[i]->GetTotalPe(); ip++) {
...
#ifdef WCSIMWCDIGITIZER_VERBOSE
			if(tube < NPMTS_VERBOSE || tube == VERBOSE_PMT)
				G4cout << "ip " << ip << " pe " << pe << " time " << time << " intgr_start " << intgr_start
				       << " upperlimit " << upperlimit << G4endl;
#endif
...
#ifdef WCSIMWCDIGITIZER_VERBOSE
				if(tube < NPMTS_VERBOSE || tube == VERBOSE_PMT)
					G4cout << "INFO: time " << time << " digi_id " << digi_unique_id << " p_id "
					       << photon_unique_id << G4endl;
#endif
			...
			if(MakeDigit) {
				...
				if(iflag == 0) {
					...
					bool accepted =
					    WCSimWCDigitizerBase::AddNewDigit(tube,
					                                      digi_unique_id,
	...
	for(G4int idigi = 0; idigi < DigiStore->entries(); idigi++) {
		...
		if(tubeid < NPMTS_VERBOSE || tubeid == VERBOSE_PMT) {
			...
			for(size_t i = 0; i < comp.size(); i++) {
				G4cout << "tube " << tubeid << " gate " << i << " p_id";
