...
// For MultiPMT: expose have different meaning, namely where to locate the sphere on the blacksheet ID/OD
// separator
G4LogicalVolume* WCSimDetectorConstruction::ConstructMultiPMT(G4String PMTName,
	...
	//  Not possible to have a mPMT without pressure mpmt_vessel now/yet.
	if(mPMT_vessel_radius <= 1. * CLHEP::mm && mPMT_vessel_cyl_height <= 1. * CLHEP::mm) {
		return ConstructPMT(PMTName, CollectionName, detectorElement, nIDPMTs);
	}

	// unique key for mPMT object. Also single PMT with pressure mPMT_vessel!
	...
	G4cout << "Vessel height: " << mPMT_vessel_cyl_height << G4endl;
	G4cout << "Vessel radius: " << mPMT_vessel_radius << G4endl;
	G4cout << "Vessel radius curv: " << mPMT_vessel_radius_curv << G4endl;
	...
	// WCSimPMTObject *PMT = GetPMTPointer(CollectionName);
	// G4double expose =  PMT->GetExposeHeight(); // THIS is the PMT expose, which I'm currently using for
	// pressure mPMT_vessel construction
	...
	G4double mPMT_vessel_cap_height =
	    mPMT_vessel_radius_curv -
	    sqrt(mPMT_vessel_radius_curv * mPMT_vessel_radius_curv - mPMT_vessel_radius * mPMT_vessel_radius);

	G4double mPMT_zRange_outer[2] = {
		0,  // must be zero or it will collide with the blacksheet and mess up the G4Navigator
		mPMT_vessel_cyl_height + mPMT_vessel_cap_height
	};
	G4double mPMT_RRange_outer[2] = { mPMT_vessel_radius, mPMT_vessel_radius };
	...
	G4Polycone* solidMultiPMT = new G4Polycone("WCmPMT",
	                                           0.0 * deg,
	                                           360.0 * deg,
	                                           2,
	                                           mPMT_zRange_outer,
	...
	G4LogicalVolume* logicWCMultiPMT =
	    new G4LogicalVolume(solidMultiPMT, G4Material::GetMaterial("Water"), "WCMultiPMT", 0, 0, 0);

	////////////////////////////////////////////////
	/// 2)Acrylic/Glass shell of the mPMT vessel ///
	////////////////////////////////////////////////
	G4Sphere* mPMT_top_sphere_vessel = new G4Sphere("WCmPMT_tsphere_vessel",
	                                                0,
	                                                mPMT_vessel_radius_curv,
	                                                0.0 * deg,
	                                                360.0 * deg,
	...
	cap_position_offset =
	    -1.0 * (mPMT_vessel_radius_curv -
	            mPMT_vessel_cap_height);  // lower position of cap by the piece that we cut off.
	G4LogicalVolume* logic_mPMT_vessel;

	// CAP
	if(mPMT_vessel_radius_curv - mPMT_vessel_radius > .1 * mm) {
		G4Box* solidCutOffTubs = new G4Box("cutOffTubs",
		                                   mPMT_vessel_radius_curv + 1. * cm,                  // x/2
		                                   mPMT_vessel_radius_curv + 1. * cm,                  // y/2
		                                   mPMT_vessel_radius_curv - mPMT_vessel_cap_height);  // z/2

		mPMT_top_cap_vessel =
		    new G4SubtractionSolid("InteriorWCPMT", mPMT_top_sphere_vessel, solidCutOffTubs);

		// G4cout << "DEBUG MODE " << vessel_radius_curv - vessel_cap_height << " " <<vessel_cap_height <<
		// G4endl;

		// cap_position_offset = -1.0* (vessel_radius_curv - vessel_cap_height); //lower position of cap by
		// the piece that we cut off.
		...
		if(mPMT_vessel_cyl_height < 0.01 * mm) {
			...
			G4Tubs* mPMT_cylinder_vessel = new G4Tubs("WCmPMT_cyl_vessel",
			                                          0.,
			                                          mPMT_vessel_radius,
			                                          mPMT_vessel_cyl_height / 2.,
			                                          0.0 * deg,
			                                          360.0 * deg);
			...
			*/
			...
			G4UnionSolid* union_vessel =
			    new G4UnionSolid("VesselUnion",
			                     mPMT_cylinder_vessel,
			                     mPMT_top_cap_vessel,
			                     0,
			                     G4ThreeVector(0, 0, cap_position_offset + mPMT_vessel_cyl_height / 2));
			...
			// Place Vessel
			new G4PVPlacement(0,                                                // its rotation
			                  G4ThreeVector(0, 0, mPMT_vessel_cyl_height / 2),  // its position
			                  logic_mPMT_vessel,                                // its logical volume
		...
		if(mPMT_vessel_cyl_height < 0.01 * mm) {
			...
			// Make union solid: PMTs can extend in cylindrical half (as the support structure)
			//                   to avoid overlaps, needs to be one solid.
			G4UnionSolid* union_vessel = new G4UnionSolid("VesselUnion",
			                                              mPMT_cylinder_vessel,
			                                              mPMT_top_sphere_vessel,
			                                              0,
			                                              G4ThreeVector(0, 0, mPMT_vessel_cyl_height / 2));
	...
	G4double innerR_curv_container =
	    mPMT_vessel_radius_curv - mPMT_outer_material_d - pmtModuleHeight;  //-expose - dist_pmt_vessel
	...
	G4double innerR_container = sqrt(innerR_curv_container * innerR_curv_container -
	                                 (mPMT_vessel_radius_curv - mPMT_vessel_cap_height) *
	                                     (mPMT_vessel_radius_curv - mPMT_vessel_cap_height));
	G4double outerR_container = sqrt(outerR_curv_container * outerR_curv_container -
	                                 (mPMT_vessel_radius_curv - mPMT_vessel_cap_height) *
	                                     (mPMT_vessel_radius_curv - mPMT_vessel_cap_height));

	// G4cout << "Inner container radii: Rcurv " << innerR_curv_container << ", R: " << innerR_container <<
	// G4endl; G4cout << "Outer container radii: Rcurv " << outerR_curv_container << ", R: " <<
	// outerR_container << G4endl;

	if(addPMTBase || nIDPMTs == 1)
		innerR_container = 0;

	G4Sphere* mPMT_top_sphere_container = new G4Sphere("WCmPMT_tsphere_cont",
	                                                   innerR_curv_container,
	                                                   outerR_curv_container,
	                                                   0.0 * deg,
	                                                   360.0 * deg,
	                                                   0.0 * deg,
	                                                   90.0 * deg);
	...
	// CAP inner
	if(mPMT_vessel_radius_curv - mPMT_vessel_radius > .1 * mm) {
		G4Box* solidCutOffContainer = new G4Box("cutOffContainer",
		                                        outerR_curv_container + 1. * cm,
		                                        outerR_curv_container + 1. * cm,
		                                        mPMT_vessel_radius_curv - mPMT_vessel_cap_height);

		mPMT_top_cap_container =
		    new G4SubtractionSolid("InteriorWCPMT", mPMT_top_sphere_container, solidCutOffContainer);
		...
		if(mPMT_vessel_cyl_height < 0.01 * mm) {
			...
			// CAP + CYLINDER inner
			G4Tubs* mPMT_cylinder_container = new G4Tubs("WCmPMT_cyl_container",
			                                             innerR_container,
			                                             outerR_container,
			                                             mPMT_vessel_cyl_height / 2.,
			                                             0.0 * deg,
			...
			G4UnionSolid* union_container =
			    new G4UnionSolid("ContainerUnion",
			                     mPMT_cylinder_container,
			                     mPMT_top_cap_container,
			                     0,
			                     G4ThreeVector(0, 0, cap_position_offset + mPMT_vessel_cyl_height / 2));

			G4cout << mPMT_cylinder_container << ", " << mPMT_top_cap_container << ", "
			       << cap_position_offset + mPMT_vessel_cyl_height / 2 << G4endl;
			G4cout << innerR_curv_container << ", " << outerR_curv_container << ", " << innerR_container
			       << ", " << outerR_container << G4endl;
			logic_mPMT_container = new G4LogicalVolume(
			    union_container,
			    G4Material::GetMaterial(mPMT_inner_material),  //"SilGel"), //whole area between pressure
		...
		if(mPMT_vessel_cyl_height < 0.01 * mm) {
			...
			G4UnionSolid* union_container = new G4UnionSolid("ContainerUnion",
			                                                 mPMT_cylinder_container,
			                                                 mPMT_top_sphere_container,
			                                                 0,
			                                                 G4ThreeVector(0, 0, mPMT_vessel_cyl_height / 2));
	...
	G4cout << "Distance from the Z axis and corrected for z0 of individual PMTs = " << pmtDistance << " mm"
	       << G4endl;
	...
	if(nIDPMTs == 1) {
		...
		if(mPMT_vessel_cyl_height < 0.01 * mm)
			...
			single_pmt_z_offset = -mPMT_vessel_cyl_height / 2;
		// singlePMT: OK now!
		new G4PVPlacement(0,
		...
		if(NbOfTotPmt != nIDPMTs) {
			G4cerr << "ERROR: Requested " << nIDPMTs << " PMTs but only acquired " << NbOfTotPmt
	...
	G4VPVParameterisation* pmtParam_id = new WCSimMultiPMTParameterisation(
	    nIDPMTs,        // NoPMTs
	...
	G4VPhysicalVolume* hemisphere = new G4PVParameterised("pmt",                 // their name
  ...
  */
	...
	*/
	...
	if(!addPMTBase) {
		...
		if(mPMT_vessel_cyl_height > 0) {
			G4Tubs* inner_cylinder = new G4Tubs("inn_cyl",
			                                    0.,
			                                    innerR_container,
			                                    mPMT_vessel_cyl_height / 2,
			                                    0.0 * deg,
		...
		G4Sphere* inner_top_sphere = new G4Sphere("WCmPMT_tsphere",
		                                          0,
		                                          innerR_curv_container,
		                                          0.0 * deg,
		                                          360.0 * deg,
		...
		if(mPMT_vessel_radius_curv - mPMT_vessel_radius > .1 * mm) {
			...
			G4Box* solidCutOffInner = new G4Box("cutOffInner",
			                                    innerR_curv_container + 1. * cm,                    // x/2
			                                    innerR_curv_container + 1. * cm,                    // y/2
			                                    mPMT_vessel_radius_curv - mPMT_vessel_cap_height);  // z/2

			inner_top_cap = new G4SubtractionSolid("InteriorWCPMT", inner_top_sphere, solidCutOffInner);

			logic_mPMT_top_sphere_inner = new G4LogicalVolume(inner_top_cap,
			                                                  G4Material::GetMaterial("Blacksheet"),
			...
			logic_mPMT_top_sphere_inner = new G4LogicalVolume(inner_top_sphere,
			                                                  G4Material::GetMaterial("Blacksheet"),
			                                                  "WCMultiPMT_inner_top",
			                                                  0,
			                                                  0,
...
G4int WCSimDetectorConstruction::FillCircles(void) {
	...
	// G4cout << "Test: vNic: " << vNiC.size() << " vAlpha: " << vAlpha.size() << " vCircle: " <<
	// vCircle.size() << G4endl;
...
G4int WCSimDetectorConstruction::CountPMT(G4int NoPmt) {
	...
	if(alphaNext > theta_min + fEta) {
		...
		G4cout << "Circle n. " << NoCircle << " - Number of PMTs: " << vNiC[NoCircle - 1]
		       << " - alpha: " << vAlpha[NoCircle - 1] * 180 / 3.141592 << " - eta " << fEta * 180 / 3.141592
		       << G4endl;
		NoCircle++;
	}
	...
	do {
		...
		if(NiCNext < 2) {
			break;
		}  // Prevents negative values of NiC
		if(alphaNext > theta_min + fEta) {
			vAlpha.push_back(alphaNext);
			vNiC.push_back(NiCNext);
			G4cout << "Circle n. " << NoCircle << " - Number of PMTs: " << vNiC[NoCircle - 1]
			       << " - alpha: " << vAlpha[NoCircle - 1] * 180 / 3.141592 << " - eta "
			       << fEta * 180 / 3.141592 << G4endl;
			G4cout << "Circle n. " << NoCircle << " - Number of PMTs: " << vNiC[NoCircle - 1]
			       << " - alpha: " << vAlpha[NoCircle - 1] * 180 / 3.141592 << " - eta "
			       << fEta * 180 / 3.141592 << G4endl;
			NoCircle++;
	...
	G4cout << "Percentage of covered hemispherical surface = " << TotPmt * (1 - std::cos(fEta)) * 100 << "%"
	       << G4endl;
	G4cout << "Percentage of covered hemispherical surface above theta_min = "
	       << TotPmt * (1 - std::cos(fEta)) * 100 / (1 - std::cos(CLHEP::pi / 2 - theta_min)) << "%"
	       << G4endl;
	G4cout << "Test: vNic: " << vNiC.size() << " vAlpha: " << vAlpha.size() << " vCircle: " << vCircle.size()
	       << G4endl;
