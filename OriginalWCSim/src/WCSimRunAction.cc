...
WCSimRunAction::WCSimRunAction(WCSimDetectorConstruction* test, WCSimRandomParameters* rand) :
	...
	useDefaultROOTout = true;  // false;  TF: ToDo, make this false WHEN flat ROOT has RooTracker trees and
	                           // when FiTQun can read that in.
...
void WCSimRunAction::BeginOfRunAction(const G4Run* /*aRun*/) {
	...
	if(useDefaultROOTout) {
		...
		// TBranch *branch = tree->Branch("wcsimrootsuperevent", "Jhf2kmrootsuperevent", &wcsimrootsuperevent,
		// bufsize,0);
		wcsimrooteventbranch =
		    WCSimTree->Branch("wcsimrootevent", "WCSimRootEvent", &wcsimrootsuperevent, bufsize, 2);
		wcsimrooteventbranch2 =
		    WCSimTree->Branch("wcsimrootevent2", "WCSimRootEvent", &wcsimrootsuperevent2, bufsize, 2);
		wcsimrooteventbranch_OD =
		    WCSimTree->Branch("wcsimrootevent_OD", "WCSimRootEvent", &wcsimrootsuperevent_OD, bufsize, 2);
	...
	geomTree->Branch(
	    "GeometryType",
	    geo_type_string,
	    "GeometryType[20]/C");  // example: for std::string data_ : tree->Branch(branchname.c_str(),
	                            // (void*)data_->c_str(),leafdescription.c_str());
	...
	cherenkovHitsTree->Branch("NHits_noDN",
	                          &(evNtup->totalNumHits_noNoise),
	                          "NHits_noDN/I");  // #PMTs x #(Ch)hits/PMTs
...
void WCSimRunAction::EndOfRunAction(const G4Run*) {
	...
	//    G4cout <<
	//    ((double(numberOfTimesMRDHit)+double(numberOfTimesFGDHit))/double(numberOfEventsGenerated))*100.
	...
	if(useDefaultROOTout) {
		...
		wcsimrootsuperevent2 = 0;
		delete wcsimrootsuperevent_OD;
		wcsimrootsuperevent_OD = 0;
...
void WCSimRunAction::FillFlatGeoTree() {
	...
	if(wcsimdetector->GetIsEggShapedHyperK())
		...
		strcpy(geo_type_string, "Cylinder");
	...
	for(unsigned int i = 0; i != fpmts->size(); i++) {
		...
		mPMT_pmt_id[i] = (tube_id[i] % wcsimdetector->GetmPMT_nID() == 0 ?
		                      wcsimdetector->GetmPMT_nID() :
		                      tube_id[i] % wcsimdetector->GetmPMT_nID());  // No. 1 to nID
		                                                                   // cylLoc = pmt->Get_cylocation();
	}
