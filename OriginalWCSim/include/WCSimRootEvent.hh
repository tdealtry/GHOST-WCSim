...
class WCSimRootCherenkovDigiHit : public TObject {
	...
	public:
		...
		WCSimRootCherenkovDigiHit(Double_t q,
		                          Double_t t,
		                          Int_t tubeid,
		                          Int_t mpmtid,
		                          Int_t mpmt_pmtid,
...
class WCSimRootTrigger : public TObject {
	...
	private:
		...
		Int_t fNtrack_slots;    // Number of slots in the tracks array. This is potentially more than fNtrack
		                        // (i.e. if any tracks have been removed that aren't at the very start/end)
		...
		Int_t fNcherenkovdigihits_slots;  // Number of slots in the digihits array. This is potentially more
		                                  // than fNcherenkovdigihits (i.e. if any digihits have been removed
		                                  // that aren't at the very start/end)
		Float_t fSumQ;
		...
		std::vector<Double_t> fTriggerInfo;  // Information about how it passed the trigger (e.g. how many
		                                     // hits in the NDigits window)
	...
	public:
		...
		TLorentzVector Get4Vertex(Int_t i) {
			return TLorentzVector(fVtxs[i][0], fVtxs[i][1], fVtxs[i][2], fVtxs[i][3]);
		}
		...
		}  // don't use fNtrack_slots directly as it doesn't take into account automatic TClonesArray
		   // shortening when tracks at start/end are removed
		...
		Int_t GetNcherenkovdigihits_slots() const {
			return fCherenkovDigiHits->GetLast() + 1;
		}  // don't use fNcherenkovdigihits_slots directly as it doesn't take into account automatic
		   // TClonesArray shortening when digits at start/end are removed
...
class WCSimRootEvent : public TObject {
	public:
		...
		const WCSimRootTrigger* GetTrigger(int number) const {
			return (WCSimRootTrigger*)(*fEventList)[number];
		}
		...
		*/
		...
		void ReInitialize() {  // need to remove all subevents at the end, or they just get added anyway...
			for(int i = fEventList->GetLast(); i >= 1; i--) {
				...
				WCSimRootTrigger* tmp = dynamic_cast<WCSimRootTrigger*>(fEventList->RemoveAt(i));
				delete tmp;
				// G4cout <<"done !\n";
			}
			Current               = 0;
