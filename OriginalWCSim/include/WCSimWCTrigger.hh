...
class WCSimWCTriggerBase : public G4VDigitizerModule {
	...
	public:
		...
		/// The main user-callable routine of the class. Gets the input & creates the output
		/// WCSimWCTriggeredDigitsCollection's, then calls DoTheWork()
		void Digitize();
		...
		/// Set the mode for saving failed triggers (0:save only triggered events, 1:save both triggered
		/// events & failed events, 2:save only failed events)
	...
	protected:
		...
		/// Set the default trigger class specific decision of whether to save multiple digits per PMT per
		/// trigger (overridden by .mac)
		...
		// they are stored here so that different trigger classes can use the same algorithms without copying
		// code
		/**
		 * \brief An NDigits trigger algorithm
		 *
		 * Looks through the input WCSimWCDigitsCollection and integrates the number of hits in a (specified)
		 * time window If the integral passes above a (specified) threshold, a trigger is issued
		 *
		 * The trigger type is kTriggerNDigits
		 *
		 * The trigger time is the time of the first digit above threshold
		 *
		 * The trigger information is the number of hits in the time window (i.e. the number of hits that
		 * caused the trigger to fire)
		 *
		 * Currently setup with the optional 'test' argument which runs the algorithm with half the hit
		 * threshold for testing purposes. Triggers issued in this mode have type kTriggerNDigitsTest
		 */
		...
		WCSimWCTriggeredDigitsCollection*
		    DigitsCollection;  ///< The main output of the class - collection of digits in the trigger window
		std::map<int, int> DigiHitMap;  ///< Keeps track of the PMTs that have been added to the output
		                                ///< WCSimWCTriggeredDigitsCollection
		...
		std::vector<std::vector<Double_t> >
		    TriggerInfos;  ///< Additional information associated with each trigger

		WCSimWCDAQMessenger* DAQMessenger;  ///< Get the options from the .mac file
		WCSimDetectorConstruction*
		    myDetector;  ///< Know about the detector, so can add appropriate PMT time smearing
		...
		G4bool multiDigitsPerTrigger;  ///< Allow the number of digits per PMT saved in each trigger window to
		                               ///< go > 1?
		// NDigits
		G4int ndigitsThreshold;          ///< The threshold for the NDigits trigger
		G4int ndigitsWindow;             ///< The time window for the NDigits trigger
		G4bool ndigitsAdjustForNoise;    ///< Automatically adjust the NDigits trigger threshold based on the
		                                 ///< average dark noise rate?
		...
		G4int
		    saveFailuresPostTriggerWindow;  ///< The posttrigger window to save after an SaveFailures trigger
	...
	private:
		...
		void FillDigitsCollection(WCSimWCDigitsCollection* WCDCPMT,
		                          bool remove_hits,
...
class WCSimWCDigiTrigger : public G4VDigi {
	...
	private:
		...
		std::multimap<int, std::vector<int> >
		    fDigiComp;  ///< Stores the unique IDs of each photon making up a digit
...
class WCSimWCTriggerNDigits : public WCSimWCTriggerBase {
	public:
		...
		WCSimWCTriggerNDigits(G4String name,
		                      WCSimDetectorConstruction*,
		                      WCSimWCDAQMessenger*,
		                      G4String detectorElement);
...
class WCSimWCTriggerNoTrigger : public WCSimWCTriggerBase {
	public:
		...
		WCSimWCTriggerNoTrigger(G4String name,
		                        WCSimDetectorConstruction*,
		                        WCSimWCDAQMessenger*,
		                        G4String detectorElement);
...
class WCSimWCTriggerNDigits2 : public WCSimWCTriggerBase {
	public:
		...
		WCSimWCTriggerNDigits2(G4String name,
		                       WCSimDetectorConstruction*,
		                       WCSimWCDAQMessenger*,
		                       G4String detectorElement);
