...
enum mPMT_orientation { VERTICAL, HORIZONTAL, PERPENDICULAR };
...
class WCSimDetectorConstruction : public G4VUserDetectorConstruction {
	public:
		...
		void SetNuPrismGeometry(G4String PMTType,
		...
		void SetLCType(G4int LightCollectorType) { LCType = LightCollectorType; };
		...
		G4double GetStackingPMTQE(G4double PhotonWavelength,
		                          G4int flag,
		                          G4double low_wl,
		                          G4double high_wl,
		                          G4double ratio);
		...
		WCSimPMTObject* GetPMTPointer(G4String CollectionName) {
			...
			if(PMTptr == NULL) {
				G4cout << CollectionName << " is not a recognized hit collection. Exiting WCSim." << G4endl;
				exit(1);
			}
			return PMTptr;
		}
		...
		void SetPMTCoverage(G4double cover) { WCPMTPercentCoverage = cover; }

		G4double GetPMTCoverage() { return WCPMTPercentCoverage; }

		// Set PMT coverage of second PMT type
		void SetPMTCoverage2(G4double cover) { WCPMTPercentCoverage2 = cover; }
		...
		void SetmPMT_VesselRadius(G4double radius) {
			...
			if(!fix_nModules) {
				WCBarrelNumPMTHorizontal = std::lround(WCIDDiameter * sqrt(CLHEP::pi * WCPMTPercentCoverage) /
				                                       (10.0 * mPMT_vessel_radius));
				if(WCBarrelNumPMTHorizontal < 1)
					G4cerr << "Bug in dimensions as less than 1 BarrelPMT specified" << G4endl;
				WCBarrelNRings =
				    std::lround((WCBarrelNumPMTHorizontal *
				                 ((WCIDHeight - 2 * WCBarrelPMTOffset) / (CLHEP::pi * WCIDDiameter))) /
				                WCPMTperCellVertical);
				WCCapPMTSpacing = CLHEP::pi * WCIDDiameter / WCBarrelNumPMTHorizontal;
				WCCapEdgeLimit =
				    WCIDDiameter / 2.0 - mPMT_vessel_tot_height;  // CapEdgeLimit needs PMT/mPMT height
			}
		}

		void SetmPMT_VesselRadiusCurv(G4double radius) { mPMT_vessel_radius_curv = radius; }
		...
		void SetmPMT_nID2(G4int nPMTs) {
			nID_PMTs2 = nPMTs;
		}  // Added by B.Quilain for the second type of photosensor in the hybrid version
		...
		G4String GetPMTtype_OD(void) {
			return mPMT_OD_PMT;
		};  // might want to replace the name mPMT by general var name
		...
		void SetPMTType(G4String type) {
			...
			WCBarrelNumPMTHorizontal =
			    std::lround(WCIDDiameter * sqrt(CLHEP::pi * WCPMTPercentCoverage / 100.0) / WCPMTRadius);
			WCBarrelNRings = std::lround((WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) /
			                                                          (CLHEP::pi * WCIDDiameter))) /
			                             WCPMTperCellVertical);
		...
		void   SetPMTCoverage(G4double cover) {
		  ...
		  if(WCDetectorName == "NuPRISM_mPMT" || WCDetectorName == "HyperK_mPMT"){
		    WCBarrelNumPMTHorizontal =
		std::lround(WCIDDiameter*sqrt(CLHEP::pi*WCPMTPercentCoverage/100.0)/mPMT_vessel_radius); } else
		    WCBarrelNumPMTHorizontal =
		std::lround(WCIDDiameter*sqrt(CLHEP::pi*WCPMTPercentCoverage/100.0)/WCPMTRadius); WCBarrelNRings =
		std::lround((WCBarrelNumPMTHorizontal*((WCIDHeight-2*WCBarrelPMTOffset)/(CLHEP::pi*WCIDDiameter)))/WCPMTperCellVertical);
		  WCCapPMTSpacing       = CLHEP::pi*WCIDDiameter/WCBarrelNumPMTHorizontal;
		}
		G4double GetPMTCoverage() {return WCPMTPercentCoverage;}
		*/
		...
		void SetDetectorDiameter(G4double diameter) {
			...
			if(WCDetectorName == "NuPRISM_mPMT") {
				WCBarrelNumPMTHorizontal = std::lround(
				    WCIDDiameter * sqrt(CLHEP::pi * WCPMTPercentCoverage / 100.0) / mPMT_vessel_radius);
		...
		G4String GetIDCollectionName2() {
			return WCIDCollectionName2;
		}  // Added by B.Quilain for hybrid PMT configuration

		G4String GetODCollectionName() { return WCODCollectionName; }

		WCSimTuningParameters* GetParameters() {
			return WCSimTuningParams;
		}  // Added by TD to set up a parameter on PMT TTS uncertainty and use it in WCSimWCPMT
		...
		void SetWCPMTODSize(G4String WCPMTODSize) {
			if(WCPMTODSize == "PMT8inch" || WCPMTODSize == "PMT5inch" ||
			   WCPMTODSize == "PMT3inch_ETEL9302B") {
	...
	private:
		...
		G4LogicalVolume* ConstructPMT(
		    ...
		    G4int nIDPMTs            = 1);  // Modified by B.Quilain 2018/12 to implement hybrid detector
		G4LogicalVolume* ConstructMultiPMT(G4String,
		                                   G4String,
		                                   G4String detectorElement = "tank",
		                                   G4int nIDPMTs            = 1);

		G4LogicalVolume* ConstructPMT(G4String,
		                              G4String,
		                              G4String detectorElement = "tank",
		                              bool WLS                 = false);
		...
		typedef void (WCSimDetectorConstruction::*DescriptionFcnPtr)(G4VPhysicalVolume*,
		                                                             int,
		                                                             int,
		                                                             const G4Transform3D&);
		...
		void DescribeAndDescendGeometry(G4VPhysicalVolume*,
		                                int,
		                                int,
		                                const G4Transform3D&,
		                                DescriptionFcnPtr);
		...
		G4LogicalVolume*
		    ConstructRadialPMT(G4bool, G4double, G4double, G4double, G4double, G4double, G4double);
		...
		G4double
		    WCCylInfo[3];  // Info for the geometry tree: radius & length or mail box, length, width and depth
		...
		G4ThreeVector WCXRotation;  // Info for the geometry tree: WC detector local X axis in the global
		                            // coordinate system
		G4ThreeVector WCYRotation;  // Info for the geometry tree: WC detector local Y axis in the global
		                            // coordinate system
		G4ThreeVector WCZRotation;  // Info for the geometry tree: WC detector local Z axis in the global
		                            // coordinate system
		...
		G4double
		    mPMT_vessel_radius_curv;  // radius of the sphere to determine curvature of cap of pressure vessel
		G4double mPMT_vessel_radius;  // radius of the pressure vessel (spherical cap)
		G4double dist_pmt_vessel;     // distance between glass of pmt and inner radius of pressure vessel
		                              // (region where water/gel lives)
