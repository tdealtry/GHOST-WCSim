...
class WCSimWCHit : public G4VHit {
	...
	public:
		...
		void SetTubeType(G4String tube_type) {
			tubeType = tube_type;
		};  // Added by B.Quilain to transmit on which PMT type the hit happened. For detectors with several
		    // PMT types in ID.
	...
	private:
		...
		G4String tubeType;  // Added by B.Quilain to transmit on which PMT type the hit happened. For
		                    // detectors with several PMT types in ID.
