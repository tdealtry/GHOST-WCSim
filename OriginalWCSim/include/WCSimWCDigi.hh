...
class WCSimWCDigi : public G4VDigi {
	...
	private:
		...
		std::vector<G4double>
		    time_double;  ///< Same information as "time" but stored in a vector for quick time sorting
		...
		std::map<int, G4ThreeVector>
		    photonStartPos;  ///< Start point of the photon of the Hit (do not use for Digits)
		std::map<int, G4ThreeVector>
		    photonEndPos;  ///< End point of the photon of the Hit (do not use for Digits)
		std::map<int, G4ThreeVector>
		    photonStartDir;  ///< Start dir of the photon of the Hit (do not use for Digits)
		std::map<int, G4ThreeVector>
		    photonEndDir;  ///< End dir of the photon of the Hit (do not use for Digits)
	...
	public:
		...
		inline void AddGate(int g, double t) {
			Gates.insert(g);
		...
		inline void SetPhotonStartPos(G4int gate, const G4ThreeVector& position) {
			photonStartPos[gate] = position;
		};

		inline void SetPhotonEndPos(G4int gate, const G4ThreeVector& position) {
			photonEndPos[gate] = position;
		};

		inline void SetPhotonStartDir(G4int gate, const G4ThreeVector& direction) {
			photonStartDir[gate] = direction;
		};

		inline void SetPhotonEndDir(G4int gate, const G4ThreeVector& direction) {
			photonEndDir[gate] = direction;
		};
		...
		inline G4double GetTime(int gate) {
			try {
				...
				G4cerr
				    << "Exception occurred while attempting to use WCSimWCDigi::GetTime to retrieve time for pe "
				    << gate << " from map of times. The time map has " << time.size()
				    << " entries:" << G4endl;
				for(std::map<int, double>::iterator time_element = time.begin(); time_element != time.end();
				    time_element++) {
					try {
		...
		void SortDigiMapsByHitTime() {
			...
			// SortDigiMapsByHitTime is called by WCSimWCDigitizerSKI::DigitizeHits to sort the
			// WCRawPMTSignalCollection. Each entry in WCRawPMTSignalCollection represents the set of photon
			// hits on a PMT. Since a photon hit has no "composition", fDigiComp is empty at this time and
			// needn't be sorted. for generality, sort if the digi composition map has the same size as other
			// maps
