...
class WCSimWCDigitizerBase : public G4VDigitizerModule {
	public:
		...
		WCSimWCDigitizerBase(G4String name,
		...
		double Truncate(double value, double precision) {
			...
			if(ratio > std::numeric_limits<int64_t>::max() || ratio < std::numeric_limits<int64_t>::min())
				return value;
	...
	protected:
		...
		G4String detectorElement;  // to identify which hit collection the trigger is associated: OD, ID, ID
		                           // PMT type 2(for hybrid)
		...
		// generic digitizer properties. Defaults set with the GetDefault*() methods. Overidden by .mac
		// options
		...
		virtual int GetDefaultDeadTime() = 0;  ///< Set the default digitizer-specific deadtime (in ns)
		                                       ///< (overridden by .mac)
		virtual int GetDefaultIntegrationWindow()  = 0;  ///< Set the default digitizer-specific integration
		                                                 ///< window (in ns) (overridden by .mac)
		virtual double GetDefaultTimingPrecision() = 0;  ///< Set the default digitizer-specific timing
		                                                 ///< resolution (in ns) (overridden by .mac)
		virtual double GetDefaultPEPrecision() = 0;  ///< Set the default digitizer-specific charge resolution
		                                             ///< (in p.e.) (overridden by .mac)

		void GetVariables();  ///< Get the default deadtime, etc. from the derived class, and override with
		                      ///< read from the .mac file
};
...
class WCSimWCDigitizerSKI : public WCSimWCDigitizerBase {
	public:
		...
		WCSimWCDigitizerSKI(G4String name,
		                    WCSimDetectorConstruction*,
		                    WCSimWCDAQMessenger*,
		                    G4String detectorElement);
		~WCSimWCDigitizerSKI();
