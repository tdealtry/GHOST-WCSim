...
class WCSimRunAction : public G4UserRunAction {
	...
	public:
		...
		TBranch* GetBranch(G4String detectorElement = "tank") {
			if(detectorElement == "tank")
				return wcsimrooteventbranch;
			else if(detectorElement == "tankPMT2")
				return wcsimrooteventbranch2;
			else if(detectorElement == "OD")
				return wcsimrooteventbranch_OD;
			else
				G4cout << "Unkown detector element: " << detectorElement << G4endl;
			return nullptr;
		}
		...
		WCSimRootEvent* GetRootEvent(G4String detectorElement = "tank") {
			if(detectorElement == "tank")
				...
				return wcsimrootsuperevent2;
			else if(detectorElement == "OD")
				return wcsimrootsuperevent_OD;
			else
				G4cout << "Unkown detector element: " << detectorElement << G4endl;
			return nullptr;
		}
		...
		void SetTriggerInfoNew(TriggerType_t trigger_type,
		                       int trigger_info, /* std::vector<G4double> trigger_info, */
		...
		void SetBranch(TBranch* branchin, G4String detectorElement = "tank") {
			if(detectorElement == "tank")
				wcsimrooteventbranch = branchin;
			else if(detectorElement == "tankPMT2")
				wcsimrooteventbranch2 = branchin;
			else if(detectorElement == "OD")
				wcsimrooteventbranch_OD = branchin;
		}
		...
		void SetRootEvent(WCSimRootEvent* revent, G4String detectorElement = "tank") {
			if(detectorElement == "tank")
				...
				wcsimrootsuperevent2 = revent;
			else if(detectorElement == "OD")
				wcsimrootsuperevent_OD = revent;
		}
	...
	private:
		...
		enum CylinderLocation { UNDEFINED = -1, WALL = 0, TOPCAP = 10, BOTTOMCAP = 20 } cylLocation[5000000];
